...
**Last updated:** 2026-05-18 — **Session 279.** ~30-min cleanup. Three things shipped at the cache-proxy/PG layer (no Android build this session). (1) Vite web admin unblocked — apostrophe-in-single-quoted-string at `AdminSidebar.tsx:51`. (2) Cache-proxy restarted — running PID 1294880 was pre-S274 rebrand and looping `relation "salem_passport_filters" does not exist`. (3) **TOUR_0001 + Whole-Salem global pool both deprecated** per operator (*"DrK's Tour is the default"*): transactionally deleted `tour_WD1` (15 stops / 14 legs / 41-entry collection) + `default_salem_walking` (447 entries); set sort_orders drks=0 / density=1 / longwalk=2; removed dead `default_salem_walking` ORDER BY pin from `cache-proxy/lib/admin-collection.js:567`. **PG end-state: 3 tours / 3 collections (all tour-bound) / 309 entries** (was 4 / 4 / 797). **Architectural pivot:** Whole-Salem global pool concept is GONE — V1 has zero global pools, collections are direct per-tour walk-derived snapshots, Dr. K's Tour is the default no-tour-active view. Per-tour `feedback_collection_pool_intersect_proximity.md` still holds; the `admin-collection.js:559-561` fallback (`historical_narration IS NOT NULL`) now serves the pool role. **Not done this session:** publish chain unrun, no Android build/install — on-device state still reflects S278's 4-tour + 447-pool layout. Full detail in `docs/session-logs/session-279-2026-05-18.md`.

### Prior — S278 (2026-05-17)

Katrina's Collection **v2 ghost batch SHIPPED end-to-end** on Pixel 8 + Lenovo. New hard-rule `feedback_si_is_reference_only.md` — SI is clue-lookup only, never content source; LMA `salem_pois` is the source of truth. `mine-si-clues-for-ghosts.py` injects 74/107 SI/LMA-clued profession overrides into `generate-ghost-portraits.py`; 5-pass prompt overhaul to cartoon/witchy/spooky/1690s-period. B-image subtlety pass per operator. Full 107-pair batch 858 s, regen-B 298 s. Pack PNG→webp (95% reduction, 9 MB final). `CollectionSheet.kt:buildRowsForCollection()` filtered to ghost-eligible only — List + Badges both show 107. No schema changes, Room v23 / identity_hash `4db15f...` unchanged. Full detail in `docs/session-logs/session-278-2026-05-17.md`.

### Prior — S276 (2026-05-17)

Find dialog TYPE_SYNONYMS Wave 2 (`FuzzySearchEngine.kt` ~60 → ~170 alias keys for tourist queries), Filter-and-Map "doesn't filter" diagnostic instrumentation (revealed two parallel marker systems: `poiMarkers` vs `narrationMarkers / narrationMarkerCache`), search-mode containment + tour pause/resume + clickable "▶ Resume — <name>" status-line chip (`SalemMainActivity.kt` / `SalemMainActivityFind.kt` / `SalemMainActivityTour.kt`), Form TX deadline deferred 2026-05-20 → 2026-07-01 (counsel-side). **Carry-forward:** verify `Priority.FIND_FILTER` (3) vs `TOUR-Paused` (4) status-line ordering on field walk. Full detail in `docs/session-logs/session-276-2026-05-17.md`.
