...
**Last updated:** 2026-05-21 — **Session 285.** Operator pivot off the carry-forward docket into historical-maps reconnaissance + POC + plan. **Inventory:** ~5 GB at `~/Development/SalemIntelligence/data/historical_maps/` (300 Sanborn sheets across 1890/1906/1950/1957 — zero noise; 198 USGS quads; 11 LoC items). **Web sweep** (general-purpose subagent) found 16 strong PD candidates beyond on-disk — BPL Leventhal dominates. **Downloaded 11 items / 477 MB** to `/mnt/sdb-images/HistoricalMapsSample/` via IIIF `full/full/0/default.jpg` (BPL's direct-download button is JS/cookie-gated; IIIF returns same source pixels as JPEG). Built thumbnail gallery at `http://127.0.0.1:4444/` (python3 http.server PID 2296249, still running). **1851 McIntyre POC end-to-end:** v1 5-GCP TPS warp via `gdal_translate` + `gdalwarp -tps -t_srs EPSG:3857 -dstalpha` + `gdal2tiles.py --xyz` produced 9,456 tiles z14-z19 in ~3 min — operator caught *"Washington Street in a whole wrong place"* (my v1 Washington Sq center was off ~328px x / +211px y in 6502px image, ~150m on ground). v2 replaced single-center GCP with **4 precise Salem Common corner GCPs** + kept 4 outer-bbox anchors (8 GCPs total). Leaflet preview at `/mnt/sdb-images/HistoricalMapsSample/preview_1851.html` with v1↔v2 toggle + opacity slider + OSM/CARTO base toggle + GCP markers. Operator: *"looks good."* **Plan mode** ran 3 parallel Explore subagents (UI infra / tile + overlay engine / asset pack + bake) confirming WickedMap is custom SurfaceView (NOT osmdroid for rendering), `TileArchive.kt` uses osmdroid SqliteArchive format with custom `osmdroidKey()` 64-bit encoding (NOT standard MBTiles — but `tools/tile-bake/merge-into-bundle.js` already handles the conversion), and `salem_tiles.sqlite` supports multi-provider rows so historical years can ride inside the existing asset pack without a second pack. Two operator decisions via AskUserQuestion: **entry point = new FAB above fabMain** + **V1 map count = 4 maps (1851/1874/1906/1911)**. Final plan at `~/.claude/plans/groovy-launching-popcorn.md` — 5 phases / ~6 sessions: Phase 1 tile pipeline + 1851 hardcoded on Lenovo; Phase 2 FAB + bottom-sheet picker + opacity slider + MenuPrefs persistence; Phase 3 bake remaining 3 maps (Sanborn 1906 wildcard — 80-sheet mosaic); Phase 4 publish-chain + verify-bundled-assets wiring; Phase 5 field smoke. Architecturally: single `salem_tiles.sqlite` with `provider='Historical-YYYY'` rows, new `HistoricalTileOverlay : MapOverlay` slots between basemap and animation overlays (respects `feedback_basemap_priority_over_animation.md`), paper-transparency preprocessing per map. Net code-side change: **zero in the Android repo** — all work in `/mnt/sdb-images/HistoricalMapsSample/`. Both field devices still carry S282 build.

### Prior — S284 (2026-05-19, closed 2026-05-20)

CI red-since-S275 cleared (10 push failures back-to-back). Root cause at `sheet_collection.xml:162-163` — `tabIndicator` constrained to `@id/tabList` / `@id/tabBadges` which lived inside the `collectionTabs` LinearLayout (grandchildren of outer ConstraintLayout, not siblings). Same invalid constraint at `CollectionSheet.kt:472` runtime meant the sliding amber underline had been silently broken on device since S275. Flattened tab bar via XML refactor (dropped wrapper, 50/50 vertical Guideline) — fixes lint AND restores runtime indicator. Commit `c8e7657`; CI run `26136388027` green. **Non-blocking:** Node 20 → Node 24 GitHub Actions deprecation 2026-06-02 (~12 days now). S284 XML fix not on field devices (cosmetic-only).

### Prior — S283 (2026-05-19)

Chatbot V1 scoped end-to-end (on-device, rule-based, POI facts only, distance-sorted). 7 design decisions locked via AskUserQuestion. Plan at `docs/plans/Chatbot_V1.md` parked for operator pondering — Phase 0 recall spike is the recommended first action.

### Older prior-session blocks pruned 2026-05-21 (S285) for the 200-line cap; see `SESSION-LOG.md` for S276–S285 + `docs/session-logs/` for full detail.
...
### S286 opener (parked at S285 close)

**NEW — Historical-map FAB-picker plan ready to start (Phase 1).** Plan at `~/.claude/plans/groovy-launching-popcorn.md`. 5 phases / ~6 sessions / 4 maps for V1 (1851 McIntyre + 1874 Hopkins + 1906 Sanborn + 1911 Walker). Phase 1 = tile pipeline + 1851 hardcoded overlay end-to-end on Lenovo (no UI yet, ~1 session). Expands V1 KEEP-list item #2 (was "1906 time-slider, ~1 week") into a 4-map FAB picker. **Per the plan's own risk note, this sits BEHIND the older Bell + portrait + chatbot carry-forwards in priority** — pull only after those land or are explicitly deferred. POC artifacts persist at `/mnt/sdb-images/HistoricalMapsSample/poc_1851/` (~290 MB v1+v2 tile pyramids + MBTiles); regeneratable from `georef.sh` / `georef_v2.sh` if storage matters. Gallery still browseable at `http://127.0.0.1:4444/` (PID 2296249).
...
2. **Historical-map FAB picker, 4 maps for V1** (1851 McIntyre / 1874 Hopkins / 1906 Sanborn / 1911 Walker) — plan at `~/.claude/plans/groovy-launching-popcorn.md` (S285). Replaces the original "1906 time-slider, ~1 week" framing.
...
**Sessions completed:** 284. **Internal ship target: 2026-08-01.**
