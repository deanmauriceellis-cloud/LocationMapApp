...
### S303 opener (parked at S302 close)

_**Tech-debt cleanup — plan-of-record authored, awaiting go/no-go (NEW S302).** Plan at `docs/plans/tech-debt-cleanup-2026-05.md`; analysis handoff + 7 open questions at `NEXTSTEPS_20260527.md`. Operator-locked posture: aggressive (incl. SalemMainActivity decomposition) with a guaranteed fall-back. 7 phases: **P0** publish-chain orchestrator + shared `.env` + txn-wrap + footgun tests (**only pre-ship-critical item**) → **P1** JobCoordinator + MarkerController → **P2** retire dead hero/icon (column-drop deferred post-V1) → **P3** isolate parked network features into `parked/` → **P4** migrate active state into the already-existing VMs (Events→Detour→Directions→Populate→Geofences→Find→Narration→Tour, smallest-first) → **P5** `!!`/catch hardening → **P6** web (optional). Guardrails: behavior-preserving commits, `-PskipPublishChain`+restore-DB, compile+unit per commit / on-device Pixel-8 smoke per phase, tag `cleanup-phaseN-done` each phase (reset to last-good tag = fall-back). Each phase shippable + interruptible for OMEN-025 at any tag. **Next: operator answers the 7 NEXTSTEPS questions + picks start point (P0 safe vs P1 infra).**_

_**Full Explore demo mode + expanded-map (Beverly) marketing demo (NEW S302).** (1) The debug-only SuperAdmin toggle is re-surfaced as user-facing **"Full Explore"** with a persistent `● FULL EXPLORE` teal toolbar pill (visible only while active; R8-stripped in retail — V1 offline posture intact). Verified Pixel 8. (2) Operator "two-way state": **Salem Tour = V1, isolated; expanded map = marketing demo, distinct.** Shipped `Salem-Custom` tiles are BLANK north of Salem (S234 widened render bbox before S235 widened vector data; S236 content bake rolled back after OOM-crashing the 4GB Lenovo). Built a **throwaway 760 MB debug demo AAB** (Beverly roads/buildings/parcels from on-disk `tools/tile-bake/salem-custom.mbtiles.s236bak`) installed to BOTH devices; V1 asset never committed. **HARD LESSON: the V1 `salem_tiles.sqlite` is GITIGNORED — `git checkout` can't restore it; back it up before any asset swap.** Exact V1 recovered from the device runtime copy + backed up at `tools/tile-bake/dist/salem_tiles.sqlite.V1-EXACT-20260527`. Deferred: Beverly POIs (Tier 2 MassGIS — demo has basemap but no Beverly markers); Lenovo OOM risk on the fat bundle (Pixel 8 = safe demo device)._
