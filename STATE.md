...
**Last updated:** 2026-04-23 — Session 161 closed. Car-tour triage + field-bug fixes + repo cleanup, no Phase-9Y forward progress. (1) Debug-log review of car tour (645 KB): GPS "spotty" traced to speed-based interval thrasher (19 rebuilds in 15 min); accuracy itself fine. Witchy vs Satellite tile alignment confirmed correct at the bake (blend compare at 4 landmarks, z=16-18). (2) V1-offline gate leaks fixed at caller for `searchPoisAt()`, `startSilentFill()`, and `AppBarMenuManager.syncCheckStates` — no more "POI failed" toasts, "Fill failed" banners, or findItem warnings during V1 runs. (3) GPS interval thrasher fixed: new `LocationManager.updateRequestParams()` adjusts live FLP subscription without teardown; interval picker reduced 3-state → 2-state (dropped >20 mph → 10s branch) and gained 10s dwell debounce. (4) Repo cleanup: ~20 stale root + `docs/` files archived (CHANGELOG, SOCIAL-PLAN, WEB-APP-PLAN, MASTER_PLAN_ARCHIVE, PLAN-ARCHIVE, REFACTORING-REPORT, tigerline-*, completed SI briefs, etc.); 6 shell scripts moved to `scripts/`; parked-master-plan placeholder deleted and pre-park snapshot archived; CLAUDE.md updated to drop stale references. Deployed to Lenovo twice; needs next-drive verification.

---

## TOP PRIORITY — Next Session (S162)
...
0. **FIRST: verify S161 GPS + V1-offline fixes on next drive.** Pull Lenovo debug log after a representative car + walk tour. Expected signals: (a) zero "POI failed" / "Fill failed" toasts; (b) zero `AppBarMenuManager: findItem(0x7f09019a) null` warnings; (c) `updateRequestParams — interval Xms → Yms (no teardown)` on the rare genuine interval transition, **NOT** `Flow cancelled — removing location updates after N updates received`; (d) fix cadence ~1-2.5s while moving (no 20-30 s gaps). If any regress, root-cause before stacking more work.
