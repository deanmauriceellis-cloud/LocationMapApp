...
**Last updated:** 2026-04-24 — Session 168 closed. Install-default sweep + one-narration-per-visit + zoom 14-20 lock + toolbar/banner rework. Install defaults flipped off: POIs Hist. Landmark, Record GPS Travel Coordinates, Use Real GPS Outside Salem, GPS-track FAB polyline, Oracle (Audio Control); detail level flipped STANDARD → DEEP. `SalemMainActivityNarration.kt` collapsed two speak calls per POI visit into one `when` (stripped → historical/body → BRIEF fallback). First-POI silence bug root-caused to `NarrationManager.enqueue()` dropping segments during the TTS bind race — fixed by deferring on `!ttsReady` + flushing on `onInit()`. Map floor raised to z14 (new `MAP_MIN_ZOOM` const) and ceiling lowered to z20; cinematic rewritten to z14 → z16 → z18 since tiles don't exist below z16 + overzoom. Toolbar: First/Prev/Next/Jump deleted, `narrationHeroBanner` block + `NarrationHero.kt` file deleted, Home moved to the left, single speaker icon on the right now handles tap=cancel-current-or-replay-last and long-press=audio content popup. Splash-skip bug diagnosed from logcat (task-affinity collision brings SalemMainActivity to front instead of SplashActivity on warm launcher taps) — fix proposal recorded, operator did not green-light it.

---

## TOP PRIORITY — Next Session (S169)

1. **Splash-skip `singleInstance` fix (deferred from S168).** Operator reported "quick witchykitty picture then jumped to GPS map" on launcher tap. Root cause in the S168 live log: SplashActivity finishes itself after launching SalemMainActivity, so the task root becomes SalemMainActivity. Next launcher tap brings that task to front on affinity match, bypassing the LAUNCHER intent filter. Proposed one-liner in `app-salem/src/main/AndroidManifest.xml`: add `android:launchMode="singleInstance"` to the SplashActivity entry (or `taskAffinity=""` + `singleTask` — equivalent effect). Verified force-stop launch plays full Katrina intro + TTS welcome. Carry-forward: operator wants to decide before applying.

2. **Smoke-test S168 in the field.** The 8 install-default flips + narration overhaul + zoom lock all landed on a clean uninstall+install on HNY0CY0W at session close. Worth a short drive/walk test to confirm: one clip per POI (no double), no dead air on first POI, slider behaves at 14-20, Home/Speaker positions feel right, speaker cancel-then-replay works.
