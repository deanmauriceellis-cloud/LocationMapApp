...
**Last updated:** 2026-05-16 — **Session 271.** Tech-debt sweep across 6 phases (operator: *"explore all the code and our strategy — surface technical debt"* → *"let's fix them all"*). Five parallel survey agents synthesized into a P0-P3 punch list; **in-band verification before planning** caught 3 false alarms from the DB-survey agent (schema drift already filtered at `publish-salem-pois.js:206`; fallbackToDestructiveMigration comments already load-bearing; MIGRATION_2_3 verification admitted-dead by install pattern). Operator approved Room v21 now via plan-mode flow. **Phase 1 doc cleanup:** CLAUDE.md 303→246 lines (deleted S188 pinned block, 82 sessions stale); STATE.md 224→199 (under 200 cap), 6 carry-forwards 70+ sessions stale moved to `docs/archive/STATE_carryforwards_removed_2026-05-16.md`; 3 plan-file status headers refreshed; SalemIntelligence memory files got PAUSED S214 banners. **Phase 2:** cache-proxy `/admin/burst-photos/pull` SSE subprocess leak fixed (`res.on('close')` SIGTERM + 600s/300s spawn timeouts). **Phase 3:** Android `SalemMainActivityFind.kt:1782` defense-in-depth `SuperAdminMode.networkBlocked` call-site gate. **Phase 4:** Room v20→v21 with 7 indices on SalemPoi (is_narrated + is_tour_poi + is_civic_poi + is_historical_property + category + district + subcategory), identity_hash `19fcd8e4347d88e9da1a50aef2734bc9`; full publish chain ran (2039 POIs / 4 tours / 73 legs / 797 passport rows); bundletool install on Lenovo + Pixel 8 confirms `schema_v=21 tables=13` no destructive migration. **Phase 5 web admin:** Sonner 1.7.4 + toast wrapper, 18+ window.alert/1 bare alert across 7 files replaced with toastError/toastSuccess; shared `web/src/lib/fetchJson.ts` (3 local impls collapsed); api.ts hardcoded `10.0.0.4` fallback → localhost + warn; ReconTriageTab AbortController; tsconfig `noUnusedLocals + noUnusedParameters` + 15 fixes across 8 files; `npx tsc --noEmit` clean. **Phase 6:** `docs/plans/post-v1-architecture-decomposition.md` written (Wave 5 obligation; 3-4 week post-V1 refactor scope). 40 files changed, 6 created. 77 days to 2026-08-01 ship; 4 days to Form TX deadline 2026-05-20. Full detail in `docs/session-logs/session-271-2026-05-16.md`.

### Prior — S270 (2026-05-16)

Recon photo triage tool Layer 1 shipped end-to-end (backend extension + new `ReconTriageTab.tsx` + sidebar wiring + verified round-trip) on top of existing S229/S231 burst-photos infra per `feedback_leverage_existing_assets.md`. 5 real sessions ready for operator triage (5/477/239/1/1741 surviving photos). Detail in `SESSION-LOG.md` + `docs/session-logs/session-270-2026-05-16.md`.
...
**Sessions completed:** 271. **Internal ship target: 2026-08-01.**
