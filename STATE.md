...
## Last Updated: 2026-04-08 Session 104 (Phase 9P.B Step 9P.9 — Admin map view (Leaflet) landed)

## TOP PRIORITY — Next Session
**Phase 9P.B Step 9P.10 — POI edit dialog (tabbed modal).** Phase 9P.A is complete; 9P.6 (TS taxonomy) landed S101; 9P.7 (AdminLayout shell) landed S102; 9P.8 (POI tree) landed S103; 9P.9 (admin map view) landed S104. The map renders all ~1,720 active POIs as a clustered Leaflet layer color-coded by kind (tour=red, business=blue, narration=green), supports tree-driven pan/zoom + gold-ringed selection highlight, marker click emits selection back to AdminLayout, marker drag→confirm modal→POSTs to `/admin/salem/pois/:kind/:id/move` with state patched in the shared `byKind` snapshot on success and reverted on cancel/failure. Next session: build `web/src/admin/PoiEditDialog.tsx` — a modal that opens when `selectedPoi` is set (via tree click or marker click), has tabbed sections per the master plan (Identity, Location/Geofence, Content, Provenance, Linked Historical Content, Categorization), reads the full row from PoiTree's `selectedPoi.poi` (no extra fetch needed, the tree already loaded everything via `include_deleted=true&limit=2000`), and PUTs to `/admin/salem/pois/:kind/:id` with the whitelisted field set per `cache-proxy/lib/admin-pois.js` (TOUR_FIELDS / BUSINESS_FIELDS / NARRATION_FIELDS — these are the source of truth, copy them into a TS const to validate the form). After 9P.10: **9P.10a (Linked Historical Content read-only tab)**, **9P.10b (Salem Oracle "Generate with AI" integration — see `~/Development/Salem/docs/oracle-api.md`)**, 9P.11 (Highlight Duplicates wiring), then Phase 9P.C publish loop.
