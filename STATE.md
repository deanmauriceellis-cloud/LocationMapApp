...
**Last updated:** 2026-05-17 — **Session 278.** Katrina's Collection **v2 ghost batch SHIPPED end-to-end** on both Pixel 8 and Lenovo. Operator: *"We are going back to the badges … SI is back up — let's figure out a way to get some good ghosts for our badges."* New hard-rule memory `feedback_si_is_reference_only.md` (operator: *"SI is just a toy, not a primary source of anything due to copyright/plagiarism issues"*) — SI is clue-lookup only regardless of pause status; LMA `salem_pois` is the source of truth. Built `mine-si-clues-for-ghosts.py` (LMA-prose two-pass NAME→DESC matcher with named-figure lexicon — Hawthorne / Bowditch / Bishop / Rebecca Nurse / Sarah Good / Proctor / Conant / McIntire / Endecott / Hathorne / Corwin / Parris — plus SI building.original_use + SI relations free-text fallbacks). **74 / 107 SI/LMA-clued profession overrides** keyed by poi_id; 33 fall to S275's random pool. Wired into `generate-ghost-portraits.py --from-pg` as a non-invasive `OVERRIDES_FILE` override (hair/expression/oddity/frame/prop still deterministic per-poi RNG roll). **Five prompt-iteration passes** over operator critique to land cartoon/witchy/spooky/period: P1 baseline + SI overrides → P2 cartoon style + new `witchy_props` pool (30 entries) + Coraline/Hocus Pocus style spine → P3 ghost-first reframing (wraith/spirit/translucent leads with max weight) → P4 period 1690s puritan colonial wardrobe + haunted Salem backgrounds (gambrel saltbox houses, crooked gravestones, gnarled oaks, fog over cobblestones) → P5 CFG 2.5→4.5 + wraith/specter/revenant tags + corpse-grey-skin + hollow-black-eye-sockets + hardened NEG against "pretty alive woman" model bias. **Full 107-pair batch in 858 s (14:18, 8.0 s/ghost)**, 214 PNGs at 768×768. New `build-ghost-review-gallery.py` writes single-page HTML contact sheet (A+B side-by-side, persona details, keep/regen/flag radios, localStorage marks, clipboard-export). **B-image subtlety pass** per operator *"B images too extreme"*: replaced extreme `expressions_alt` entries with micro-expressions ("the faintest knowing smile" / "a slow subtle wink") + lowered weight 1.5→1.15 + dropped "exaggerated"/"stretched mouth" + face-inpaint denoise 0.75→0.45 + `--regen-b` re-rolls from current pool. Regen-B across all 107 in 298 s (4:58, 2.8 s/B); A bit-identical, only B regenerated. **Ship pipeline executed end-to-end:** pack PNG→webp (179 MB → 8.5 MB, 95% reduction), replace `app-salem/src/main/assets/ghosts/*.webp` (107 pairs), repopulate `salem_pois.ghost_asset_a/b/ghost_frame` via parametrized `populate-ghost-personas.py --manifest <v2>` (new `prop` roll shifted RNG; frame slugs reassigned), run 5-script publish chain (`salem-pois → tours → tour-legs → poi-collection → align-asset-schema`, Room v23 / identity_hash `4db15f...` unchanged), build debug APK + AAB, bundletool `build-apks --connected-device` + `install-apks` on Pixel 8 (`41231FDJH0018J`) + Lenovo (`HNY0CY0W`) with `adb uninstall` first per WAL-safety rule. Both verified installed. **Bonus fix from Lenovo screenshot:** `CollectionSheet.kt:buildRowsForCollection()` was showing all 447 collection POIs in the List with empty 64dp portrait boxes for the 340 non-HIST_BLDG entries; added the same `!ghostAssetA.isNullOrBlank()` filter the Badges grid uses. List + Badges now both show 107; count `X / 447 → Y / 107` (aligns with `feedback_ghosts_not_stops.md`). Rebuild + bundletool re-install on both. Operator *"works well enough."* **No schema changes; Room v23 / identity_hash unchanged; no publish-chain regression.** Full detail in `docs/session-logs/session-278-2026-05-17.md`.

### Prior — S277 (2026-05-17)

Exploratory-only session. Operator opened with the lively-map golden rule (*"this is a casino of tourists - our map should be animated"*) and iterated through three reframings before settling on "ants" — 8-16px tiny CGI dots as pure background motion. Explore subagent audited the WickedMap engine and confirmed it's already production-wired with **4 live ambient animations at 30fps heartbeat** — adding ants is a 5th `MapOverlay` sibling, not new infrastructure. Two small gaps documented (no zoom gate; no sidewalk polygon kind). **Parked plan** at `docs/plans/ambient-ant-sprites-parked.md`. No code/DB/schema/build/install changes. Full detail in `docs/session-logs/session-277-2026-05-17.md`.

### Prior — S276 (2026-05-17)

Three connected pieces + one project update. **(1) Find dialog TYPE_SYNONYMS Wave 2** — expanded `FuzzySearchEngine.kt` from ~60 to ~170 alias keys spanning operator-named cases (pot / smoke shop / flowers / bathroom / parking) + the stated 90% of tourist queries (humorous / parking / transport / bathrooms) + adjacent staples (pharmacy / drugstore / emergency / urgent / gas / fuel / groceries / supermarket / souvenirs / ATM / cannabis / tobacco / vape / florist). Pure Kotlin source change — no DB / schema / publish-chain impact. **(2) Filter-and-Map "doesn't filter" bug** field-reported on Lenovo: per operator *"put verbose debugging on all of this when we are in debug mode, why guess?"* — instrumented entire flow with `DebugLogger.d("FilterMap", ...)` lines (clear/replace/observe/enter/exit/marker-tap/`loadCachedPoisForVisibleArea` with caller stack frame). Trace from "pot" query (33 results) exposed two parallel marker systems: `poiMarkers` (layered, what `clearAllPoiMarkers()` walks) **was empty when filter-map entered**, while `narrationMarkers` / `narrationMarkerCache` / `narrationMarkersOnOverlay` (S118/S234, ~580 markers) — the cache-backed system holding the actually-visible POIs — was never touched. Filter added 33 markers on top of 631 existing. **(3) Search-mode containment + tour pause/resume + Resume chip** — operator design model: search is its own mode; active tour pauses on enter, auto-resumes on exit-without-pick, stays paused with a persistent clickable "▶ Resume — <tour name>" status-line chip on pick. Implementation across four files: `SalemMainActivity.kt` (new `internal var tourPausedBySearch` + `detachAllNarrationMarkers()` helper — removes narration markers from overlays, preserves `narrationMarkerCache` so re-attach via `loadNarrationPointMarkers()` is zero-allocation cache hits); `SalemMainActivityFind.kt` (enter: pause tour if Active + `clearTourOverlays()` + `detachAllNarrationMarkers()` before the existing clearAllPoiMarkers and filter-map add; exit: `loadCachedPoisForVisibleArea()` + `loadNarrationPointMarkers()` + auto-resume `if tourPausedBySearch && state is Paused`; marker click = pick = sets `tourPausedBySearch=false` then exits filter then opens detail, so tour stays Paused); `SalemMainActivityTour.kt` (`observeTourState` Paused branch is now a clickable `"▶ Resume — <name>"` entry with onClick → `tourViewModel.resumeTour()`). Reused existing `Priority.TOUR` (level 4); no new status priority needed. Build clean (`:app-salem:compileDebugKotlin` + `:app-salem:bundleDebug` in 8s). Installed on Lenovo via bundletool (uninstall + bundletool flow per WAL-safety rule). **Field walkthrough deferred** to operator. **Carry-forward:** verify `Priority.FIND_FILTER` (level 3) vs `TOUR-Paused` (level 4) status-line ordering during active filter-map mode — TOUR-Paused currently outranks FIND_FILTER which may read confusingly; operator field walk will tell us whether to clear TOUR during filter-map or bump FIND_FILTER above it. **(4) Form TX deadline deferred** 2026-05-20 → 2026-07-01 (operator direction, counsel-side, no code impact). New memory `project_form_tx_deadline.md` saved + indexed. 5 build/install cycles on Lenovo through the session. 72 days to 2026-08-01 ship; 45 days to new Form TX target 2026-07-01. Full detail in `docs/session-logs/session-276-2026-05-17.md`.

### Prior — S275 (2026-05-17)

Katrina's Collection **Phase 2 + Phase 3 SHIPPED end-to-end**: 107 paired ghost portraits (A normal + B fourth-wall-break smirk, 2% runtime swap), 8 frame overlay PNGs, Room **v22 → v23** (identity hash `4db15f763c9dbd5a529d24b128cecada`), augmented CollectionSheet with new "Badges" tab + witch-hat row stamps replaced by ghost-portrait tiles. New `GhostResolver.kt` runtime; new memory `project_katrinas_collection_ghost_asset_pairing.md`. ~4.6 MB total app payload. **Phase 4 (operator field smoke) carry-forward:** walk a Salem POI to verify greyscale → color transition + frame appearance after first narration trigger, confirm 2% B-swap visibly fires in the wild. Full detail in `docs/session-logs/session-275-2026-05-16.md`.

### Prior — S274 (2026-05-16)

"POI Passport" rebranded → **"Katrina's Collection"**. Phase 1 mechanical rename across ~50 files: PG tables, cache-proxy lib + scripts, Room content DB **v21 → v22** (`PoiPassport` → `CollectionEntry`), Room user DB **v3 → v4** with `MIGRATION_3_4` preserving paid-user history. 107 HIST_BLDG POIs confirmed ghost-eligible. Full detail in `docs/session-logs/session-274-2026-05-16.md`.
...
**Sessions completed:** 278. **Internal ship target: 2026-08-01.**
