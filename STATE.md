...
**Last updated:** 2026-05-17 — **Session 277.** Exploratory-only session. Operator opened with *"I want the map to be animated … this is a casino of tourists - our map should be animated - this is a golden rule of applications with mapping"* and iterated through three reframings (1000 sprites → 64×64 individuals with tour cohorts → "ants" — 8-16px tiny CGI dots, pure background motion). Explore subagent audited the WickedMap engine and confirmed it's already production-wired with **4 live ambient animations at 30fps heartbeat**: `AnimatedWaterOverlay` (whitecaps, 2.5s/8s duty), `FireflyOverlay` (cemetery orbs, continuous pulse), `RollingGrassOverlay` (park grass sway, 1.5s/14s duty), `SpriteOverlay` (geofence-triggered haunt sprites). Heartbeat lives at `wickedmap/WickedAnimationOverlay.kt:55-108` self-invalidating via `postInvalidateDelayed(33ms)`; orchestrator wired into `SalemMainActivity.kt:1442-1477` at overlay position 0 (drawn under markers — inherits `feedback_basemap_priority_over_animation.md` compliance). `MapOverlay` is a 2-method `tick + draw` interface; an ants overlay is a 5th sibling, not new infrastructure. Two small gaps: no zoom gate exists today (whitecaps render at every zoom — ants would need `minZoom=18`); no sidewalk polygon kind in `PolygonLibrary` (only water/cemetery/park — ants need polygon-bounded random walks OR baked sidewalk-edge JSON OR trailing the 4 baked tour polylines). **Parked plan written** at `docs/plans/ambient-ant-sprites-parked.md` with concept + engine-reuse table + 2 gaps with options ranked by cost + 3-phase scope + 4 deferred decisions + explicit non-goals + 5 risks + verified anchors. Operator: "we will revisit it shortly." **No code, no DB, no schema, no publish-chain, no build/install changes.** All S276 carry-forwards untouched (FIND_FILTER vs TOUR-Paused status-line ordering verification, Katrina's Collection Phase 4 field smoke, Collection S5 field walk, Recon Layer 2, Pixel 8 portrait Phase A survey). Full detail in `docs/session-logs/session-277-2026-05-17.md`.
...
**Sessions completed:** 277. **Internal ship target: 2026-08-01.**
