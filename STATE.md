...
### S307 opener (parked at S306 close) — "we continue next session"

_**ACTIVE WORKSTREAM = `PerfStabilityReview_20260529.md` perf/stability fixes.** Tier 0 (cold-start ANR, S305) + **Tier 1 (OOM floor) + Tier 2 (jank) DONE + Pixel-8-validated, coverage-safe** (S306, commits `dfbc99b`/`398102a`): z19 tile-fetch clamp while tilted (coverage-NEUTRAL — no z20 tiles exist), ghost/hero/dock downsampling (`SampledAssetDecoder`), `onTrimMemory` real eviction, ambient `WickedAnimationOverlay` suppressed under tilt (0 heartbeats in 14k tilted lines), dispatchDraw alloc hoist, narration-pulse guard, FAB debounce. **HARD RULE for all tilt work: never shrink the wedge-fill extension (`EXTRA_TOP_FRACTION=5.0`) — no blank wedges (memory `feedback_tilt_coverage_is_load_bearing`).** **NEXT in priority order:** (1) **Tune the marker scale down** — the new universal `MarkerIconHelper.markerScale()` (`smallestScreenWidthDp/411`, Lenovo 1.73× / Pixel 1.0×, `1a59b1c`) overshot; operator finds markers a touch too large on BOTH → lower the base-dp or scale. (2) **Run the Lenovo 48° OOM gating test** (never ran S306 — pre-empted) → decides whether **Tier 3** (angle-proportional `EXTRA_TOP_FRACTION`, runtime-derived from container W/H; + optional z18 deeper clamp) is required vs optional. (3) **Deep-tilt-under-walk-sim ANR** — Tier 2 killed the 30fps ambient churn but the continuous heading-up/GPS per-fix redraws under tilt still ANR'd on both devices; remainder = #3/#17/#18/#35 (per-fix marker/ring/invalidate churn) + #5 (heading-up rotation re-record under tilt)._
...
**Sessions completed:** 306. **Internal ship target: 2026-08-01.**
