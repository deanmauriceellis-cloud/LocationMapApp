...
**Last updated:** 2026-04-28 — Session 195 closed. Layers checkboxes (POIs Hist. Landmark / POIs Civic) repurposed from tour-mode-only widening into true global overrides gating eligibility in BOTH explore and tour modes. Phase A backfilled `is_historical_property=true` on 358 active HISTORICAL_BUILDINGS with populated `historical_narration`; Phase B (one commit): Room v13 → v14 (identity hash `5a42a013703bb605f5ad4f065309ee8a`), DAO `findNarrated*` widened to admit `is_historical_property OR is_civic_poi`, NarrationGeofenceManager unified eligibility gate + commemorative detector + civic-hist-eligibility helper, getNarrationForPass prefers historical>short for those classes. Per-POI Quality flags panel landed in PoiEditDialog (with Suppress button). 123 POIs flipped to `is_civic_poi=true` (WORSHIP/EDUCATION/PARKS_REC); 142→265 civic POIs. Tier filter (Show-All-FAB-OFF baseline) widened to admit hist-property + civic so the toggle actually surfaces them. Lint dedup fix: checkDuplicates skips members at exact-coord clusters of 3+, label renamed "Co-located POIs". Heading-up rotation re-enabled GPS-only (sensor branch stripped); toggle-ON snaps north-up first when no GPS bearing yet. Operator drove 45 min on the new build — 1689 GPS points snapped against Tiger MA in PostGIS: median 2.6m / mean 3.3m / p90 6.6m / p99 17.4m offset, 87.8% within 6m, with bearing-from-GPS-to-road clustering 51% in SE/E quadrants → real ~3-4m systematic NW bias in the Lenovo's GPS chip (multipath off downtown brick), basemap is fine. 7 commits, final `22b5e08`. Full detail in `docs/session-logs/session-195-2026-04-28.md`.

**Previous (S193/S194):** S194 — historical-narration QC tooling. 6 corrupted narrations NULLed, shared validator at `cache-proxy/lib/historical-narration-validator.js`, `historical_narration_needs_refinement` lint check (89 POIs flagged), POI tree color-dot indicators, `OracleVariantPicker.tsx` (5×3 variant generator). Carry-forward: commemorative-tribute generator path. S193 — ~13h marathon: TTS chunk-aware onDone fix, advance-headline-before-delay (5 paths), walk-sim FAB await-tour-Active, kill all synthetic route fallbacks (`feedback_authored_tours_are_definitive`), Room v12→v13 historical-tour wiring, overnight gemma3 bake wrote 294 historical_narrations (70 → 364 coverage), HIST_BLDG runtime short-only fallback. Final commits: `f3ab266` / `9bf8e8d`. See live logs.
...
## TOP PRIORITY — Next Session (S196)
...
0a. **(S194/S195 carry-forward) Commemorative-tribute generator path.** Build `buildCommemorativeTributePrompt()` in `cache-proxy/scripts/generate-historical-narrations.js` that frames the LLM around the pre-1860 SUBJECT (not the post-1860 object); relax the year guard for tribute mode; reword STRICT_RULES rule #5 from "skip statues" to "narrate the commemorated subject, EMPTY if subject is post-1860 or unknown". Memory + carry-forward comment already in place at line 766. Then widen `OracleVariantPicker.isHistoricEligible()` to accept commemorative POIs and add a 6th historic variant.

0b. **(S192 carry-forward) Backfill `year_established` on the 479 buildings the generator skipped because effective_year was null or ≥ 1860.** Pull from SalemIntelligence `mode_c_historic_building_details.built_year` + MHC inventory. Then re-run generator.

0c. **(S195 NEW) Driving mode considered explicitly.** S195 drive-test surfaced that walking-tuned geofences (40m + serial TTS) are brittle at 10 m/s — the engine fires 30+ ENTRYs in a 30s window down Derby Street while TTS only drains 2-3. Either V2 adds a "driving mode" with smaller radii + cluster-prioritized narration, or V1 explicitly tells the user "this app is for walking." No code change yet — operator decision.

0d. **(S195 NEW) The 7 narrated POIs that drove >20m off Tiger** are likely worth a coord-validation pass via the Geocodes modal. They're in the GeoJSON at `/tmp/s195-snap-points.geojson` if we want to look. (May not survive a /tmp purge — re-run the snap-to-Tiger query if needed.)

1b. **PRE-AAB — clean the 9 commercial-hero leaks** (S190 surfaced, still owed). Either NULL out `image_asset` on the 9 POIs flagged by `verifyBundledAssets`, or move them to `merchant_tier=1`. The check is bypassed `-x verifyBundledAssets` for dev-builds; it's a hard gate for any signed AAB upload.
