...
**Last updated:** 2026-05-26 — **Session 301.** **Graphics redo — map-marker half wired** (art-bible §4.1): the 107 HISTORICAL_BUILDINGS now render their Katrina's Collection **ghost badge** as the map marker (`MarkerIconHelper.ghostBadge()` circular crop + ink ring, resolved in `narrationIconForZoom` on `ghost_asset_a`; verified Pixel 8), and the 17 WORSHIP POIs get a **woodcut church-glyph circle marker** (generated via `render_worship_glyph.py`, replaces the witch sigil at `poi-circle-icons/worship/place_of_worship.webp`; no code change — existing `dot()` path). Commits `5757954` + `2b56090`. Also root-caused an operator "all POIs show in tour mode / same icon — didn't have this 48h ago" report: **not a code or DB regression** — the S299 fresh reinstall wiped SharedPreferences and the S206 layer-default "mirror" defaults the unwritten tour-mode Historical-Buildings toggle to the explore value (ON); "same icon" was the Show-All-OFF view leaving only HIST_BLDG+LANDMARKS (which share one icon by S216 design). Remedy = re-uncheck the tour-mode layer (persists); operator "good for this." Still pre-redo: the OTHER `poi-circle-icons/` categories (§4 "POI circle marker"), splash 12→4, §7 dead-system retirement. _(S300, 2026-05-25, was a no-op opener.)_
...
### S302 opener (parked at S301 close)

_**Two active threads.** (1) **Graphics redo — heroes + map markers now BOTH wired on-device.** Heroes (124 bespoke h3 + 40 category scenes) shipped S299; map markers (107 HIST_BLDG ghost badges + 17 WORSHIP church glyph) shipped S301. Generators in `tools/art-bible-samples/` (`render_hero_full.py`, `render_hero_pilot.py`, `render_hero_haunted.py`, `render_category_scenes.py`, `render_worship_glyph.py`). Generated hero/ghost assets gitignored (reproducible + `/mnt/sdb-backups`); the worship-glyph + ghost-badge code ARE tracked/committed. **NEXT (graphics):** (a) operator field-walk the Pixel 8 → flag wrong heroes/markers → targeted re-roll (`render_hero_full.py --ids=`); (b) the OTHER `poi-circle-icons/` categories still pre-redo witch sigils — redo if wanted (§4 "POI circle marker"); (c) ~5 named monuments txt2img can't draw (Washington Arch, Witch Trials Memorial, WWII Memorial, Samantha) — ControlNet-from-photo or accept atmospheric; (d) splash trims 12→4 woodcut-Katrina (operator: "at the end"); (e) retire dead `hero/` UUID dir + 803 dangling `image_asset` refs + `custom_icon_asset` (art-bible §7). **Note (S206 mirror UX):** a fresh-installed paid user starting a tour sees the full Historical-Buildings layer (mirror defaults tour-mode toggle to the explore ON value); operator said "good for this" — left as-is, but candidate for a retail hard-default-OFF if it bites. (2) OMEN-025 remains the ship-blocking TOP PRIORITY but is gated on operator creds — see below._
