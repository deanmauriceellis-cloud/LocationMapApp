...
**Last updated:** 2026-04-28 — Session 193 closed. S193 shipped three demo-blocking field-bug fixes mid-session — TTS chunk-aware `onDone` (POI narrations no longer cut off mid-sentence), advance-headline-before-delay across all 5 newspaper-fire paths (no more 1692 newspaper repeats), and walk-sim FAB await-tour-Active (no more 21 km Downtown perimeter loop when an authored tour is selected). Also shipped (a) dormant historical-tour wiring end-to-end (Room v12 → v13, identity hash `d0d3d240e2d04c159767b90860fe7a8c`, `is_historical_tour` column through PG / publish-tours / admin endpoints / web admin checkbox / TourEngine / NarrationGeofenceManager); (b) hard rule per operator: "what we have as tours is definitive" — removed every synthetic-route fallback (Downtown perimeter, Heritage Trail JSON, stop-coordinate stitch); empty polyline now toasts + aborts. Saved as `feedback_authored_tours_are_definitive.md`; (c) 1692 newspaper queue: `pollNext()` returns null at corpus exhaustion (no loop-back), prefs key bumped v2 → v3 to start fresh; (d) de-hardcoded `publish-tour-legs.js` identity hash (now reads latest schema JSON). Final commit `7d15b1c`. Files NEW: `app-salem/schemas/.../13.json`, `docs/session-logs/session-193-2026-04-28.md`, `feedback_authored_tours_are_definitive.md`. Files MODIFIED: `NarrationManager.kt`, `NarrationGeofenceManager.kt`, `HistoricalHeadlineQueue.kt`, `TourEngine.kt`, `Tour.kt`, `SalemContentDatabase.kt`, `SalemMainActivity.kt`, `SalemMainActivityNarration.kt`, `publish-tours.js`, `publish-tour-legs.js`, `verify-bundled-assets.js`, `admin-tours.js`, `tourTypes.ts`, `TourTree.tsx`, both `salem_content.db` copies. PG: `ALTER TABLE salem_tours ADD COLUMN is_historical_tour BOOLEAN NOT NULL DEFAULT false`. Full detail in `docs/session-logs/session-193-2026-04-28.md`.
