...
**Last updated:** 2026-05-16 — **Session 273.** Operator-owed TTS field smoke from S272 surfaced a 3-bug chain across 4 build/install cycles, all fixed and field-verified on Lenovo + Pixel 8. (1) **TTS master-mute walk-sim stall** (S272 master-gate regression): `NarrationManager.enqueue()` silently dropped segments when master OFF → state stayed Idle → Activity collector never saw completion → `currentNarration` stuck → subsequent geofence ENTRYs appended to `narrationQueue` → walk-sim dwell loop saw `state=Idle && queueNonEmpty` and froze for the 180s safety cap on every step (3-min "I stop walking sometimes"). Three-layer fix: new `emitSyntheticComplete(segment)` helper posts `Idle` via `Handler(Looper.getMainLooper()).post` after synchronous `Speaking` so StateFlow's equality-dedup doesn't collapse the pulse when prior value was Idle; `enqueueNarration` PLAY DIRECT gates `narrationCancelForPoiAtMs = …; cancelSegmentsWithTag("newspaper_1692")` on `isTtsEffectivelyOn()` so the S146 500ms cancel-window guard doesn't suppress legitimate synthetic Idle (witnessed `Idle suppressed (transient cancel-for-POI 75ms ago)`); symmetric DEBUG ALLOW-side gate logging in `NarrationManager` (R8-stripped). (2) **Business gate ignored despite Ships & Services unchecked** (operator: *"if business unchecked when all POIs visible, I should not hear businesses"*): 7 `speakTagged{Hint,Narration}` POI call sites in `SalemMainActivityNarration.kt` (3 enqueueNarration PLAY DIRECT + 1 Linger and Listen + 3 playNextNarration) passed only 4 args, defaulting `category=null`, and `groupForCategory(null)` falls through to MEANINGFUL — bypassing the BUSINESSES toggle entirely. Plumbed `point.category` through all 7. (3) **Show-All-POI override bypassed audio gates**: after Bug 2 fix businesses STILL narrated when "Show All POIs" FAB was on; `AudioControl.isPoiSpeechEnabled()` short-circuited with `if (showAllOverride) return true`. Operator clarified: Show-All scopes VISIBILITY only; audio toggles always respected. Removed short-circuit; added DEBUG per-call trace (`isPoiSpeechEnabled cat=X group=Y enabled=Z (showAll=… meaningful=… ambient=… businesses=…)`); updated `setShowAllOverride` log copy. Other `isShowAllOverride()` consumers (NarrationGeofenceManager qualification + Narration visibility) intentionally separate — left alone. Three files: `NarrationManager.kt` (+57 LOC), `SalemMainActivityNarration.kt` (+~25 LOC), `AudioControl.kt` (+~20 LOC). Operator close: *"working now, businesses are silent"*. Full detail in `docs/session-logs/session-273-2026-05-16.md`.
...
**Sessions completed:** 273. **Internal ship target: 2026-08-01.**
...
- **Room DB:** `app-salem/src/main/assets/salem_content.db`, **v21** (S271), identity_hash `19fcd8e4347d88e9da1a50aef2734bc9`. Witch Trials intact (npc_bios 49 / articles 16 / newspapers 202). 4 tours / 73 legs (S224). **5 passports / 797 baked POIs (S269)** — 1 global Whole-Salem pool (447 POIs filtered via categories + historical_narration + min_geofence_radius_m=10) + 4 walk-derived per-tour passports (49 / 72 / 188 / 41) authored as `pool ∩ proximity`. **UserDataDatabase v2 → v3 (S268)** with `passport_visit` table + hand-written `MIGRATION_2_3` (strict, no destructive fallback per S180 lockdown). **PG schema** gained `salem_passport_filters.auto_bake BOOLEAN` (S269). **v21 added 7 hot-DAO indices on SalemPoi (S271):** `is_narrated`, `is_tour_poi`, `is_civic_poi`, `is_historical_property`, `category`, `district`, `subcategory`. v11-v21 column-history archived in session logs.
