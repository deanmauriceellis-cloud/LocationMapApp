...
### S309 opener (parked at S308 close)

_**S308 went entirely to the BlueStacks 3D-tilt corruption** — root-caused as a **renderer-agnostic BlueStacks virtual-GPU texture-memory limitation that is NOT app-fixable** (OpenGL + DirectX both fail under load; the software-layer workaround can't escape the HW-accelerated GL composite). The adaptive-GPU fix `0b81e7e` was **REVERTED**; the `markerScale` debug log + investigation doc (`docs/plans/adaptive-tilt-gpu-fix-2026-05-29.md`) were kept. **NEW carry — BlueStacks/emulator support-posture decision:** it's an unreliable GPU for tilt with no app fix; options = document as unsupported-for-tilt / capability-gate tilt OFF on detected broken-GL emulators (revisits the "no turning things off" rule) / in-app DirectX hint. **The marker "effects" work below NEVER STARTED** (the GPU investigation consumed S308) — it carries forward as the operator's stated next step. Real hardware renders tilt fine; a direct Pixel/Lenovo tilt-render confirmation is still owed._

_**ACTIVE WORKSTREAM = `PerfStabilityReview_20260529.md` perf/stability fixes.** Tier 0 (cold-start ANR, S305) + **Tier 1 (OOM floor) + Tier 2 (jank) DONE + Pixel-8-validated, coverage-safe** (S306, `dfbc99b`/`398102a`). **Marker-scale tune DONE (S307):** ghost badges equalized to the POI size ladder + all POI graphics −33% via `MarkerIconHelper.POI_GRAPHIC_RENDER_SCALE=0.67` (one knob; `labeledDot` got an `iconScale` param) — Lenovo-deployed, operator "looks good." **HARD RULE for all tilt work: never shrink the wedge-fill extension (`EXTRA_TOP_FRACTION=5.0`) — no blank wedges (memory `feedback_tilt_coverage_is_load_bearing`).** **NEXT in priority order:** (1) **Marker "effects"** — operator's stated next step ("the effects we do next apply to all"); the S307 equalize+scale work was prep for this. Scope TBD. (2) **Run the Lenovo 48° OOM gating test** (still never run — pre-empted S306 + S307) → decides whether **Tier 3** (angle-proportional `EXTRA_TOP_FRACTION`, runtime-derived from container W/H; + optional z18 deeper clamp) is required vs optional. (3) **Deep-tilt-under-walk-sim ANR** — Tier 2 killed the 30fps ambient churn but the continuous heading-up/GPS per-fix redraws under tilt still ANR'd on both devices; remainder = #3/#17/#18/#35 (per-fix marker/ring/invalidate churn) + #5 (heading-up rotation re-record under tilt)._
...
**Sessions completed:** 308. **Internal ship target: 2026-08-01.**
