...
**Last updated:** 2026-05-10 — **Session 239 (St. Peter's flat duplicate marker fix + upright drift fix + magnify-FAB icon scaling fix).** Three tilt-mode rendering bugs all traced to the same root cause: hardware-accelerated display list staleness. The S237 strip-then-pass-2 design relied on `mv.overlays.removeAll(markers)` taking effect during MapView's `onDraw`, but MapView's RenderNode wasn't being re-recorded each frame, so the renderer replayed cached marker draw commands → flat ghost markers under the tilt matrix. The same cache mechanism caused upright drift after FAB +/- zoom (when osmdroid's `setZoom` updates `mv.projection`) and made the magnify FAB (x1→x5) fail to scale upright icons (when the animator updates `mv.scaleX`) — both update mv's RenderNode without dirtying TiltContainer's display list, so pass-2 replays with stale `fabScale` + stale projection until something else forces re-record. **Fix shipped (Option 2):** added a per-Marker tilt-suppression flag (`BillboardMarker.tiltActive` companion + `draw()` override that early-returns when set), flipped on tilt 0↔>0 transitions with one-shot `mv.invalidate()`. Removed the strip/restore in `TiltContainer.dispatchDraw`. Registered a `MapListener` on the MapView while `tiltDeg > 0` (auto-unregistered at tilt=0) that calls `TiltContainer.invalidate()` on every osmdroid scroll/zoom event. Added `.setUpdateListener { binding.tiltContainer.invalidate() }` on the magnify FAB animator chain in `setupZoomToggle` so pass-2 sees in-flight scaleX. **One ANR rabbit-hole on Option 1 (`mv.invalidate()` before `super.dispatchDraw`)** was reverted within minutes — `View.invalidate()` schedules next-frame, so it created an infinite re-record loop instead of forcing same-frame re-record. **Side-quest hero-asset reachability audit:** 385 of 386 `hero/<uuid>.webp` files in the APK are dead at runtime (94 shadowed by `heroes/<slug>.webp` priority-0 in `PoiHeroResolver`, 291 orphan with no DB reference); ~12 MB AAB pre-ship reclaim opportunity logged for later. Operator: "much better, no more drift." **Carry to S240:** operator-mentioned other tilt-mode odds and ends (deferred at session close); all S238/S237/S236 carry-forwards still apply. **Previous (S238/S237/S236) context still applies below.**

**Recent context — for per-session detail see `SESSION-LOG.md`:** S238 shipped Phase 3 wedge fill (`topY` 1.0→0.6) + cull loosen + diagnostic strip. S237 shipped two-pass tilt rendering (strip-then-pass-2). S236 attempted +10mi-with-buildings raster bake → Lenovo crash → rolled back to S234 tiles (clean-repro test still owed). S234 shipped tile bake +10mi q=60 via 8-worker parallel infra + `PoiCache` singleton + heading-up fast path + V1 call-site gates. S233 promoted matrix-tilt 3D to V1 with dedicated 3D FAB. S231 shipped splash decision tree + Rapid Recon time-driven tick + admin sidebar reorg.
...
**Sessions completed:** 239. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
