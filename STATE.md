...
## Last Updated: 2026-04-09 Session 109 (side quest: column drift cleanup + GPS observer fixes + GPS Journey recorder)

## TOP PRIORITY — Next Session
**Phase 9P.B Step 9P.11 — Highlight Duplicates wiring.** Carried over from S107 → S108 → S109 again. Phase 9P.A complete; 9P.6/9P.7/9P.8/9P.9/9P.10/9P.10b all landed (S101→S106). S108 added admin-UI polish; **S109 was a side quest** that did not progress 9P.11. Operator pivoted twice: first to investigate "the admin tool doesn't have all the POIs the app sees" (root cause: importer was reading the unpopulated `salem_content.sql` not the bundled `.db`; PG narration_points was 814 rows of NULL narration since 9P.2 landed), then to investigate "GPS was terrible during my Salem test" on the Lenovo TB305FU (root cause: `100m` static dead zone in `SalemMainActivity` was eating 70% of fixes during slow downtown driving). Four independent fixes shipped + a new "GPS Journey" recording feature was built and committed. After S106, the operator can ask the Salem Oracle (gemma3:27b on local RTX 3090, port 8088) to compose, rewrite, or expand POI narration / description content from inside `PoiEditDialog`. The header pill polls `/api/oracle/status` every 30s and the Generate sub-dialog (z-1100, sits above the main edit dialog) calls `POST /api/oracle/ask` with `current_poi_id=poi.id` and pastes accepted text into `short_narration` / `long_narration` / `description` via `react-hook-form.setValue({ shouldDirty: true })`, then the existing PUT pipeline persists it. Each accepted insert is logged to `localStorage[salem-oracle-audit]` (capped 500 entries, FIFO). End-to-end smoke-tested S106 against live Oracle: response shape matches typed client exactly (9 fields in `OracleAskOk`, 8 primary_sources fields, `current_poi_id` confirmed to accept arbitrary strings — not just Salem catalog IDs).
...
## S109 fixes summary (everything that actually shipped this side-quest session)

1. **Column drift cleanup** (commit `08d431e`): dropped dead PG `pass1_narration` column, renamed `pass2_narration / pass3_narration` to `narration_pass_2 / narration_pass_3` to match the bundled DB / Room entity, fixed the importer to read `salem-content/salem_content.db` (the post-build artifact) instead of `salem_content.sql` (which has all narration columns NULL because the actual content lives in separate UPDATE files applied after the main INSERT). PG narration_points went from 814 NULL rows to **817 fully populated** (short=817, long=817, narration_pass_2=12, narration_pass_3=12). Also fixed 3 "stale overpass" rows with `created_at = 0` via a `toTimestampOrNow()` helper.

2. **GPS proportional dead zone** (commit `d229498`, `SalemMainActivity.kt:1267-1292`): replaced the fixed `100m` threshold with `(update.accuracy * 2f).coerceIn(5f, 100f)`. Root cause of operator's "GPS updates were taking 10-15 seconds" report — at slow Salem traffic speeds (~6 m/s), 100m takes exactly 15 seconds, so the cursor was deterministically refreshing every 15s as fixes accumulated movement past the threshold. The proportional dead zone self-adapts to walking, slow driving, and L1-only multipath without mode-specific tuning. Lenovo chip is fine (lifetime mean TTFF 6.49s, accuracy 3.09m, 4 constellations) — the bug was 100% app-side.

3. **idlePopulate gated to tour mode only** (commit `524a26e`, `SalemMainActivity.kt:1284-1305`): the 10-min stationary spiral was firing in both Tour mode and Explore mode. In Explore the operator is browsing freely and the spiral pulls in POIs far outside the visible viewport — wasted bandwidth. In Tour mode it's useful (prefetches the next stop's neighborhood while the user lingers at the current stop). Now gated to `tourState is Active || Paused`. Also fixed a stale "5 min" comment that lied about the actual 10-min threshold.

4. **GPS Journey recorder + visibility toggle** (commit `5428795`): NEW feature. Always-on Room recorder in a separate `UserDataDatabase` (kept isolated from the bundled `SalemContentDatabase`), 24h rolling retention, hourly piggyback prune. New bottom-left FAB toggles polyline visibility ONLY (recording continues regardless). Default ON, persisted in SharedPreferences `gps_track_visible`. Polyline is semi-transparent magenta (`#B3FF00FF`), 8dp wide, painted below POI markers. Recording runs entirely in the activity's foreground lifetime — no foreground service, no background-location permission, no extra manifest changes. Manual on/off control deferred to a future utility/settings menu (operator's decision). 5 new files in `app-salem/.../userdata/` + SalemMainActivity wiring + activity_main.xml FAB.

## S109 known-but-not-fixed (next-session cleanup candidates)

- **`DWELL_MS = 3_000L` in `NarrationGeofenceManager` is dead code.** Declared, never referenced. Geofence ENTRY/APPROACH triggers fire on the first fix that lands inside the ring, no continuous-dwell requirement. Means single jittery fixes can fire false-positive narration in dense areas like the Essex/Charter cluster. Recommended fix is a per-POI `firstSeenInside: Long?` map with "require 2 consecutive fixes inside the ring" semantics — express it as fix-count, not milliseconds, since at the current 5s adaptive sampling the millisecond value is meaningless. ~10 lines in `NarrationGeofenceManager.checkPosition()`.

- **Narration queue improvements**: in dense POI clusters (Witch House corner) all 5+ in-range POIs queue immediately on the first fix and play back-to-back (~3-4 minutes of committed audio) even after the operator walks away. Considered fixes: queue size cap (e.g. MAX=4), stale-pop check (verify still in `2× geofenceRadiusM` before playing), cluster batching (treat 3+ simultaneous as a single attention event), time-bound the queue. None applied — flagged for future. Stale-pop check is the cheapest (~5 lines).
