...
**Last updated:** 2026-05-29 — **Session 304.** Tech-debt **Phase 0 shipped** (`14c2481`: `cache-proxy/scripts/publish-all.js` fail-fast orchestrator + shared `cache-proxy/lib/env.js` + transaction-wrapped DROP/DELETE in the 3 publish scripts + a pre-existing `publish-poi-collection --dry-run` `pool.end()` deadlock fix + characterization tests [`MenuPrefs` mirror 10, `MarkerIconHelper` 5] + Node 20→24 CI bump; verified by a real write-path run with the asset restored byte-for-byte) + **Phase 1a started** (`a5bf61e`: new `JobCoordinator` — named jobs, auto cancel-before-relaunch, single `cancelAll()`, 6 tests — and the first cluster `geofenceReload` migrated; `gpsObsHeartbeatJob` + parked clusters left untouched). Deployed to Pixel 8; operator walk-sim smoke **clean** (0 crashes, 0 `HEARTBEAT STALE` across a 2.5-min onPause→onResume — **S303 symptom did not recur**; 4 POIs narrated; geofence path clean). Opened with a 9-agent workstream survey (`UltraCodeReview_20260529.md`): Aug-1 ship is **operator-blocked** (OMEN-025 creds + un-started Play Console ID clock), not engineering-blocked. **P1 paused mid-phase for more review next session.** Tags `cleanup-phase0-pre/-done`. Full S303+S304 detail: `SESSION-LOG.md`.
...
### S305 opener (parked at S304 close)

_**Resume tech-debt Phase 1** (paused mid-phase; rollback tags `cleanup-phase0-pre` → S303 state, `cleanup-phase0-done`=`14c2481`; P1a=`a5bf61e`). FIRST apply the S304 review finding: `JobCoordinator.jobs` HashMap → `ConcurrentHashMap` (latent off-Main self-evict race once a non-Main-completing cluster migrates; benign today) + confirm `DebugLogger.d` is R8-stripped. THEN migrate the remaining active job clusters (cachePoi → walkSim; populate cluster has resume-state — careful; narration is S303-adjacent) → build **1b MarkerController** (overlay add/clear/invalidate + absorb `narrationMarkerMutex`; collapses ~12 clear/add fns) → on-device walk-sim smoke → tag `cleanup-phase1-done`. Operator also wants **more code review** next session. Plan `docs/plans/tech-debt-cleanup-2026-05.md` (P0 DONE; P2 dead-hero / P3 isolate-parked / P4 VM-decomp / P5 hardening / P6 web remain). `NEXTSTEPS_20260527.md` Qs resolved-by-action: start=P0, defer `custom_icon_asset` column-drop, posture = P0 now / rest post-cred or post-ship._

_**S303 GPS-OBS fix — smoke clean, not definitively closed.** S304 Pixel 8 walk-sim showed 0 stale across a 2.5-min onPause→onResume (no recurrence) + 4 POIs narrated, but walk-sim feeds synthetic GPS — a real-world drive (5/27-style: real screen-off + quiet hardware motion sensor) is still the gold-standard close. Nice-to-have, not blocking._
...
**Sessions completed:** 304. **Internal ship target: 2026-08-01.**
