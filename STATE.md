...
**Last updated:** 2026-05-15 — **Session 268.** Shipped 3 of 5 plan-slice sessions for the POI Passport architecture — 60% landed in one session. **S1 backend foundation**: new PG tables `salem_passport_filters` + `salem_passport_pois`; admin CRUD endpoints + preview at `/admin/salem/passports`; bake script `publish-poi-passport.js` with forward-compat SQLite gating; web admin "Passports" sub-tab driven by `/api/admin/salem/categories` + `/api/admin/salem/tours`; `default_salem_walking` filter authored end-to-end → 85 POIs matched. **S2 Room schemas**: `SalemContentDatabase` v19 → **v20** (new `PoiPassport` entity + DAO, identity hash `837ec05ad90541fa76a8a413a06394e0`); `UserDataDatabase` v2 → v3 with strict hand-written `MIGRATION_2_3` (no destructive fallback per S180 lockdown); `verify-bundled-assets.js` refactored to read hash dynamically from `schemas/*.json` — kills the manual-hash-bump footgun. **S3 UI + visit tracking**: new `PassportSheet.kt` full-screen DialogFragment (category-grouped POI list with filled/hollow witch-hat stamps, passport-picker dropdown, reset overflow); new witch-hat toolbar icon wired via `MenuEventListener.onPassportRequested()`; fire-and-forget `passport_visit` UPSERT at `NarrationGeofenceManager` ENTRY + `PoiDetailSheet.queueSheetReadThrough` non-stripped path. Operator binding decisions from plan mode (7 dialog clarifiers): ship in V1; visit log is POI-keyed only (single global table; per-tour passports are filtered ID lists rendering against it; Business POIs later plug into the same log); filter knobs = category multi-select + `require_historical_narration` + `min_geofence_radius_m` + year-built range; completion fires when `count(passport_visits for tour) == count(tour_passport.poi_ids)`; stamp art = filled vs hollow witch hat. Mid-session pivot: per-tour intersection on `salem_tour_stops.poi_id` returns zero (all post-S190 tours are polyline-only with NULL poi_id) — `tour_id` is metadata-only in V1. New memory rule saved: `feedback_tour_stops_not_poi_anchored.md`. Debug APK 99.4 MB; `assembleDebug` + `verify-bundled-assets` clean (24/24); no field walk yet (operator handoff for bundletool install + walk validation). **S4** (deletion of 4 dead stop-UIs + completion-trigger replacement + `SalemMainActivityDetour.kt:180` anchor fix) and **S5** (field validation + polish) still owed. V1 ship-cliff status unchanged. ~77 days to 2026-08-01 ship; 5 days to Form TX deadline 2026-05-20. Full detail in `docs/session-logs/session-268-2026-05-15.md`.
...
### S269 opener: POI Passport S4 — destructive session

Plan: `/home/witchdoctor/.claude/plans/abstract-doodling-nest.md`. S4 deletes the four leftover stops-based tour UIs and replaces the tour-completion mechanism:

1. **Delete** `showActiveTourDialog()` (~638-859), `updateTourHud()` + `removeTourHud()` (1300-1374), numbered-pin loop in `drawTourRoute()` (~1110-1145, keep the polyline draw above it) in `SalemMainActivityTour.kt`. Rewrite `showTourCompletionDialog()` body (function header at 1564, stat lines at 1615-1620) to render Passport summary for the just-ended tour + "Open Passport" CTA.
2. **Delete** stops machinery in `TourEngine.kt` (`currentStopIndex`, `completedStops`, `skippedStops`, `advanceToNextStop()`, `skipStop()`, `addStop()`, `removeStop()`, `reorderStops()`, `maybeAdvanceOnEntry()`, 4 `KEY_*` SharedPrefs), `TourModels.kt` (stops/progress fields on ActiveTour/TourProgress/TourSummary), `TourViewModel.kt` (advance/skip/add/remove wrappers + `getStopLocation()` / `distanceToStop()` / `isAtCurrentStop()` at ~192/194/249/250/253).
3. **CRITICAL** — replace `SalemMainActivityDetour.kt:180` (`activeTour.progress.currentStopIndex.coerceIn(...)`) BEFORE deleting `currentStopIndex`. New helper in `TourEngine`: `currentLegIndexForDetourAnchor(userLat, userLng): Int` walks `tour_legs` for the active tour and returns the index of the nearest leg-end vertex.
4. **New `TourState.Completed` trigger**: `passportVisitDao.countHeardAmong(tourPassportPoiIds) == tourPassportTotal` fires Completed once per session (latch in `ActiveTour.completionFiredThisSession: Boolean`, cleared in `endTour()` / `startTour()`).

Then **S5** — operator field walk on Lenovo (bundletool install + tap witch-hat icon + verify 85 hollow stamps + walk-sim a few POIs → stamps accumulate + reset works + Pixel 8 portrait toolbar fit at 9 icons); empty-state messaging on PassportSheet when filter yields 0 POIs; memory + STATE/SESSION-LOG/MASTER_SESSION_REFERENCE polish.

**S268 carry-forwards still open:** operator end-to-end smoke test of the S268 build (bundletool install → tap toolbar witch-hat → 85 hollow stamps → reset → walk → stamps accumulate); MIGRATION_2_3 verification (the v2→v3 upgrade path is exercised only on in-place install over the pre-S268 build — `adb uninstall` skips it). Linger and Listen (S267) field walk to validate subtopic body queueing still owed.

**S267 latency-fix carry-forwards (not addressed in S268):**
...
**Sessions completed:** 268. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
...
- **Room DB:** at `app-salem/src/main/assets/salem_content.db`, **v20 schema** (S268), identity_hash `837ec05ad90541fa76a8a413a06394e0`. Witch Trials intact (npc_bios 49 / articles 16 / newspapers 202). 4 tours / 73 legs (S224). 1 passport / 85 baked POIs (S268). v11 (S186) `is_civic_poi`. v12 (S192) `historical_narration`. v13 (S193) `is_historical_tour`. v14 (S195) `is_historical_property`. v15 (S198) dropped `body_points` from WitchTrialsNewspaper. v16 (S216) dropped `historical_note` from SalemPoi. v17 (S219) added `narration_subtopics` JSONB to SalemPoi. v18 (S226) added 6 haunt columns. v19 (S227) added `haunt_duration_s` REAL. v20 (S268) added `PoiPassport` entity (`poi_passport` table). **UserDataDatabase** also bumped v2 → v3 (S268) with `passport_visit` table + hand-written `MIGRATION_2_3` (strict, no destructive fallback per S180 lockdown).
