...
**Last updated:** 2026-05-29 — **Session 305.** Three threads. **(1)** Tech-debt **Phase 1 banked at the JobCoordinator infra level** (`54b8a66`, tag `cleanup-phase1-done`): ConcurrentHashMap hardening + cachePoi/autoDump/narrationHighlightAnim migrated; walkSim/populate/1b-MarkerController principled-deferred (P3/P4). **(2)** Full **multi-agent perf/stability review** (`wf_eaa9cdcd-b4b`, 98 agents, ultracode) → **`PerfStabilityReview_20260529.md`**: 85 raw → **63 confirmed / 61 V1-reachable** (22 false positives killed), Tier 0–3 fix order. Headline: the #1 ship risk is a **cold-start ANR**, not the 3D tilt. **(3)** **Tier 0 cold-start ANR FIXED + on-device-validated** (`9ccdca0` + `086361a`): the 370 MB tile copy + verify + PolygonLibrary + ambient-overlay grid-scans + PlaceResolver + splash JPEG decode all moved off the main thread, splash gates the map handoff on `OfflineTileManager.awaitReady()`. Pixel 8 fresh-install pass CLEAN — 370 MB copy **294 ms off-main (no ANR)**, 2nd-launch cheap probe 3 ms (no scan), ambient overlays async-attached, **operator-confirmed "splash works fine."** The parked **3D-tilt OOM** was root-caused during the review (24× viewport extension + unsized tile cache + `onTrimMemory` no-op + un-recycled full-res ghost/hero bitmaps from the S299/S301 graphics redo) and is queued **Tier 1** for next session. Full S305 detail: `docs/session-logs/session-305-2026-05-29.md`.
...
### S306 opener (parked at S305 close) — "we continue fixing and testing"

_**ACTIVE WORKSTREAM = the perf/stability fixes from `PerfStabilityReview_20260529.md`** (repo root; 63 confirmed findings, Tier 0–3 fix order, refuted appendix + critic gaps). **Tier 0 (cold-start ANR cluster) DONE + on-device-validated** (`9ccdca0` #1+#30, `086361a` #32+#13/#31/#57). Continue with **Tier 1 — 3D-tilt OOM cluster** (the parked "worked before" regression; ships in retail via `BuildDefaults.TILT_3D_ENABLED=true`, all builds — the `MenuPrefs.kt:141` `=BuildConfig.DEBUG` comment is STALE). Root-caused as a confluence: `TiltContainer` extends the MapView to ~24× tile area (#33/#24), no zoom cap → z20 (#14), osmdroid tile cache never sized for the extent (critic-2), `onTrimMemory` sheds 600KB JSON not the bitmaps (critic-1), full-res ghost(384²)/hero(1152×512) decodes never recycled / undersized LRUs (#6/#21/#28 — the S299/S301 graphics-redo LOS pressure = the regression). Fix recipe: cap `targetTop`/`EXTRA_SIDE_CAP` + zoom clamp while tilted + size the tile cache + make `onTrimMemory` evict bitmaps (MarkerIconHelper/GhostResolver/PoiHeroResolver caches + `tileProvider.clearTileCache()`) + downsample ghost/hero decodes. Then **Tier 2** (30fps unconditional MapView invalidate #11/#12 — highest-leverage; `project()` allocs #9; per-GPS-fix churn #3/#17/#18) and **Tier 3** (bitmap recycle/LRU sizing, TTS-shutdown dead-code critic-3). Each fix on-device-tested ("we continue fixing and testing")._

_**Carries:** (a) **#31 PlaceResolver** off-main fix needs LOCATION granted to exercise on-device (fresh-install pass took the UNKNOWN path). (b) Min-spec **Lenovo** Tier-0 pass (slower eMMC copy — should still be off-main/no-ANR + show the "Preparing your offline map…" hint). (c) **Tech-debt plan** `docs/plans/tech-debt-cleanup-2026-05.md` (P0+P1 done; P2 dead-hero / P3 isolate-parked / P4 VM-decomp / P5 / P6 remain) — now LOWER priority than the perf tiers. `walkSim` JobCoordinator migration needs a `suspend cancelAndJoin(name)` extension first (S125 race). (d) S303 GPS-OBS fix smoke-clean across S304+S305; a real-world drive is the gold-standard close (nice-to-have)._
...
**Sessions completed:** 305. **Internal ship target: 2026-08-01.**
