...
**Last updated:** 2026-04-25 — Session 176 closed. **Three S175 carry-forwards SHIPPED**: P3b (live re-routing), P3c (MBTA-origin walk-sim), P2c (Router JVM parity tests). New `DirectionsSession` (`app-salem/.../routing/`) tracks drift off the active polyline (>25 m sustained over 2 consecutive fixes triggers reroute) and arrival within 15 m of destination (clears the session). Wired into `TourViewModel` so every GPS update inspects the active route. P3c: when `lastGpsPoint` is >3 km from MBTA Salem station (42.524, -70.8989), `walkTo(POI)` forks to `startSimulatedWalkFromMbta` — bundled-router route from station to POI is published as the active route, then a new `walkSimAlongPolyline` helper drives the simulator at 1.4 m/s using the existing walkMutex/walkSimJob plumbing. P2c: new `core/src/test/` source set with `JdbcRoutingBundleLoader` (sqlite-jdbc) and `RouterParityTest` (10 tests, ±1 mm bit-exact pinning of the four S175 reference routes plus degenerate edge cases). Two debug endpoints added: `/directions-session-test` (deterministic 7-step harness for the session state machine) and `/p3c-test` (drives the full out-of-Salem path on-device). All pass on Lenovo TB305FU. Full detail in `docs/session-logs/session-176-2026-04-25.md`.
...
## TOP PRIORITY — Next Session (S177)

0. **DEBUG / VISUAL POLISH ON DIRECTIONS POLYLINE** — carry-forward from S175. Color is `#22C55E` (green) on Lenovo. Operator owes a manual walk-through (tap-POI → Directions, report line width, opacity, labels, start marker, etc.). Routing math is mm-parity with TigerLine; any issue is in `drawWalkingRoute` / `showDirectionsInfo` / `showTurnByTurnDialog`.

1. **P4 — Web router in cache-proxy.** Carry-forward from S175. New `cache-proxy/lib/salem-router.js` opens the same `salem-routing-graph.sqlite` via `better-sqlite3`, exposes `GET /api/salem/route?from_lat&from_lng&to_lat&to_lng[&source=live]` and `POST /api/salem/route-multi`. `?source=live` falls through to TigerLine's live `tiger.route_walking()` for verification — logs divergence warning if results differ by >5%. Same single graph the APK uses, so web + Android produce identical routes; the new `RouterParityTest` reference fixtures from S176 P2c are an obvious port target for the JS side.

2. **P5 — Web Directions UI.** Carry-forward from S175. Web admin POI popup gets a "Directions" button → Leaflet polyline overlay using the P4 endpoint. Style coherent with the Android overlay (green polyline + border, distance/duration label).

3. **P6 (deferred) — Pre-bake tour-leg polylines.** For each `salem_tour_stops` row, pre-compute polyline + distance + turn-by-turn between consecutive stops and store in new `route_geom_to_next` + `turns_to_next` (TEXT JSON) columns. Eliminates router calls during guided tours; admin tour editor can preview polylines immediately when a stop is dragged.

4. **DirectionsSession JVM tests (deferred).** S176 P2c covered Router only. The on-device `/directions-session-test` endpoint already validates the same state machine + geometry helpers, so this is convenience-not-correctness — pull in if/when DirectionsSession changes.

5. **S176 P3b/P3c — done.** Deleted from priority list; full detail in the live log.
