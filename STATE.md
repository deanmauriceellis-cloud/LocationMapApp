...
**Last updated:** 2026-04-30 — **Session 207 (Civic narration trim).** Operator: "civic POIs have too much information, like the memorials and statues — is there a way to control their narrative?" DB inventory invalidated the initial commemorative-runtime hypothesis (zero civic match the commemorative regex; zero have `historical_narration`; zero are pre-1860). Real source of bloat was `long_narration` heard at DEEP detail (15+ civic rows >800 chars). Operator picked: NULL `long_narration` for all civic. Snapshot of 62 rows to `docs/archive/civic-long-narration-snapshot-2026-04-30.csv` (reversible), UPDATE applied, full publish chain + Lenovo HNY0CY0W uninstall+install. No code changes; no schema changes (Room v15 unchanged). Carry-forward: operator validation drive on civic short-narration feel; selective re-author from snapshot if any single civic POI feels too thin. **Session 206 (prior):** Operator opened with "make sure the web api tool is working." Cache-proxy was down on 4300; restarted it; verified S205's "Apply auto-eligible 706" had been clicked between sessions (missing-subcategory backlog now 211 review-only). Operator hit `apply-categorization → 413` — root cause was `cache-proxy/server.js:25` global `express.json({ limit: '16kb' })` rejecting the ~19 kB 211-row payload; bumped to `1mb`. Wired `AutoCategorizeModal` POI name as button → opens PoiEditDialog + closes modal so operator can drill from auto-cat review into the standard editor. Confirmed three independent guards keep auto-categorize structurally unable to read or write HISTORICAL_BUILDINGS / CIVIC rows (recent HB updated_at is operator's manual edits, not auto-cat). Ran the full 5-script publish chain (2,215 POIs / 3 tours / 46 legs / Room v15 identity_hash `3e927300be7b2a8971fa6afb4aa5af78`) + assembleDebug (102 MB) + Lenovo uninstall+install. **Operator drove ~50 min with wife and reported "lots of things seemed to be missing"** — pulled 1M-line logcat: DensityTour (0 stops) started at 19:55:52 with allowHist/allowCivic=false, layers toggled mid-drive, **22 POIs queued / 7 spoke / 12+ stale-dropped at driving speed (>40 m behind user)**. Three compounding causes: tour-mode silenced layers by default, queue starvation at speed, DensityTour has 0 stops. Investigation found two real bugs en route. **Bug 1 — pref split silently lies:** explore-mode prefs default ON, tour-mode prefs default OFF; if operator has boxes checked in Explore (default) and starts a tour, tour-mode prefs are unset → fall through to false → markers + narration both go restrictive without warning. Fixed by making `MenuPrefs.histLandmarkPrefDefault` / `civicPrefDefault` accept optional `prefs: SharedPreferences?` and mirror the explore-mode pref when in tour mode + unset; threaded `prefs` through 8 call sites in `AppBarMenuManager`, `TourEngine`, `SalemMainActivity`. **Bug 2 — `is_tour_poi=true` "force-visible override" only honored at the layer gate, not the tier filter above it.** 128 of 222 tour-flagged POIs (PARKS_REC 67, EDUCATION 36, WORSHIP 19, ENT 3, SHOP 2, WITCH_SHOP 1) silently invisible at restrictive layer states. One-line fix: added `p.isTourPoi` to tier-filter admission in `SalemMainActivity.kt:2212`. Two rebuild + reinstall cycles on Lenovo. Discussed but did NOT ship a speed-aware queue cap for the queue-starvation case (the actual cause of Samantha not narrating on the drive — she was queued in slot #4 then stale-dropped 52 s later at 242 m past). **Session 205 (prior):** Category/subcategory lint pass + SI-driven auto-categorization. `PoiHeroResolver.kt` icon prefix-match fix (strip `CATEGORY__` namespace), 4 new Taxonomy lint checks, SI-driven auto-categorize engine (706 auto-eligible / 212 review at dry-run). Web admin `AutoCategorizeModal.tsx` shipped. Vite HMR config fixed for LAN access. **Session 204 (prior):** All 7 device-dependent V1 carry-forwards from S203 closed in one pass. Real bug shipped (#11 GPS-OBS heartbeat — adaptive GPS interval picker lifted upstream of the stationary freeze; stale threshold 30s→45s; verified clean). #3 Witch Trials content audit cleared 11/49 NPC bios of S192 meta-gap violations (anti-meta-gap prompt rule + 2 regen passes + 3 hand-edits). #17/#29/#21/#25/#20 verified. **Cross-repo Salem commit owed (still).** **Session 203 (prior):** V1 backlog drive-through — Phase A cleanup, Phase B V1/V1.0.1/V2 triage, Phase C V1 execution, Phase D blocked on Lenovo. **Session 202 (prior):** Recon Camera. **Session 201 (prior):** Aug 1 ship anchor + strategic doc cleanup. **Session 200 (prior):** Five shipping arcs (commercial-prose legal cleaning, MASSGIS landmark surfacing, McIntire stub, etc.). Detail in `docs/session-logs/session-{200,201,202,203,204,205,206}-*.md`.
...
### S208 next-session actions

0. **S207 civic narration validation** — on the next drive/walk, confirm that civic POIs (schools, fire stations, community centers, parks signage, etc.) feel right at DEEP detail now that they speak only short_narration. If any specific civic POI deserves DEEP, pull from `docs/archive/civic-long-narration-snapshot-2026-04-30.csv` and selectively re-author.
1. **Operator drive regression on the S206 APK** — verify the three S206 fixes hold on a real drive: 413 unblocked, tour-mode pref mirror, tier-filter `is_tour_poi` honored end-to-end.
2. **Speed-aware queue cap** (NEW carry-forward, queue-starvation discussion deferred at S206 close) — at >15 mph, replace any queued narration item rather than appending so the operator hears the closest unheard POI on a fast drive instead of a 50-second blurb about a place 450 m behind. Alternative: switch to SHORT-narration tier at high speed. Pick approach + ship.
3. **DensityTour has 0 stops design question** — a 0-stop authored tour silently triggers OUT-OF-SALEM walk-sim fallback. Either auto-fall-back to Explore mode at tour start, or refuse to start a 0-stop tour. Decide + ship.
4. **Cross-repo Salem commit owed (S204→S205→S206 carry-forward, still owed)** — `Salem/cmd/genbiographies/main.go` + 3 hand-edited bio JSONs need committing in the Salem repo. Operator-driven.
5. **Walk the 211 review-only auto-categorize proposals** — 24× ENTERTAINMENT→TOUR_COMPANIES, 68× SHOPPING→OFFICES, etc. Already-applied 706 auto-eligible at S205→S206 boundary. Apply the remaining cross-cat batch deliberately.
6. **Rebuild signed AAB + 30-min Lenovo smoke** — first signed AAB since S180 baseline. Use the V1 gating checklist (Visit Website ACTION_VIEW handoff, Find dialog Reviews/Comments hidden, Find Directions visible, toolbar gating, webcam "View Live" hidden, recon camera DEBUG-only — verify the BuildDefaults switch ships retail posture in release).
7. **Tier-2 auto-bake** (carry-forward from S203→S206) — wrap the 5-script publish chain as a `preBuild`-dependent Gradle task with stale-bake warning. ~45 min.
8. **Operator field-walk validation** (S204 carry-forward) — wife's next Salem walk validates GPS-OBS heartbeat + Witch Trials bios + full TTS-gated dwell on real POI traversal.
9. **Drive regression in airplane mode** — validate offline posture under real GPS load (#20 carry-forward).
10. **#9 Re-author 5 polyline tours via web admin** (S185 carry-forward, operator-driven).
11. **#6 McIntire content drain** (S200 carry-forward, operator hand-author).
12. **CLAUDE.md "Pinned for next session start" cleanup** — the pinned block still references Room v11 and S188/S187/S186 state; actual schema is v15 and many pinned items have shipped. Per close protocol CLAUDE.md isn't routinely touched, but a one-time refresh is overdue.
