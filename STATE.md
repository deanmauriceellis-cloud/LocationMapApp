...
**Last updated:** 2026-05-05 — **Session 226 (per-POI Haunt Effect — admin-driven sprite peek shipped end-to-end).** S170's 7 sprite figures (witch / owl / black-cat / katrina-kitty / skeleton / mouse / rat) finally wired into the running app via a new `SpriteOverlay : MapOverlay` (~190 LOC) hosted by the existing `WickedAnimationOverlay` bridge alongside water/firefly/grass. Schema Room v17 → v18 (identity `e6c5a128efdd9949f7297c88ada7d698`) added 6 columns to `salem_pois`: `haunt_sprite_id` TEXT (NULL = blank, admin opt-in only) + `haunt_outer_range_m` / `haunt_outer_interval_s` / `haunt_inner_range_m` / `haunt_inner_interval_s` INT + `haunt_enabled` BOOL DEFAULT TRUE. Two-band cadence per the operator's spec (outside outer range silent; outer band fires every outer_interval_s; inner band fires every inner_interval_s). 1-second swoop animation: bell-curve alpha + float-up + scale curve + lazy bitmap cache. New **Haunt** tab in `web/src/admin/PoiEditDialog.tsx` between Flags and Location with sprite dropdown + 4 numeric inputs + Enabled toggle (purple-tinted card). Dialog grew to 1100 px × 95 vh + Quality flags box trimmed `max-h-48 → max-h-32` so the form has room. Cache-proxy `admin-pois.js` UPDATABLE_FIELDS extended; `publish-salem-pois.js` carries the columns through PG → SQLite; `align-asset-schema-to-room.js` stamps v18; `verify-bundled-assets.js` constant updated. Bundled 7 sprite WebPs (~992 KB) into `app-salem/src/main/assets/sprites/`. `SalemMainActivity` injects `SalemPoiDao`, instantiates SpriteOverlay in `setupMap`, async-loads haunt configs from Room into the overlay, feeds GPS to `setUserLocation` on every fix. Post-deploy snag: admin-created POI without narration was filtered out of the marker loader because `findNarrated()` requires `is_narrated=true`; fix: loosen that DAO query to also surface `haunt_sprite_id IS NOT NULL AND haunt_enabled = 1` rows (the geofence narration system's downstream filters short-circuit non-narrated POIs anyway). End-to-end working on Lenovo HNY0CY0W: marker count 1303 → 1304 (the +1 is the new admin-created Beverly/Salem City Line POI configured with skeleton haunt + default tuning); SpriteOverlay haunt configs loaded — 2 POIs (Witch House witch / 250 m / 20 s outer / 80 m / 5 s inner; City Line skeleton / blank → defaults 200 m / 60 s / 50 m / 15 s); no FATAL / no schema-migration crash. Field-walk verification of the actual sprite firing on Lenovo near either POI is owed. Sprite polish (rotation through 16-frame WebPs, audio, per-character motion profiles) deliberately deferred. **Session 225 (prior — web admin can create POIs via new + New POI flow, S225 PoiCreateDialog).** **Sessions 217-224 (prior — category-aware FAB narration + Show-All FAB override; per-POI TigerLine/Google Validate workflow; storytelling-with-subtopics framework Room v17 + subtopic UI + 497-POI auto-gen; tour detour feature; cache-proxy dotenv autoload; web-admin restart; lean+state protocol amendment; walk-sim queue stall fix + Find/PoiDetailSheet V1-dead code purge).** **Operator-stated hard pause from S214 still in effect: Salem Oracle (:8088) + SalemIntelligence (:8089) are undergoing major reformulation to attach full per-claim provenance; without source attribution V1 content cannot ship legally — do not propose work consuming them or backfilling content from them until the operator signals the pause is lifted (memory `project_oracle_intelligence_paused_provenance.md`).** **Cross-repo Salem commit owed (still).** Per-session detail in `docs/session-logs/session-{217..226}-*.md` and `SESSION-LOG.md`.

> Older "Previous" entries (S180 / S183-S189 trimmed S201; S195 trimmed S202; S196-S199 trimmed S204; S213-S216 prose trimmed S224; S217-S224 prose collapsed S225; S225 prose collapsed S226) — moved out to keep STATE.md under the 200-line cap. Detail lives in `SESSION-LOG.md` + `SESSION-LOG-ARCHIVE.md` + per-session live logs at `docs/session-logs/session-NNN-*.md`.
...
### S227+ next-session docket — Haunt polish + Find menu refinement (deferred from S224)

**Haunt feature carry-forwards:**
- Operator field-walk validation of the haunt firing on Lenovo near Witch House + City Line.
- Sprite rotation by user→POI bearing using the 16-frame WebPs (currently first-frame-only render).
...
**Sessions completed:** 226. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
...
- **Room DB:** at `app-salem/src/main/assets/salem_content.db`, **v18 schema** (S226), identity_hash `e6c5a128efdd9949f7297c88ada7d698`. Witch Trials intact (npc_bios 49 / articles 16 / newspapers 202). 4 tours / 73 legs (S224). v11 (S186) `is_civic_poi`. v12 (S192) `historical_narration`. v13 (S193) `is_historical_tour`. v14 (S195) `is_historical_property`. v15 (S198) dropped `body_points` from WitchTrialsNewspaper. v16 (S216) dropped `historical_note` from SalemPoi. v17 (S219) added `narration_subtopics` JSONB to SalemPoi. v18 (S226) added 6 haunt columns (`haunt_sprite_id` + 4 tuning ints + `haunt_enabled`).
