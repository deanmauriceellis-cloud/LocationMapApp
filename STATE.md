...
## Last Updated: 2026-04-08 Session 103 (Phase 9P.B Step 9P.8 — POI tree (react-arborist) landed)

## TOP PRIORITY — Next Session
**Phase 9P.B Step 9P.9 — Admin map view (Leaflet).** Phase 9P.A (Backend Foundation) is complete; Step 9P.6 (TS taxonomy port) landed S101; Step 9P.7 (admin route + AdminLayout shell) landed S102; Step 9P.8 (POI tree) landed S103. Next: create `web/src/admin/AdminMap.tsx`, render all 1,720 POIs as clustered Leaflet markers (verify `leaflet.markercluster` is installed first), wire the AdminLayout `handlePoiSelect` callback to pan/zoom the map to the clicked POI, click marker → opens edit dialog (placeholder until 9P.10), enable marker drag with `marker.dragging.enable()` + dragend confirm modal that POSTs to `/admin/salem/pois/:kind/:id/move`. The PoiTree is already wired to emit `onSelect({kind, poi})` events into AdminLayout — the next session just has to build the map and consume them. After 9P.9: 9P.10 (edit dialog), **9P.10b (Salem Oracle "Generate with AI" integration — see `~/Development/Salem/docs/oracle-api.md`)**, then Phase 9P.C (publish loop: regenerate `salem-content/salem_content.db` from PG so APK builds pick up admin edits).
