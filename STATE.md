...
**Last updated:** 2026-05-16 — **Session 274.** Operator pivoted off the S273 docket onto a brand-rebrand: "POI Passport" rebadged as **"Katrina's Collection"** to differentiate from the many local Salem-Passport products. Operator's framing: gamified ghost collection — each unique historical building becomes a catchable "ghost" with bespoke Salem-themed art. Phase 1 of a 4-phase plan shipped clean. **Phase 1 — mechanical rename across all layers (~50 files):** PG (`salem_passport_filters` → `salem_collections`, `salem_passport_pois` → `salem_collection_entries`, `filter_id` → `collection_id`; idempotent ALTER block at bottom of `salem-schema.sql`). Cache-proxy (`admin-passport.js` → `admin-collection.js`, `publish-poi-passport.js` → `publish-poi-collection.js`; API `/api/admin/salem/collections`, `collection-walk-candidates`, `walk-derived-collection`). Room content DB **v21 → v22** (`PoiPassport` → `CollectionEntry`, table `poi_passport` → `collection_entry`; new identity hash `07f63ead9c5c3c05d87dcb6d54f64ba8`; no MIGRATION needed — bundled asset uses `fallbackToDestructiveMigration` per S180 design). Room user DB **v3 → v4** (`PassportVisit` → `PoiVisit`, table `passport_visit` → `poi_visit`; hand-written `MIGRATION_3_4` renames in place to preserve paid-user history per S180 lockdown). Android UI (`PassportSheet` → `CollectionSheet`; `TourModels` property cascade `passportId/Total/Heard` → `collectionId/entryTotal/entriesVisited`; `onPassportRequested` interface → `onCollectionRequested`). Web admin (`PassportTab` → `CollectionTab`, `WalkPassportDialog` → `WalkCollectionDialog`, TS `PassportFilter` interface → `Collection`, sidebar id/label). Resources (4 layout/drawable file renames, `passport_stamp_filled` color → `collection_entry_visited`). Cleaned the stale `poi_passport` ghost table out of the bundled asset post-rename. Built clean, bundletool installed on Lenovo + Pixel 8 (uninstall first per WAL-safety rule); both devices logcat confirmed `SalemContentDatabase opened: schema_v=22 tables=13` with `collection_entry` present and zero destructive migrations. **Phase 2-4 carry-forwards:** 107 HIST_BLDG POIs are ghost-eligible (live PG count; operator's recall "94" was off by 13 — likely commemoratives in their head; HISTORICAL_LANDMARKS does NOT get ghosts per operator). Phase 2 = AI-Studio batch of 107 unique cute Salem ghost portraits + `salem_pois.ghost_asset` column + `GhostResolver.kt`. Phase 3 = UI polish (collected = color portrait, uncollected = full POI info + silhouette art per operator decision). Phase 4 = field smoke. **Default-walking-is-obsolete** carry-forward from operator: the 447-entry Whole-Salem collection can be retired when convenient. Memory updates: renamed `feedback_passport_pool_intersect_proximity.md` → `feedback_collection_pool_intersect_proximity.md`; renamed `feedback_stamps_not_stops.md` → `feedback_ghosts_not_stops.md`; new `project_katrinas_collection_rebrand.md`. Full detail in `docs/session-logs/session-274-2026-05-16.md`.

### Prior — S273 (2026-05-16)

Operator-owed TTS field smoke from S272 surfaced a 3-bug chain across 4 build/install cycles, all fixed and field-verified on Lenovo + Pixel 8: (1) **TTS master-mute walk-sim stall** — `NarrationManager.enqueue()` silently dropped segments when master OFF, freezing walk-sim for the 180s safety cap on every step; fixed with `emitSyntheticComplete()` helper (Handler-posted Idle pulse) + skip cancel-stamp when muted to dodge the S146 cancel-window guard. (2) **Business gate ignored despite Ships & Services unchecked** — 7 POI call sites passed `category=null` to `speakTagged*`; plumbed `point.category` through all 7. (3) **Show-All-POI override bypassed audio gates** — removed the `if (showAllOverride) return true` short-circuit in `AudioControl.isPoiSpeechEnabled()`; Show-All now scopes to visibility only. Three files, ~100 LOC. Operator close: *"working now, businesses are silent"*.

### Prior — S272 (2026-05-16)

Three chained operator asks: (1) **CLAUDE.md + STATE.md doc lint** after a mid-session `adb uninstall com.example.wickedsalemwitchcitytour` returned `DELETE_FAILED_INTERNAL_ERROR` — exposed both files referenced the old package name while the real `applicationId` is `com.destructiveaigurus.katrinasmysticvisitorsguide` (S245 rebrand). Fixed: new "Naming" 5-row disambiguation block in CLAUDE.md (brand "Katrina's Mystic Visitors Guide - Salem" / applicationId / Kotlin namespace `com.example.wickedsalemwitchcitytour` (intentionally not renamed) / Gradle module `:app-salem` / repo slug), Room v20→v21 + hash refresh, 9→25 `salem_*` tables, IP.md "14 patentable innovations" cleanup, day-count refresh; CLAUDE.md 246→263, STATE.md still under 200. (2) **TTS master mute toggle** in toolbar speaker long-press popup ABOVE "What gets narrated" — operator: *"turn on and off sound completely from there"* + greyed when no TTS path ("like on bluetooth") + option (b): preserve user's checked-state when disabled so narration resumes once routing recovers. 4 files +169 LOC: `AudioControl.kt` (`PREF_TTS_MASTER` + `engineReady`/`routeUsable` split signals + `isTtsEffectivelyOn()`), `NarrationManager.kt` (push ttsReady on init/fail/shutdown + master gate at top of `enqueue()` above the `userInitiated` exemption), `SalemMainActivity.kt` (`AudioDeviceCallback` watches output devices, scans `GET_DEVICES_OUTPUTS` against TTS-friendly type allowlist BUILTIN/WIRED/A2DP/BLE/USB/DOCK/HDMI, pushes to `AudioControl.setRouteUsable`; unregistered in `onDestroy`), `AppBarMenuManager.kt` ("Sound" section above "What gets narrated" with disabled+greyed+subtitle in unavailable path). Built clean, installed clean on Lenovo + Pixel 8 (uninstall first per WAL-safety rule); Pixel 8 logcat confirmed `TTS route refresh — outputs=[1, 2, 18, 24] usable=true` + `schema_v=21 tables=13` + `engineReady → true` + `ttsReady ← true`. Field smoke (popup visibility, master-off silence verification, BT SCO grey-out on real hardware) owed to operator. (3) **CI unblock** — CI had been red for 10 consecutive pushes back to S263 (2026-05-13). Root cause structural: `verify-bundled-assets.js` (Gradle preBuild) hard-checked four large gitignored asset paths (`salem_tiles.sqlite` 207MB / `poi-icons/` 544MB / `hero/` / `heroes/`) absent on GHA runners. Fix: detect CI via `process.env.CI`, demote those four checks to `[CI-skipped]` warnings, keep all Room schema / row-count / identity-hash / commercial-hero leak checks as hard failures. Local runs (CI unset) still hard-check every path. Unblocked the next-in-line failure (`lintRelease` with 3 `UseAppTint` errors on `sheet_passport.xml:39,72,84`); migrated `android:tint` → `app:tint`. **Final CI on `90af4e3`: SUCCESS** — first green in 11 pushes. Four commits: `9557976` doc lint, `ffebf5a` verifier CI mode, `cf13997` TTS master mute, `90af4e3` lint fix. 74 days to 2026-08-01 ship; 4 days to Form TX deadline 2026-05-20. Full detail in `docs/session-logs/session-272-2026-05-16.md`.
...
**Sessions completed:** 274. **Internal ship target: 2026-08-01.**
