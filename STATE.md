...
**Last updated:** 2026-05-29 — **Session 305.** Tech-debt **Phase 1 banked at the JobCoordinator infra level** (`54b8a66`, tag `cleanup-phase1-done`): applied the S304 review finding (`JobCoordinator.jobs` → `ConcurrentHashMap` + atomic self-evict) and migrated the remaining **clean** active clusters — `cachePoi`, `autoDump`, `narrationHighlightAnim` — all previously leak-exposed, now under the single `cancelAll()`. **Deferred (documented):** `walkSim` (needs a `cancelAndJoin(name)` extension — S125 race), the `populate` recon cluster (→ P3), and **1b MarkerController** (operator decision: redistributed to P3 parked-marker collapse + P4 retail-overlay decomp; `narrationMarkerJob` must NOT migrate — S234 anti-double-attach mutex). Plan-of-record updated. On-device Pixel 8 smoke: **Phase 1 paths healthy** (walk-sim ran, narration played, no FATAL), but **3D tilt drove a memory blowout** (`lowmemorykiller` culling other apps, 59 MB LOS bitmap churn, 66–96 skipped frames; tilt extends MapView to `viewport=1798x12600` at z20 with 622 overlays). **Ships in retail** (`TILT_3D_ENABLED=true`) and operator confirms it **worked before → REGRESSION, root-cause + fix next session.** Full S305 detail: `docs/session-logs/session-305-2026-05-29.md`.
...
### S306 opener (parked at S305 close)

_**TOP NEW — 3D-tilt memory blowout (REGRESSION, ships in retail).** Operator confirms 3D tilt "was working before"; now tilt + walk-sim + 622 overlays at z20 spirals into OOM (`lowmemorykiller` culls other apps, 59 MB LOS bitmap churn, 66–96 `Choreographer` skipped frames; no FATAL — it's memory/render, not a code crash). Cause: `TiltContainer` extends the MapView to `viewport=1798x12600` (`targetTop`→~10,500px at 36°; tilt ladder 0/30/36/42/48 in `setupTilt3dButton` SalemMainActivity:1902). **Ships in retail** (`BuildDefaults.TILT_3D_ENABLED=true` in all builds — the `MenuPrefs.kt:141` `=BuildConfig.DEBUG` comment is STALE), so pre-Aug-1-worthy, esp. on min-spec Lenovo (would OOM-kill, not just lag). Root-cause the regression (candidates: overlay-count growth — 622 is high; S299/S301 graphics-redo bitmap LOS pressure; or a TiltContainer extension-math change) then fix = cap `targetTop` viewport extension (+ optionally suppress ambient animations / lower max-tilt / downsample tiles under memory load). Its own commit, separate from the tech-debt phases. NOT caused by S305 Phase 1 (job tracking doesn't touch tilt/tiles/overlays/render)._

_**Resume the tech-debt plan** (`docs/plans/tech-debt-cleanup-2026-05.md`; **P0 + P1 DONE**, tags `cleanup-phase0-done`=`14c2481` / `cleanup-phase1-done`=`54b8a66`; rollback `cleanup-phase0-pre`=S303 state). Next: **P2** (dead `hero/`/`custom_icon_asset` retirement — coordinates with art-bible §7) or **P3** (isolate parked network features — now ALSO absorbs the parked-marker clear/add collapse [transit/aircraft/metar] + the recon populate/idle/probe/silentFill cluster). Then P4 (VM decomp — absorbs the retail-overlay/MarkerController work) / P5 (hardening) / P6 (web). `walkSim` JobCoordinator migration, if wanted, needs a `suspend fun cancelAndJoin(name)` extension first (S125 stale-location race). Operator wants **more code review** as we go. Full background-cycle (onPause→onResume) walk-sim smoke was NOT completed before the tilt interruption — confirm no leaked loops next drive. (Also: the S303 GPS-OBS heartbeat fix is smoke-clean across S304+S305 but a real-world drive remains the gold-standard close — nice-to-have.)_
