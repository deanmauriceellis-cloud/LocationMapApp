...
**Last updated:** 2026-05-23 — **Session 290.** Detour from the 1692 docket: shipped a **"Historian" restricted admin role** for the web portal (operator's wife authoring POI content). The portal had a single shared Basic Auth credential and no roles; added an optional second credential (`ADMIN_HISTORIAN_USER/PASS` in `cache-proxy/.env`) mapped to a `historian` role that can edit ONLY a POI's narrations, descriptions, category/subcategory, and flags. **Backend is the authoritative boundary** — `admin-auth.js` resolves either credential to `req.adminRole` + new `requireFullAdmin` guard + `filterHistorianViolations`; `server.js` adds `GET /admin/whoami` and mounts `requireFullAdmin` on every off-limits `/admin` prefix; `admin-pois.js` scopes the historian `PUT` to a field whitelist (403 otherwise) and locks create/delete/move/geocode/location to full admin; mass-edit + category writes locked too. **Frontend** gates UX only — `AdminLayout` fetches whoami → hides non-POI tabs + create/publish/photo buttons, renders a new compact `HistorianPoiDialog` (not the 2600-line `PoiEditDialog`), and makes map markers non-draggable for a historian. **Attribution**: POI `PUT` now runs in a txn that sets the `salem_audit_log` trigger's `app.actor`/`app.source` — historian → `actor='Historian'`, admin → `actor='admin'` (both previously null); historian narration/description edits also stamp the POI's `data_source` to `'Historian'` (the "how authored" annotation; `data_source` stays out of the historian whitelist so it can't be spoofed). `node:test` admin-auth suite 9/9 + `npm test`. Six commits `c6cf932..cfd5fdc`. Live + verified on the cache-proxy (running as a background process); LAN access at **http://10.0.0.229:4302/admin** (historian login pass set by operator). **Zero Android/Room/publish-chain/app impact.** Web/cache-proxy only. **The 1692 alignment + per-map zoom-envelope + 1700/1851/1911 precision-pass work from the prior opener was NOT touched — see S291 opener below.**
...
### S291 opener (parked at S290 close)

**S290 was a Historian-role detour (web portal) — the 1692 docket below is unchanged and resumes.** New carry-forward from S290: operator still owes the **historian browser visual smoke** — log in at http://10.0.0.229:4302/admin with the historian credential and confirm only the POIs tab shows, the compact `HistorianPoiDialog` opens (no Location/Provenance/Danger tabs), markers are non-draggable, and a save attributes `actor='Historian'` + `data_source='Historian'`. Existing pre-S290 audit rows keep their null actor (only new edits are instrumented).
...
**Restore-on-restart — landed on devices in S289 rebuilds.** `SalemMainActivity.onCreate` now reads `SharedPreferences("historical_maps")."year"` after mapView layout and calls `applyHistoricalOverlaySelection(savedYear)` if non-null (commit `1660ce0` from S288 post-close was finally installed via S289's many AAB rebuilds).
