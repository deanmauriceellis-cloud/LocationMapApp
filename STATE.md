...
1. **Animations visible in the running Salem app — non-negotiable S172 deliverable.** Operator stated explicitly at S171 close: "make sure the next session, the animation works in the app." Recommended path: ship a thin `WickedAnimationOverlay : org.osmdroid.views.overlay.Overlay` subclass that runs our `AnimatedWaterOverlay` + `FireflyOverlay` logic, projecting geo→screen via osmdroid's `Projection.toPixels(GeoPoint)`. Add it to `SalemMainActivity.binding.mapView.overlays` at startup after `PolygonLibrary.load()`. This is ~30–60 min of focused work, breaks zero tour features, and gives the operator visible whitecaps + cemetery orbs in the LIVE Salem map next session. Important caveat to flag at session open: the operator stated mid-S171 "I do not want Osmdroid doing our tile work" — the overlay approach leaves osmdroid rendering tiles. Reconfirm at S172 open whether they want this interim path (animations live now, full migration deferred) or push directly to the migration (animations live only after the full swap). Most likely answer: ship the overlay first, do migration after.

2. **Plan + start the osmdroid → WickedMapView migration in `SalemMainActivity`.** Load-bearing follow-up. Open the session with a real Plan after #1 ships: enumerate the 338 osmdroid call sites, group them by feature (POI markers / geofences / walker pin / radar / aircraft / METAR / transit / location ball / walking directions / camera ops / lifecycle), order by user-visible priority (POIs first per operator's "GIS/map overlay tiles and POIs" framing), commit to one stage per session. Stage 1 = swap the layout to `WickedMapView` + replace the simple camera/lifecycle calls (`zoomLevelDouble`, `boundingBox`, `controller.animateTo`, `onResume/onPause`, `setTileSource` no-op, `mapCenter`) with WickedMap equivalents on **our** types (no osmdroid imports in `wickedmap/`). Stage 2 = `MarkerOverlay` for POIs. Per-stage rebuild + Lenovo field-test before moving on. Operator accepts temporary regressions in low-priority features (radar/aircraft/METAR/transit) during the migration; tour features (POIs, geofences, walker, location ball) must come back online stage-by-stage.
