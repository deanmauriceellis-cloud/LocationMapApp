...
**Last updated:** 2026-05-24 — **Session 295.** **OMEN-025 anti-piracy Phase 1, Session 3** (operator: app cloning/copying protection is the critical pre-V1 issue, "build this in now"). Shipped the runtime half of Layer 2: `core/.../activation/ContentManifestVerifier.kt` — the on-device counterpart to S293's `sign-content-manifest.js`. Per launch: verifies the detached RSA-2048/SHA-256 `.sig` over the verbatim `content-manifest.json` against the embedded pubkey (`res/raw/content_manifest_pubkey.der`), recomputes the canonical `manifestHash` (self-consistency), and re-hashes `salem_content.db` + in-scope assets from disk (tiered D4 — bulk assets presence/size at activation only; 354 MB tile DB Phase-2-deferred). JVM-testable by construction (injected pubkey + opener + log; no Robolectric); `forAndroidAssets(context, R.raw.…)` = production wiring (`:core` stays decoupled from `:app-salem` R). **9 unit tests, 0 failures** incl. the load-bearing cross-check that the Kotlin canonical hash byte-matches the real build value `978937…` (= the value baked into the Worker's `EXPECTED_MANIFEST_HASHES`). `:core:compileDebugKotlin` + `:core:testDebugUnitTest` green. **Zero Room/PG/publish-chain/app-runtime impact** — `:core` Kotlin + test only. **Next = Session 4** (activation Worker — code unblocked; deploy gated on Cloudflare token + Google SA JSON).

### Prior — S294 (2026-05-24) — pruned to one line at S295 close

Unplanned UX session. Diagnosed Starbird's missing Directions button to stripped-mode commercial POIs (`merchant_tier=0`) hiding the "Things You Can Do" container; per operator ("every POI should have directions") shipped a **universal dedicated Directions button on every POI sheet** (both render paths; `host.walkTo`), removed the duplicate synthesized action, recolored the directions route green→**yellow** (`#FFD60A`). Operator-confirmed on Lenovo. UI/Kotlin only. Commit `3132adb`. Detail in `docs/session-logs/session-294-2026-05-24.md`. (S293: OMEN-025 Phase 1 kickoff — OMEN-reviewed 8-session plan `docs/plans/OMEN-025-anti-piracy-phase1.md` + Sessions 1–2 shipped: Layer-2 signed manifest + `core` activation foundations `DeviceKeyManager`/`ActivationReceipt`/`ActivationStore`; cert pinning DROPPED, Worker LMA-owned on `*.workers.dev`.)
...
### S296 opener (parked at S295 close)

**Carries:** (a) **S3 on-device benchmark** — confirm `verifyCheap` sub-second on Pixel 8 + Lenovo (instrumented tier; fold into S6 wiring alongside S2's deferred on-device `DeviceKeyManager` round-trip); (b) git-maintenance request open with OMEN (`~/Development/OMEN/reports/locationmapapp/S294-git-maintenance-request-2026-05-24.md`); (c) before any release bake re-run `sign-content-manifest.js` AFTER align (S293 sig stale vs a fresh build's align rewrite); (d) optional Directions button relabel.

**TOP: resume OMEN-025 Phase 1 at Session 4.** Follow the stepped plan `docs/plans/OMEN-025-anti-piracy-phase1.md`. Sessions 1–3 done (S1–S2 build signer + `core` foundations S293; **S3 `ContentManifestVerifier` + 9 JVM tests S295**). **Session 4 (NEXT, code unblocked / deploy gated):** activation Worker in the LMA repo (`worker-activate/`, `*.workers.dev`) — (1) write the endpoint contract (`/nonce`, `/activate`, `EXPECTED_MANIFEST_HASHES`) → hand to OMEN as the NOTE-L023 #2 response; (2) `wrangler.toml` + `src/index.js` router; (3) `src/nonce.js` HMAC; (4) `src/integrity.js` (SA-JWT mint via Web Crypto → `decodeIntegrityToken`); (5) retain-nothing + native rate-limit; (6) `wrangler dev` tests with a sample token. ⏸ **Deploy** needs operator Cloudflare API token + Google service-account JSON. Then S5 (Android network path — waits on S4 deploy), S6–S7 (⏸ Play Console: Play Integrity + gate UI + INTERNET re-scope + rollout), S8 (disclosures + cred inventory).

**OWED to OMEN (per-item NOTE-L023 in next report):** confirm `~/keys/content-manifest.pem` pathed (cred inventory #5); confirm `*.workers.dev` decision so OMEN withdraws NOTE-DA005; ack #1 (pin dropped) / #3 (unit tests now started — 9 verifier tests S295) / #4 (residual recorded).
...
**Sessions completed:** 295. **Internal ship target: 2026-08-01.**
