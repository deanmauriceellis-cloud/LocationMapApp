...
**Last updated:** 2026-05-20 — **Session 284.** Cleared CI red-since-S275 (10 push failures back-to-back) with a single XML refactor. Root cause: `app-salem/src/main/res/layout/sheet_collection.xml:162-163` — `tabIndicator` View constrained to `@id/tabList` / `@id/tabBadges`, but those lived inside the `collectionTabs` LinearLayout (S275 Katrina's Collection tabbed UI), grandchildren of the outer ConstraintLayout, not direct siblings. ConstraintLayout only allows constraint refs between siblings → 2 `NotSibling` lint errors → `lintRelease` task failed every push since S275. Same invalid constraint at runtime in `CollectionSheet.kt:472` (`startToStart = anchor.id`) meant the "sliding amber underline under active tab" effect had been silently broken on device too — indicator just sat at parent default. Operator picked proper refactor over baseline-suppression. Flattened the tab bar: dropped the LinearLayout wrapper, made `tabList` + `tabBadges` direct children of the outer ConstraintLayout split 50/50 by a vertical Guideline, background `#16151B` moved onto each tab with `android:foreground="?attr/selectableItemBackground"` for the click ripple. `tabIndicator` now constrains to real siblings — fixes lint AND lets the runtime `showTab()` code actually move the underline on tab switch (first time since S275 once next install lands). Verified locally: `lintRelease` 0 errors (was 2) / `routing-jvm:test` pass. Commit `c8e7657`; CI run `26136388027` came back green — first successful build since S274 close (2026-05-17). Non-blocking annotation: Node 20 → Node 24 GitHub Actions deprecation lands **2026-06-02** (~13 days) — cheap one-line workflow bump when in scope. Net code-side change: one XML file. No PG / Room / cache-proxy / publish chain / device installs. S284's XML fix is NOT on the field devices yet (Pixel 8 + Lenovo still carry S282 build) — cosmetic only, not blocking the next walk-sim. Briefly explored a "collector as master agent" architecture for the burst-photo pipeline (devices as sleep placements; gather/filter/persist) — operator pivoted off and asked to drop that branch + the slave-2 tangent + the local-out-of-network framing from session scope. Existing S270/S271 recon carry-forwards remain the authoritative pointer if collector work resumes later.

### Prior — S283 (2026-05-19)

Chatbot V1 scoped end-to-end (on-device, rule-based, POI facts only, distance-sorted). Operator declared scope: *"Where is the Phillips House?"* / *"Where can I find pizza?"* / *"What are pizza places close to me?"*. Recommended SQLite FTS5 over hand-rolled inverted index. 7 design decisions locked across 3 AskUserQuestion rounds: dedicated full-screen Find screen via drawer menu / FTS5 + Levenshtein-name fuzzy fallback / locations + short blurb inline AND direct-answer for high-confidence name queries / all 2,039 active POIs indexed (name + category + subcategory + description + short_description) / GPS-unavailable falls back to alphabetical sort / synonyms deferred to post-v1 / drawer entry near top. Wrote `docs/plans/Chatbot_V1.md` (~150 lines) with locked scope + 7 open items + 6 implementation phases + 5 risks + explicit out-of-scope list. **Phase 0 recall spike** is the recommended first action when implementation kicks off. Plan parked for operator pondering. Net code-side change: zero.
...
### Older prior-session blocks pruned 2026-05-20 (S284) for the 200-line cap; see `SESSION-LOG.md` for S275–S282 + `docs/session-logs/` for full detail.
...
### S285 opener (parked at S284 close)

**CI is green again as of S284 close** (run `26136388027`). One non-blocking CI annotation: GitHub deprecating Node 20 actions starting **2026-06-02** (~13 days). `actions/checkout@v4`, `actions/cache@v4`, `actions/setup-java@v4`, `actions/setup-node@v4` will be forced to Node 24. Cheap one-line workflow bump (`.github/workflows/ci.yml`) when in scope.

**Chatbot V1 plan still parked for operator pondering.** `docs/plans/Chatbot_V1.md` is the authoritative scope doc — all 13 locked decisions + 7 open items + 6-phase implementation roadmap live there. **Phase 0 recall spike** is the recommended first action when operator gives the go-ahead: 5-min PG query over ~15 tourist-staple keywords (pizza, coffee, beer, ATM, restroom, parking, ice cream, bathroom, sushi, witch trials, ...) against `name + category + subcategory + description + short_description` columns to validate the synonyms-deferral assumption. The 7 open items include the Hawthorne disambiguation edge case (uniqueness rule for direct-answer triggering), FTS bake hook placement (fold into `align-asset-schema-to-room.js` vs. new standalone script), and a few smaller calls. Wait for operator to finish pondering before kickoff.

**S284 device-state note:** S284's `sheet_collection.xml` fix is NOT on the field devices (Pixel 8 + Lenovo still carry the S282 build — 14 Mall narrated-flag fix on S281 architecture). The S284 change is cosmetic only (sliding amber tab indicator under active tab in CollectionSheet); doesn't touch narration / GPS / queue / publish chain. Lands automatically next time any debug AAB rebuilds. Not blocking the next walk-sim.
