...
## TOP PRIORITY — Next Session (S194)
...
0a. **(S193 carry-forward) Three re-runnable historical_narration gaps.** Of the 8 pre-1860 HISTORICAL_BUILDINGS still missing `historical_narration`, 5 are commemorative artifacts (statue / arch / memorial / plaque / "First Muster" marker) correctly skipped — the stored year is the year of the *subject*, not of the artifact. The other 3 are genuine pre-1860 sites the S192 generator rejected on grounding thresholds: `the_merchant` (1700), `central_wharf` (1790), `scarlet_letter_house` / 14 Mall Street (1847). Targeted re-run: `node cache-proxy/scripts/generate-historical-narrations.js --ids=the_merchant,central_wharf,scarlet_letter_house` then full publish chain.

0b. **(S193 carry-forward) Mark a tour `is_historical_tour=true` to activate the new runtime path.** The end-to-end wiring is dormant until a tour is flagged — admin tool → tour metadata → "Historical tour" checkbox → republish. Best candidate is a polyline-only walking route through pre-1860 buildings (Witch House → Gedney → Pickering → Peirce-Nichols → Joshua Ward, etc.).

0c. **(S192 carry-forward, supersedes prior 0a) Backfill `year_established` on the 479 buildings the generator skipped because effective_year was null or ≥ 1860.** Many are genuine pre-1860 sites whose LMA `year_established` is just blank. Pull from SalemIntelligence `mode_c_historic_building_details.built_year` + MHC inventory. Then re-run generator. This is where coverage meaningfully expands.
...
**Sessions completed:** 193. Salem 400+ target 2026-09-01 aspirational.
...
- **Room DB:** 9.1 MB at `app-salem/src/main/assets/salem_content.db`, **v13 schema** (S193), identity_hash `d0d3d240e2d04c159767b90860fe7a8c`. Witch Trials intact (npc_bios 49 / articles 16 / newspapers 202). v11 (S186) added `is_civic_poi` to SalemPoi. v12 (S192) added `historical_narration` to SalemPoi (70 populated). v13 (S193) added `is_historical_tour` to Tour. `publish-tour-legs.js` no longer hardcodes the hash — reads the latest schema JSON.
