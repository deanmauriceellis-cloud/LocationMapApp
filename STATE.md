...
**Last updated:** 2026-04-29 — Session 199 closed. Three big shipped end-to-end. (1) **Salem Witch Trials content rewritten under attorney-driven word caps** — Salem Oracle newspapers ≤250 words (avg 213, was 466), Witch Trial History tiles ≤150 (avg 166, was 369–419), People-of-Salem bios ≤150 (avg 135, was 2486 with the long fictional `narrative.*` concatenation). All three Salem Oracle generators (`Salem/cmd/{gennewspaper,gentiles,genbiographies}/`) got tightened prompts with explicit hard word budgets, dropped the duplicative "first POINTS entry repeats the lede" rule, and a new `normalizeASCII()` helper (curly quotes / em+en dashes / ellipses / NBSP → ASCII at the source). 267 generations (16 tiles + 202 newspapers + 49 Tier-1+2 bios) on the RTX 3090 (`salem-village:latest` gemma3:27b), 0 failures, 45 min wall time. Final asset-DB word counts: newspapers avg 213 (26 over by max 32), tiles avg 166 (14 over by max 36), bios avg 135 (3 over by max 3) — operator approved few-words-over band. (2) **Import pipeline restructured.** `import-witch-trials-npc-bios.js` switched from concatenating 7 fictional `narrative.*` sections out of `data/json/npcs/` to using the tight `tts_full_text` from `data/json/biographies/` cross-referenced with `npcs/` only for metadata (role, faction, age_in_1692, historical_outcome). New JSON-direct importers `import-witch-trials-newspapers-from-json.js` + `import-witch-trials-tiles-from-json.js` replace the SalemIntelligence (`:8089`) middleman; source of truth is now Salem Oracle JSONs on disk. Incidental `publish-witch-trials-to-sqlite.js` Date-→-ISO `s()` wrapper extended to newspaper + article inserts (was throwing `SQLite3 can only bind…` on PG `timestamptz` columns). Newspaper PK collision (old `1691-11-01` vs new `salem_oracle_1691-11-01`) cleaned in PG + both SQLite DBs. (3) **`is_civic_poi` drift across 179 POIs fixed** — operator field-discovered M&M Contractors narrating during a tour walk despite category=SERVICES; root cause `is_civic_poi=true` was never auto-cleared on category change. Audit: 179 drifted (PARKS_REC 67 + SERVICES 50 + EDUCATION 36 + WORSHIP 19 + 7 misc) — same lint area as S196 carry-forward 0d `civic_flag_mismatch`. One-shot SQL `UPDATE salem_pois SET is_civic_poi = false WHERE category != 'CIVIC' AND is_civic_poi = true AND deleted_at IS NULL` → 0 drift. Drift prevention wired in BOTH backend (`cache-proxy/lib/admin-pois.js` `buildUpdateClause()` auto-syncs flag on category change unless body explicitly overrides) AND frontend (`web/src/admin/PoiEditDialog.tsx` `useEffect` toggles checkbox on category dropdown change). Bidirectional, CIVIC→true / anything-else→false. Full publish chain re-ran end-to-end (publish-salem-pois → publish-tours → publish-tour-legs → publish-witch-trials-to-sqlite → bundle-{newspapers,npc-bios,articles}-into-db → cleanup-stale-newspaper-ids → align-asset-schema-to-room); APK on Lenovo HNY0CY0W. Operator session-end: "testing, session end and we continue to refine." S196 carry-forward 0d (`civic_flag_mismatch` lint relaxation) is now resolved by the auto-sync — lint can flip back to strict next session. Full detail in `docs/session-logs/session-199-2026-04-29.md`.
...
## TOP PRIORITY — Next Session (S200)
...
0a. **(S196 NEW) Multi-admin auth + actor-from-session wiring.** Add `salem_admin_users` table (username, hashed password, role, created_at). Update Basic Auth to verify against env user OR table. Wire admin write endpoints + scripts (regen, publish chain, ingest) to `SET LOCAL "app.actor" = req.user` and `"app.source" = '<endpoint name>'` so the audit log gets attribution prospectively. Operator: "I will create additional admin accounts so we can audit who changes what."
...
0c. **(S196 NEW) Hand-author the 199 rejected narrations** (lint: `narration_regen_failed`). Operator-side. Many are post-1860 hb_* houses where the SI dump has too few pre-1860 facts; some are genuinely thin grounding that needs human research. Operator should walk the lint tab, suppress what's accepted-silent, hand-author the rest.

0d. **(RESOLVED S199)** `civic_flag_mismatch` lint can now flip back to STRICT — any non-CIVIC with `is_civic_poi=true` is suspect. S199 fixed the 179 drifted POIs and wired backend + frontend auto-sync so the drift can't recur. One-line change in `cache-proxy/lib/admin-lint.js` next session.

0f. **(S199 NEW)** Eyes-on the new shorter Witch Trials content during a real walk on the Lenovo. Confirm the 250/150/150 word caps feel right when narrated — newspaper cadence, tile bridge sentences, bio rhythm. Spot-check a People-of-Salem entry for narrative completeness at the new shorter length. Operator session-end: "testing, session end and we continue to refine."

0g. **(S199 NEW carry-forward)** Newspaper PK normalization — old `id = date` rows were a legacy from before `cmd/gennewspaper` adopted `id = "salem_oracle_" + date`. `bundle-witch-trials-newspapers-into-db.js` does INSERT OR REPLACE; if any future PG load brings back date-only ids the bundle will accumulate dupes. Pin a constraint OR add a delete-stale-ids step at top of bundle script.

0e. **(S192 carry-forward) Backfill `year_established` on the 500+ HIST_BLDG rows missing it.** Pull from SI + MHC. Reduces the lint cap-hit so other content gaps surface.

0f. **(S195 NEW, still open) Driving mode considered explicitly.** S195 drive-test surfaced walking-tuned geofences (40m + serial TTS) are brittle at 10 m/s. Operator confirmed S196 "this is a walking app" — V1 stance is a walking-only disclaimer. Driving mode is V2.
