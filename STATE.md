...
### S215 next-session actions (added by S213/S214; older items below remain)
...
- **S214 edge-point Dijkstra** (Phase 2 of waypoint-binding) — today's snap puts each free waypoint on a walkable edge but `routeBetween` still pulls each waypoint to the nearer edge endpoint at compute time, so legs visit corners not mid-block points. Add a 2-pseudo-source Dijkstra that splits the bound edge virtually so legs actually thread through `(edge_id, fraction)`.
- **S214 visual edge-highlight tick** (low-priority polish) — render a thin fuchsia tick on the bound edge in `TourStopLayer` so the edge binding is visible without opening MoveConfirm.
