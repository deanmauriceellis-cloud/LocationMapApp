...
**Last updated:** 2026-05-06 — **Session 227 (Haunt refinement — duration knob + screen-aligned upright fix).** Seven-iteration polish of S226's per-POI Haunt. Schema Room v18 → v19 (identity `745afa3eb4ce04bd7873671ea297b6e0`) adds one column: `haunt_duration_s` REAL (NULL → SpriteOverlay default 1.0s). Admin Haunt tab gains a **Dance duration (min)** input — wire stays seconds, `buildDefaults` ÷60 on display + `buildPayload` ×60 on save. SpriteOverlay rewritten: alpha decoupled from scale (flat-top trapezoid 15/70/15 fade — the original bell-curve made a 2-3s dance feel like 300ms because scale was multiplied by alpha bell so only the middle "big+bright" bobs registered), pseudo-3D depth bob with randomized amplitude / cycles / phase per fire, frame-11-pinned upright facing per S170 demo calibration, one elliptical orbit per dance (random radii 70-140 × 50-100 px, random clockwise/counter-clockwise direction), gentle scale clamp 0.7-1.4. S170's 7 animated WebPs split into 112 per-frame WebPs (`assets/sprites/<id>/00.webp`...`15.webp`, ~2.2 MB total). **The breakthrough fix at session close**: when the operator reported "skeleton pointing 150° from north/up", a per-sprite rotation hack was added then immediately deleted once `WickedAnimationOverlay.kt:22-27` docstring revealed the real cause — osmdroid pre-rotates the canvas by `mapView.mapOrientation` in heading-up tour mode, polygons (water/grass/fireflies) are rotationally symmetric so they look fine, but sprites with a clear up-axis inherit the rotation. The systemic fix: pass `mapView.mapOrientation` into a new `CameraState.mapOrientationDeg` field, then in `SpriteOverlay.draw` `canvas.save() / rotate(-mapOrientationDeg, cx, cy) / drawBitmap / restore()`. Sprite stays anchored to the POI but always renders upright on screen regardless of map heading. Operator close: "Much better." Field-walk validation of the upright screen-aligned skeleton on Lenovo near the bridge POI is owed. Bridge skeleton currently `haunt_duration_s = 3.0` (saved before unit conversion deployed → 3 seconds, not 3 minutes — operator may want to re-edit in admin to exercise long-duration path). **Session 226 (prior — per-POI Haunt Effect S170 sprites wired into running app, Room v18, Haunt admin tab, marker-loader DAO loosened).** **Session 225 (prior — web admin can create POIs via + New POI flow, S225 PoiCreateDialog).** **Sessions 217-224 (prior — category-aware FAB narration + Show-All FAB override; per-POI TigerLine/Google Validate workflow; storytelling-with-subtopics Room v17; tour detour feature; cache-proxy dotenv autoload; web-admin restart; lean+state protocol amendment; walk-sim queue stall fix + Find/PoiDetailSheet V1-dead code purge).** **Operator-stated hard pause from S214 still in effect: Salem Oracle (:8088) + SalemIntelligence (:8089) are undergoing major reformulation to attach full per-claim provenance; without source attribution V1 content cannot ship legally — do not propose work consuming them or backfilling content from them until the operator signals the pause is lifted (memory `project_oracle_intelligence_paused_provenance.md`).** **Cross-repo Salem commit owed (still).** Per-session detail in `docs/session-logs/session-{217..227}-*.md` and `SESSION-LOG.md`.

> Older "Previous" entries (S180 / S183-S189 trimmed S201; S195 trimmed S202; S196-S199 trimmed S204; S213-S216 prose trimmed S224; S217-S224 prose collapsed S225; S225 prose collapsed S226; S226 prose collapsed S227) — moved out to keep STATE.md under the 200-line cap. Detail lives in `SESSION-LOG.md` + `SESSION-LOG-ARCHIVE.md` + per-session live logs at `docs/session-logs/session-NNN-*.md`.
...
Seven-iteration polish of S226's per-POI Haunt. Net-new schema: Room v18 → v19 (identity `745afa3eb4ce04bd7873671ea297b6e0`) with one column `haunt_duration_s` REAL (NULL → 1.0s default). Admin form **Dance duration (min)** — wire stays seconds, conversion in `buildDefaults` (÷60) and `buildPayload` (×60). SpriteOverlay rewritten: alpha decoupled from scale, pseudo-3D depth bob with randomized amp/cycles/phase per fire, frame-11-pinned upright facing per S170 demo calibration, elliptical orbit motion (random radii 70-140 × 50-100 px, random direction), gentle scale clamp 0.7-1.4. S170's 7 animated WebPs split into 112 per-frame WebPs (~2.2 MB total). **Systemic fix found in `WickedAnimationOverlay.kt:22-27` docstring**: osmdroid pre-rotates the canvas by `mapView.mapOrientation` in heading-up tour mode → polygons unaffected, sprites inherit rotation. New `CameraState.mapOrientationDeg` field; `SpriteOverlay.draw` does `canvas.rotate(-mapOrientationDeg, cx, cy)` per sprite to render upright on screen regardless of map heading. Operator close: "Much better." Bridge skeleton currently 3.0s (saved before unit conversion); operator may want to bump to 180.0s in admin (= 3 min) to exercise long-duration path. Field-walk validation of the upright screen-aligned sprite owed.

### S226 outcome — Per-POI Haunt Effect shipped (2026-05-05)

S170's 7 sprite figures wired into the running app as an admin-driven, per-POI haunt: pick a sprite from the **Haunt** tab in the POI editor, set outer/inner range + intervals (defaults 200 m / 60 s outer, 50 m / 15 s inner), and a 1-second sprite peek fires near the POI on the cadence band when the user comes within range. Schema Room v17 → v18 (identity `e6c5a128efdd9949f7297c88ada7d698`) — 6 columns added to `salem_pois`. Runtime via new `SpriteOverlay.kt` (~190 LOC) inside the existing `WickedAnimationOverlay` bridge. Marker-loader DAO loosened so a haunt-configured POI is visible on the map even without narration. Admin opt-in only; default blank for every POI. Smoke-seeded Witch House (witch) + Beverly/Salem City Line (skeleton); marker count went 1303 → 1304.
...
### S228+ next-session docket — Haunt field-walk + further polish + Find menu refinement (deferred from S224)

**Haunt feature carry-forwards:**
- Operator field-walk validation of the upright screen-aligned skeleton on Lenovo near the bridge POI (S227 owed).
- Operator may bump bridge `haunt_duration_s` from 3.0s → 180.0s via the new admin minute field to exercise the long-duration path; if so, re-bake + redeploy.
- Optional audio component (whoosh / cackle / hoot / skitter) — would add a `haunt_audio_id` column.
- Per-character motion profiles (cat ground-hugging, witch overhead arc, owl perch).
- Operator may also want haunts that fire ONLY in tour mode or ONLY in explore mode (currently both).
...
**Sessions completed:** 227. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
