...
**Last updated:** 2026-04-26 — Session 185 closed. **S183/S184 walking-route work is now in the AAB on the Lenovo.** PG operator-curated tour (`tour_WD1` + 14 legs) flows PG → publish-tours.js + publish-tour-legs.js → asset `salem_content.db` → app via new Room `TourLeg` entity (v9 → v10). `TourViewModel.computeTourPolyline()` reads baked legs and assembles them with the same anchor + clip rendering policy as the admin's `TourLegsLayer.drawLeg`. Walk-sim now consumes the baked legs path (was falling back to "Downtown Salem" because tour_WD1 has no JSON in `assets/tours/` and no POI-bound stops). Historical Mode is auto-skipped for stop-less tours so POIs aren't silenced by an empty whitelist. New tour model is **polyline-only** — tours are just paths users follow, POIs govern their own narration via geofence proximity. The 5 historical Kotlin tours (Essentials/Explorer/Grand/Witch Trials/Heritage Trail) are dropped from the asset; operator will re-author them in PG via the admin tool when wanted. Two latent landmines flushed during the session: (1) asset DB tables had `DEFAULT` clauses from `salem-content/create_db.sql` while Room's codegen schema has none → `TableInfo.equals` mismatch → fallback destructive on every install; (2) asset's `PRAGMA user_version` was 9 while Room expected 10 → upgrade migration ran → fallback destructive. New `cache-proxy/scripts/align-asset-schema-to-room.js` rewrites every Room-managed table using the canonical `createSql` from `app-salem/schemas/<DB>/<v>.json` and stamps both identity_hash + user_version. Admin: POI markers at zoom ≥17 now show humanized category above + name below (mirrors Android `MarkerIconHelper.labeledDot`); cluster threshold lowered to 17 to match. Files: `app-salem/src/main/java/.../{content/model/TourLeg.kt (NEW), content/dao/TourLegDao.kt (NEW), content/db/SalemContentDatabase.kt, content/di/SalemModule.kt, content/SalemContentRepository.kt, ui/TourViewModel.kt, ui/SalemMainActivity.kt, tour/TourEngine.kt}`, `app-salem/build.gradle`, `app-salem/schemas/...10.json (NEW)`, `app-salem/src/main/assets/salem_content.db`, `cache-proxy/scripts/{publish-tour-legs.js (NEW), align-asset-schema-to-room.js (NEW), publish-tours.js, verify-bundled-assets.js}`, `web/src/admin/AdminMap.tsx`, `web/src/index.css`. Full detail in `docs/session-logs/session-185-2026-04-26.md`.
...
## TOP PRIORITY — Next Session (S186)

1. **Continue refining the Android app + web admin tool.** Operator's stated direction at S185 close. No specific blockers — pick from #2-7 below or follow operator direction.
