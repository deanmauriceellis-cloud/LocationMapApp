...
**Last updated:** 2026-05-01 — **Session 216 (consolidation: walk-sim onboarding + per-leg active highlight + auto-bake + S189 dedup + HISTORICAL_LANDMARKS split + historical_note column drop).** Ten threads landed. **(1) FAB walk onboarding:** in Salem (real GPS or manual long-press) walk-sim now builds a routed walk from current position to nearest tour-polyline point; out of Salem drops on waypoint 1. **(2) Per-leg active highlight:** each tour leg is its own Polyline pair, closest-leg-to-user → bright blue 12/18 px on top via full overlay-list rebuild; inactive legs full-strength green 6/10. **(3) Two-finger map rotation:** `RotationGestureOverlay` registered in `setupMap`. **(4) Auto-bake gradle preBuild (S180/S185 carry-forward done):** `publishSalemPois → publishSalemTours → publishSalemTourLegs → alignSalemAssetSchema → verifyBundledAssets` chained in `app-salem/build.gradle`; `assembleDebug` now propagates admin edits to Lenovo automatically. Escape hatch `-PskipPublishChain`. **(5) S189 hard-purge of soft-deleted POIs:** new `cache-proxy/scripts/dedup-merge-and-purge.js` (normalised name + ≤300 m, per-field merge with LIVE coords NEVER touched), 220 → 0 dead, 13 merges into living cluster-mates / 207 orphan deletes. **(6) Live-cluster dedup with BCS-keeper rule:** new `cache-proxy/scripts/dedup-live-clusters.js` (Union-Find clusters, source priority `salem_intelligence_bcs > manual_curated > openstreetmap > api_sync > destination_salem > massgis_mhc`, FK redirection across 6 tables before delete), 35 clusters → 43 deletes / 93 fields merged. John Ward House cluster collapsed to one BCS tour-POI keeper. **(7) HISTORICAL_LANDMARKS category split:** new PG `salem_poi_categories.HISTORICAL_LANDMARKS` row (label "Historical Landmarks", `pref_key=poi_hist_landmark_on` reuses existing Layers toggle, default off); 484 `data_source = 'massgis_mhc'` rows reclassified out of `HISTORICAL_BUILDINGS` (now 105 curated). 8 Kotlin files registered the new category in their hardcoded lookup tables; `NarrationGeofenceManager.TOUR_HIST_LANDMARK_CATEGORIES` swapped BUILDINGS→LANDMARKS so curated buildings always narrate while the MHC tail is layer-gated. Icons + heroes alias to `historic_house` set; operator will regenerate distinct landmark imagery later. **(8) Walk-sim onboarding race fix:** `TourViewModel.startTour(skipOnboarding: Boolean = false)` flag — both walk-sim entrypoints pass `true` to suppress the parallel `onboardToNearestPolylinePoint` directions session whose drift-reroute loop was painting fresh `#22C55E` polylines on top of the per-leg overlays mid-walk. Plus `tourViewModel.clearDirections()` inside `startWalkSim()` for any pre-existing session. **(9) Czech LLM-disclaimer prefix strip:** `Poznámka: Tento výstup je generován automaticky a může obsahovat chyby.` removed from `gardner_pingree_house.historical_narration`; corpus-wide scan turned up no other contamination. **(10) `historical_note` column dropped, Room v15 → v16:** lifted into `description` (10 clean fills + 133 appended with `\n\n` + 2 substring overlaps skipped + 0 gaps), `ALTER TABLE DROP COLUMN`, 5 Kotlin + 4 cache-proxy/web files updated, KSP regenerated `schemas/16.json` identity_hash `2b9838099e6df9b9a616f538c6e99a0c`. Also: bumped tour-POI `geofence_radius_m` floor 40→80 m chasing John Ward House non-narration, then reverted to 40 m once log analysis revealed a downstream 40 m dequeue staleness drop (real fix is leg-authoring or POI relocation operator-side). All shipped to Lenovo HNY0CY0W via auto-bake. **Session 215 (prior):** Edge-point Dijkstra — free tour waypoints persisted with `(edge_id, edge_fraction)` by S214 now actually thread through the bound edge-foot at routing time. **Session 214 (prior):** Tour waypoints bind to TigerLine edges at placement; TourModePreview panel mirrors S186 narration gate. **Operator-stated hard pause from S214 still in effect: Salem Oracle (:8088) + SalemIntelligence (:8089) are undergoing major reformulation to attach full per-claim provenance; without source attribution V1 content cannot ship legally — do not propose work consuming them or backfilling content from them until the operator signals the pause is lifted (memory `project_oracle_intelligence_paused_provenance.md`).** **Session 213 (prior):** TTS pre-normalization layer in NarrationManager (markdown + abbreviation expansion + 32-test suite). **Cross-repo Salem commit owed (still).** Detail in `docs/session-logs/session-{200..207,213-216}-*.md`.
...
### S217 next-session actions

- **John Ward House (and similar) tour-leg fix** — operator-side. The leg currently passes 55–73 m from the POI's BCS coords, outside the 40 m narration trigger. Either re-author the leg in admin to pass within 40 m, or move `john_ward_house` in admin closer to the leg. Geofence floor was tested at 80 m and reverted to 40 m per operator rule + downstream dequeue 40 m staleness drop.
- **HISTORICAL_LANDMARKS icon + hero regeneration** — currently aliased to the `historic_house` set used by HISTORICAL_BUILDINGS. Operator plans distinct landmark imagery for production.
- **Content rebuild for `description` post-merge** — 133 POIs had `historical_note` appended onto `description` with `\n\n` separator. Operator's commercial-production content pass will rewrite these cleanly.
- **Dequeue 40 m staleness drop** — narration queue rejects POIs whose distance >40 m at play time. Worth confirming this matches the rule precisely or whether it should align with each POI's own `geofence_radius_m`.
- **S213 TTS field listen-test on Lenovo** (carry-forward) — confirm abbreviation expansion + markdown strip in real listening (`Capt. John Smith`, `St. Peter's Church`, `Salem, Mass.`, `*The Scarlet Letter*`).
- **S215 visual edge-highlight tick** (S214 Phase-2 polish, low priority) — render a thin fuchsia tick on the bound edge in `TourStopLayer`.
- **S215 suspicious-leg graph investigation** — Dr. K legs 13/14 + WD1 legs 3/6/10/11 show "short straight, long routed" — pre-existing routing-bundle data oddity, fixable in `tools/routing-bake/`.
...
- ✅ **Tier-2 auto-bake** (S203→S206→S216) — `publishSalemContent` umbrella task hooked to `preBuild` in `app-salem/build.gradle`; 4 publish scripts run in order before `verifyBundledAssets`. **S216.**
