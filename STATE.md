...
**Last updated:** 2026-04-26 — Session 178 closed. **P6 SHIPPED** + new `:routing-jvm` module + first/last-approach segments + surgical OSM pedestrian ingest. P6: tour-leg polylines pre-baked from the same TigerLine bundle the runtime Router uses, written to both the new `tour_legs` table (in `salem_content.sql`) and the runtime `app-salem/src/main/assets/tours/*.json` files (replacing S125's OSRM-baked content). Routing engine extracted from `:core` into a new pure-JVM `:routing-jvm` module so Android and the salem-content build pipeline share one Router; package preserved (`com.example.locationmapapp.core.routing`), all 10 RouterParityTests still green at the new location. `WalkingDirections.bundleRoute` got first/last-approach segments capped at 60m so live polylines visibly start at the user's GPS and end at the POI (when within tolerance) without drawing straight lines across Salem Harbor. For pier/peninsular POIs that TIGER doesn't reach, surgical OSM ingest of 11 hand-picked osm_ids landed: Derby Wharf, two Seven Gables waterfront paths, seven Charter Cemetery interior paths, and Salem Willows footway — all hand-verified with TIGER hookup endpoints within 30m, mtfcc='OSMP' for traceability. Derby Wharf Light snap dropped 349.3m → 5.3m, Charter Cemetery 83.8m → 5.5m, Witch Trials Memorial 69.8m → 8.3m. A first attempt at broad pgr_createTopology-style OSM bulk ingest (2,071 features) produced 608 disconnected graph components and broke 57 tour legs before being rolled back — captured as a hard rule in `feedback_no_osm_use_local_geo.md` addendum. Two new memories: `feedback_v1_no_external_contact.md` (Android app makes ZERO outside contact except GPS — V1 mandate), `feedback_internal_db_single_source.md` (internal DB is canonical; web admin may validate against PG, runtime app makes no validation calls). Full detail in `docs/session-logs/session-178-2026-04-25.md`.

S177 standing: cache-proxy salem-router (JS port of bundle), web admin Directions UI. S176 standing: live re-routing (`DirectionsSession`), MBTA-origin walk-sim, Router JVM parity tests (now in `:routing-jvm`, 10/10 green, ±1 mm). S175 standing: on-device Salem walking router with TigerLine-parity bundled SQLite (now 16,237 edges / 12,756 walkable nodes after surgical OSM ingest). S174 standing: web admin Tours tab with full CRUD. S172 standing: animations live in `SalemMainActivity`. Water visual is "not the best" per operator and is still owed.

---

## TOP PRIORITY — Next Session (S179)

0. **DEBUG / VISUAL POLISH ON DIRECTIONS POLYLINE** — carry-forward from S175 (still owed). Color `#22C55E` on Lenovo, math mm-parity with TigerLine on Android AND web. Any issue is in `drawWalkingRoute` / `showDirectionsInfo` / `showTurnByTurnDialog` (Android) or `<DirectionsLayer>` / `<DirectionsPanel>` (web).

1. **Edge-splitting for long OSMP edges.** Derby Wharf is currently one 613m edge with no intermediate nodes; the `derby_wharf` POI itself snaps 180m to one of the endpoints. Split long OSMP edges every ~30-50m at intermediate vertex positions so mid-feature POIs snap close. Same pattern would help Salem Willows footway (163m).

2. **Content-data fixes for outlier POIs.** Two POIs have wrong coords causing >60m snap gaps even with the OSM features in place: `salem_common_2` is at (42.5203, -70.8816) but Salem Common is at ~(42.5232, -70.8908) — 600m off; `salem_willows` is at (42.535, -70.86945) mid-parking-lot instead of on the Fort Avenue walkway. Both are `UPDATE salem_pois SET lat=…, lng=…` then rerun salem-content + rebake `salem_content.db` + rebuild APK. Heritage Trail tour also references 6 POIs not in the curated tour-POI set (validator errors); decide whether to drop the tour for V1 or add the missing POIs.

3. **Tour-legs Room consumer.** Runtime tour player still calls `Router.route()` per leg instead of reading the baked `tour_legs` table. Add Room entity + DAO, schema bump v9→v10, identity_hash refresh in `bundle-witch-trials-newspapers-into-db.js`, wire `WalkingDirections.getBundledTourRoute()` (or successor) to read from DAO first, fallthrough to Router on miss. Saves ~90 ms on tour start.

4. **Water-aware approach segments (post-V1)** — current 60m cap means peninsular destinations (`salem_ferry_terminal` at 437m, future ferry-pier POIs) end at the snap node with no visible final-leg. Long-term fix: consult the MassGIS coastline polygon at bake time, or extend the routing graph with the actual pier walkways via the surgical OSM allowlist pattern.

5. **DirectionsSession JVM tests (still deferred).** Convenience-not-correctness; pull in if/when DirectionsSession changes.

6. **P6 — done in S178.** Removed from priority list.
