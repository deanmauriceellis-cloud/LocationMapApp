...
## Last Updated: 2026-04-08 Session 105 (Phase 9P.B Step 9P.10 — POI edit dialog (tabbed) landed)

## TOP PRIORITY — Next Session
**Phase 9P.B Step 9P.10b — Salem Oracle "Generate with AI" integration.** Phase 9P.A is complete; 9P.6 (TS taxonomy) landed S101; 9P.7 (AdminLayout shell) landed S102; 9P.8 (POI tree) landed S103; 9P.9 (admin map view) landed S104; 9P.10 (POI edit dialog tabbed modal) landed S105. The dialog opens on marker click (tree click is select-only per master plan §1296), renders kind-aware tabs (General · Location · Hours & Contact · Narration · Provenance · Linked Historical (9P.10a stub) · Danger Zone), uses `react-hook-form` dirty-fields-only payloads against `PUT /api/admin/salem/pois/:kind/:id`, JSONB textareas with parse-on-submit, soft-delete confirm in Danger Zone with `DELETE /api/admin/salem/pois/:kind/:id`. AdminLayout now exposes `handlePoiSaved` / `handlePoiDeleted` callbacks that patch the shared `byKind` snapshot in place — tree and map reflect changes without re-fetching. Next session: implement Phase 9P.10b — add `web/src/admin/oracleClient.ts` (typed Salem Oracle client targeting `http://localhost:8088`, no auth, permissive CORS), wire the "Oracle: ready / unavailable" status pill in `AdminLayout.tsx` header (poll `/api/oracle/status` once at mount), add the **"Generate with Salem Oracle"** button to the Narration tab of `PoiEditDialog.tsx` (opens sub-dialog with prompt textarea + Reset history checkbox + Generate button → `POST /api/oracle/ask` with `current_poi_id=poi.id`, 5-15s spinner, displays returned text + up-to-8 primary_sources with attribution, "Insert into short_narration / long_narration / description" buttons, "Iterate" follow-up button). Reference: `~/Development/Salem/docs/oracle-api.md` (492 lines, Salem-owned). Skip 9P.10a (Linked Historical Content tab is currently a stub showing "No links yet — see Phase 9Q") until Phase 9Q has run. After 9P.10b: 9P.11 (Highlight Duplicates wiring), then Phase 9P.C publish loop (9P.12-9P.14).
...
- ⏳ **9P.10b** — Salem Oracle "Generate with AI" integration — NEXT
