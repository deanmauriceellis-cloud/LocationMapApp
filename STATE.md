...
**Last updated:** 2026-04-24 — Session 167 closed. APK packaging audit complete. **APK: 706 MB → 79 MB.** Shipped: (1) Walking Tours hang root-caused to missing `PRAGMA user_version = 9` in asset DB triggering `fallbackToDestructiveMigration` — stamped in both source and asset DBs. (2) Walk simulator smoothness — `LocationMode.MANUAL` now bypasses the stationary gate. (3) New `POIs Hist. Landmark` toggle for the 487 `massgis_mhc` POIs. (4) Dead Current/Proposed/Both POI-location selector removed. (5) `poi-icons/` trimmed to witchcraft-style-only WebP (544 MB → 3.4 MB, 7 other styles archived). (6) `poi-circle-icons/` to 128 WebP (36 MB → 1.2 MB). (7) `salem_tiles.sqlite` trimmed to Witchy-only + Salem city bbox + z16-19 + WebP q=78 (207 MB → 30 MB; pipeline at `tools/tile-bake/trim-to-salem.py`). (8) Tile archive **bundled in APK assets** with first-launch auto-extract via `OfflineTileManager` + `noCompress 'sqlite'`. (9) `TileSourceManager` collapsed to single-basemap — Esri/Mapnik/Dark sources + `buildDarkTileSource`/`onDarkModeToggled` paths all deleted. Layers icon in toolbar preserved per operator directive as the home for future toggles. Well under Play Store 150 MB base-module limit — no asset pack required.

---

## TOP PRIORITY — Next Session (S168)

**APK packaging audit complete (S167).** All three pre-Play-Store packaging blockers resolved (poi-icons downsize ✅, tile archive shrink ✅, tile delivery path ✅). APK is 79 MB, comfortably under 150 MB. **Carry-forward from S165/S166: install on Lenovo TB305FU and smoke-test type searches ("dentist", "lawyer", "gym", "coffee") before further Find work.** Also owed: Find WebView gate for stripped commercial categories (#9, S154 carry-forward).
