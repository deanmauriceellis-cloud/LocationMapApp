...
**Last updated:** 2026-05-08 — **Session 232 (debug-only matrix-tilt 3D prototype on osmdroid).** Curiosity-driven session. Net shipped: `TiltContainer` perspective-matrix wrapper around the MapView gated behind `BuildDefaults.TILT_3D_ENABLED = BuildConfig.DEBUG`. Long-press the **+** zoom button to cycle 0° → 30° → 45° → 60° → 0°; angle persists via `MenuPrefs.PREF_TILT_3D_DEG`. Top-pinched trapezoid (`topY = 1.0*h*t`) puts the operator marker at ~75% screen height at max tilt; dark-purple TC background blends the upper-corner wedges. Narration bar + tour-HUD suppressed during tilt (they were getting added as TiltContainer children and warping). ~18 `Marker(binding.mapView)` constructions across 8 files were sed-replaced with `BillboardMarker(binding.mapView)` (now a one-line pass-through subclass — the canvas-matrix billboard trick failed on the Lenovo's HW-accelerated canvas, and a wider-MapView two-pass approach to fill wedges with real tiles was reverted because the seam where the two perspective transforms met read as a "strange division at horizon"). Field-edit field validation (S229+S230 carry-forward) was punted; pre-flight verified clean. Side conversation: Pixel 7a recommended as the budget rooted Android target (~$200-250 used). **Previous (S231) context still applies below.**
...
**Sessions completed:** 232. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
