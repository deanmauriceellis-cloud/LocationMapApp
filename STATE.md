...
0. **PRE-MONDAY HARD PRIORITY — Webex demo: emulator + working TTS routed into the call.** The 2026-04-20 lawyer meeting is a **Webex call**, not in-person. Operator needs to screen-share from their Linux workstation, which means the demo runs on an **emulator** (not the Lenovo), AND the demo must include TTS narration playing audibly THROUGH THE WEBEX CALL (so the lawyer hears it). Webex-on-Linux specifics: "Share Computer Audio" toggle must be on, or a PulseAudio `module-loopback` must route the emulator's audio sink into the Webex mic input. Target AVD: `Salem_Tour_API34` on port 5570 per `feedback_salem_emulator.md`. Tasks: (a) wait for RTX 3090 to free up (triptych campaign still running at S146 close — start emulator only after per `feedback_gpu_caution.md`); (b) boot the AVD cold; (c) `./gradlew :app-salem:installDebug` with `ANDROID_SERIAL=emulator-5570`; (d) verify TTS engine + US English voice data on the AVD (install Google TTS if default system image lacks it); (e) verify end-to-end audio routing — play from emulator, have a second machine dial into a test Webex meeting, confirm they HEAR the audio; (f) dry-run the demo flow (splash voiceover → welcome dialog → Heritage Trail start → hear a POI narrate → hear a newspaper cut off by a POI ENTRY to demonstrate S146 priority fix). This item supersedes everything else below if time is tight. Full detail in live log.
