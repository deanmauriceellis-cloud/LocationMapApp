...
**Last updated:** 2026-05-06 — **Session 230 (Field-edit can CREATE new POIs from the Lenovo, end-to-end).** Closed the S229 carry-forward "create new POI here" entry kind. **Trigger:** long-press on empty map (when field-edit toolbar ON) opens a slimmed AlertDialog "Create new POI" sheet pre-pinned at the long-press lat/lng — required: name only; optional: re-pin via tap-to-place, recon photo, free-text note. **JSONL schema bumped 1 → 2** with a top-level `kind: "update" | "create"` discriminator on every line; create rows omit `poi_id` / `poi_name` / `current_*` and carry `proposed_name`. Server defaults missing `kind` to `"update"` for back-compat with S229 pulls. **Web admin gate:** `cache-proxy/lib/admin-field-edits.js` `/apply` route branches on `e.kind === 'create'`, requires operator-supplied `category` (subcategory optional id) in the request body, generates id via slug+6-hex (one retry on collision), INSERTs with `data_source='admin_create_field'` + `admin_dirty=TRUE`. **UI:** new `<CreateCard />` in `web/src/admin/FieldEditsTab.tsx` (teal-bordered, "Pending Create" badge) shows proposed name + location plus a strict Category `<select>` (required) + Subcategory `<select>` (optional, gated on category pick); Apply button disabled until a category is chosen. Server-side validated end-to-end with a synthetic create-kind JSONL line: list → apply-without-category 400 → apply-with-`{category:"SHOPPING"}` INSERTs row → PG SELECT confirmed; synthetic data cleaned. APK installed on Lenovo HNY0CY0W (`adb uninstall && adb install`); `tsc --noEmit` clean. **Owed for next session:** field validation of the long-press → name → save → pull → web-admin-create flow on a real walk (parallel to the still-owed S229 update-flow field validation); GPS-burst round-2 field walk; POI/path-alignment review of the existing 769 burst photos via the Photos overlay; S227 upright-skeleton bridge field-walk. **Sessions 220-229 (prior — see `SESSION-LOG.md` for the rolling 10-session window; Room is still at v19 identity `745afa3eb4ce04bd7873671ea297b6e0`, no schema bump this session).** **Operator-stated hard pause from S214 still in effect: Salem Oracle (:8088) + SalemIntelligence (:8089) are undergoing major reformulation to attach full per-claim provenance; without source attribution V1 content cannot ship legally — do not propose work consuming them or backfilling content from them until the operator signals the pause is lifted (memory `project_oracle_intelligence_paused_provenance.md`).** **Cross-repo Salem commit owed (still).** Per-session detail in `docs/session-logs/session-{220..230}-*.md` and `SESSION-LOG.md`.
...
### S230 outcome — Field-edit can CREATE new POIs (2026-05-06)

Closes S229's "create new POI here" carry-forward end-to-end through the same A→C gate. Long-press on empty map (in field-edit ON mode) opens a teal "Create new POI" AlertDialog sheet pre-pinned at the long-press lat/lng — required at save: name only; optional: re-pin (tap-to-place), recon-photo attach, free-text note. JSONL schema bumped 1 → 2 with a `kind: "update" | "create"` discriminator; create rows carry `proposed_name` and omit `poi_id` / `poi_name` / `current_*`. The web admin gate is unchanged in flow but extended in apply: when `e.kind === 'create'`, the request body must supply `category` (subcategory optional id) — server generates a slug+6-hex id, INSERTs with `data_source='admin_create_field'` + `admin_dirty=TRUE`. New `<CreateCard />` component in `FieldEditsTab.tsx` (teal-bordered, "Pending Create" badge) renders proposed name + location plus strict Category + optional Subcategory dropdowns; Apply button gated on category pick. Validated server-side end-to-end with a synthetic create JSONL: list → apply-no-category 400 → apply-with-category INSERTs cleanly, PG row confirmed, synthetic data cleaned. APK on Lenovo HNY0CY0W; build SUCCESSFUL in 12s; `tsc --noEmit` clean. **Owed:** the actual long-press → save → pull → web-admin-create on a real walk.
...
### S231+ next-session docket — Field-edit field validation (update + create) + GPS-burst review + Haunt field-walk

**S229 + S230 field-edit feature carry-forwards:**
- **Field validation of the UPDATE loop** (S229 carry): multiple edits across multiple POIs (move + category + subcategory + photo attach), pull via `tools/pull-field-edits.py`, exercise Apply + Reject through the web admin Field Edits inbox.
- **Field validation of the CREATE loop** (S230, NEW): toggle field-edit ON, long-press empty spots to seed several new POIs (different names + optional photos), pull, then in the web admin pick categories and click **Create** — confirm fresh `salem_pois` rows with `data_source='admin_create_field'` + `admin_dirty=TRUE`.
- Optional polish post-field-test: more edit kinds still missing — delete-this-POI / split-this-POI / duplicate-this-POI. Operator may want.
...
**Sessions completed:** 230. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
