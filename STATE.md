...
### S262+ next-session docket (post-S261 — Waves 1+2+3 closed; AAB upload-eligible)

**S262 opener (parked from S261, operator: "investigate next session"):** execute `docs/plans/s261-align-asset-schema-hardening.md` — single function edit in `cache-proxy/scripts/align-asset-schema-to-room.js` wrapping per-entity DROP+CREATE+INDEX+INSERT in `db.transaction()` so partial failure rolls back. ~15 LOC delta. 5-step verification suite (smoke + row-count parity + identity-hash + verify-bundled-assets + optional rollback-behavior test). Then pick: Wave 4 (Spanish i18n) OR content authoring docket (66 short narrations, 221 missing subtopics, 14 oversized historicals, ~5-10 pre-1860 landmark backfills) OR admin UX polish (Mass Edit re-export-stale button, Field Edits sync error mapping, Mass Edit success toast).
...
**Sessions completed:** 261. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
