...
**Last updated:** 2026-04-26 — Session 187 closed. **Admin Lint tab is live** (4th view in the web admin alongside POIs/Tours/Witch Trials). 15 instant data-quality checks + 1 on-demand (address↔geocode deep scan via `tiger.geocode()`) — Narration ×2, Tour gates ×1, Historical Buildings ×2, Content ×2, Geography ×2, Duplicates, Cleanup, Provenance, Tour data ×3. Drill-down tree on the left, plain-English message + fix hint per item, four per-item actions: Open Editor / Show on Map (also filters map to current check's POIs + zooms 17→20) / Geocodes (modal with Tiger candidates + override per candidate + validate-current footer) / Suppress (false-positive flag — operator: "things like the lighthouse"). Tiger MA address data fully loaded (576K addr / 703K featnames / 761K edges, 13 tables, ~322 MB on disk under `~/tiger-staging/`); ma_edges import patched around salem-router's pgrouting columns on the parent `tiger.edges` table. New `salem_lint_suppressions` PG table (idempotent CREATE on cache-proxy startup, no manual migration). Lint counts surface significant carry-forward updates: cleanup_dedup_losers actual is **365** (S185 estimated 110), 500+ historical buildings missing year_established (capped — likely more), 119 tour POIs missing image, 75 outlier coords. Files NEW: `cache-proxy/lib/admin-lint.js`, `web/src/admin/{LintTab.tsx, GeocodeCandidatesModal.tsx}`. Files MODIFIED: `cache-proxy/server.js` (+1 line), `web/src/admin/{AdminLayout.tsx, AdminMap.tsx}`. Full detail in `docs/session-logs/session-187-2026-04-26.md`.
...
## TOP PRIORITY — Next Session (S188)

1. **Use the Lint tab to drive content cleanup.** S187 shipped the tool; S188 is the operator working through it. In rough priority order: backfill `year_established` on the 500+ Historical Buildings flagged (Lint → Historical Buildings → Open Editor); author narration for the 30 silent Civic POIs and the 8 museum tour-flags (Lint → Narration); hard-delete the 365 dedup losers pre-AAB (Lint → Cleanup); run the address-geocode deep scan once and use the Geocodes modal to override/validate; suppress lighthouse-style false-positives as they appear.
