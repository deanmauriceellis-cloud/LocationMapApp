...
**Last updated:** 2026-05-23 — **Session 289.** 1692 raster recovered + basemap minZoom bug fixed + per-map zoom-envelope rebake. S288-end "corrected anchor" 1692 bake was found to have produced **15,364 fully-transparent 216-byte tiles** (no content at all — the cream operator saw was the Witchy basemap leaking through). Root cause: my corrected lat/lngs put Parris and Meeting House at identical longitude while operator's pixel picks were 36 px apart → TPS folded into a butterfly → cutline cropped the empty zone between the folded lobes. Recovery: reverted to S288 mid-bake .points + Hale Farm correction only (6 GCPs) → 15 MB warp with real content. Cutline still self-intersects in gdal's pixel-space inverse-TPS (no fix from shapely `make_valid` / `buffer(0)`), so baked without cutline per operator preference ("preserve full area"). **Picker upgrade**: added "+ Free pick" mode to `tools/historical-maps/1692/gcp_picker.html` (click Upham → click Leaflet → auto-numbered #100+ free GCP); anchor #12 relabeled to Hale Farm. **Basemap unraveled mid-session**: `TileSourceManager.kt:58 buildOfflineArchiveSource("Salem-Custom", 16, 19)` was declaring minZoom=16 even though DB had z14/z15 tiles → at z14-z15 osmdroid rendered nothing. Widened to (11, 19) and lifted `MAP_MIN_ZOOM` 14.0 → 11.0. New `tools/tile-bake/downsample-overviews.py` fabricated z11-z13 from z14 by 2×2 stitch + bilinear + WebP q=70 method=6 (+221 tiles, 0 MB net). **Verbose tile-fetch logging** wired throughout (`BasemapTile REQ` + `TileArchive DECODE`/`MISS` first-occurrence per key + osmdroid `setDebugMode/setDebugMapView/setDebugTileProviders`, all `BuildConfig.DEBUG`-gated, R8 strips). **Architectural pivot per operator**: each historical map has its own natural zoom envelope based on source detail. Upham 1866 has no per-building detail; z15-z19 just stretched the same drawn lines, while z11 is the regional view that's actually useful. Re-baked 1692 as **z11-z14 only** (65,574 → 91 tiles, salem_tiles.sqlite **377.9 → 353.3 MB**, -24.6 MB net). For 1700/1851/1911: each will get its own envelope (1700 z13-z17, 1851 z16-z19, 1911 z14-z18). App-disappeared mishap mid-session (cd-chain broke pipeline → uninstall ran but install didn't) — recovered with clean rebuild.
...
### S290 opener (parked at S289 close)
...
**Sessions completed:** 289. **Internal ship target: 2026-08-01.**
