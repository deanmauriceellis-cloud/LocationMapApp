...
**Last updated:** 2026-05-04 — **Session 224 (refinement — walk-sim queue stall + Find/PoiDetailSheet V1-dead code purge + lean+state session-start protocol).** Six entries in a refinement session while the SI provenance reformulation continues. (1) **Session-start protocol amended** lean → lean+state — STATE.md and the most recent live log now mandatory at session start; CLAUDE.md + memory `feedback_session_protocol.md` rewritten. (2) **Walk-sim queue stall** found from operator's "I was walking then it stopped but FAB still says walk" report on `tour_LongWalk` (Lappin Park → Rockafellas cluster). Root cause: `enqueueNarration` unconditionally bumped `narrationCancelForPoiAtMs` even on queue-only adds, so an auto-ENTRY 13ms after a previously-playing POI's natural Idle suppressed that Idle via the S146 cancel-for-POI guard, stalling the queue (walker dwelled 180s safety-cap, advanced 1 step, re-fired the same POI). Fix: stamp + newspaper-cancel only on the direct-play branch (`if (jumpToFront || currentNarration == null)`); queue-only adds leave the timestamp alone. Operator field-confirmed clean walk through the previously-stuck cluster. (3) **`tour_LongWalk` (26 legs)** published to Lenovo; total assets now **4 tours / 73 legs / 2081 POIs / Room v17 `d44caaec…`**. (4) **V1-dead code purge** across `SalemMainActivityFind.kt` (2296 → 1946 LOC, –350) + `PoiDetailSheet.kt` (870 → 839, –31): Comments section (~190 LOC of socialViewModel-driven UI), Yelp Reviews button, in-app `showFullScreenWebView` (88 LOC) + 5 `android.webkit.*` imports, V2 website spinner+fetch + `var resolvedUrl`, V2 `geo:` Directions fallback — all `if (!FeatureFlags.V1_OFFLINE_ONLY)` branches collapsed; `FeatureFlags` import removed from both files; stale `@SuppressLint("SetJavaScriptEnabled")` removed. **Net –381 LOC.** (5) **Two pre-existing Kotlin warnings** cleaned: deprecated `setBuiltInZoomControls(false)` (redundant w/ `zoomController.setVisibility(NEVER)`); always-false `if (activeTour == null)` replaced with `val resolvedTour = activeTour!!` pin + 3 downstream call-site rewrites. `compileDebugKotlin --rerun-tasks` zero warnings. (6) **Operator-direction at close:** S225+ are refinement on the **Find menu surface** — categories, subcategories, hero pictures, click-through (POI detail) presentation — while SI rebuild continues. **Session 223 (prior — backlog junk).** Cleared S222 carry-forward — cache-proxy autoloads `dotenv` (`require('dotenv').config()` at top of `server.js`, `dotenv@^16.6.1` added). Plain `npm start` survives reboot without external env-source. Also retired stale CLAUDE.md priority-block item #6. **Session 222 (prior — utility, restart web-admin post-reboot).** **Session 221 (prior — tour detour feature + web admin fuzzy search + 2 GB stale-file recovery).** End-to-end detour across 11 files (3 new), `TourState.Detour` variant + 4 SharedPreferences keys persist across cold launches, narration gate relaxed during detour, fuchsia banner / dialog flow / polyline overlays. **S217-S220 (prior):** category-aware FAB narration + Show-All FAB override (S217); per-POI Validate via TigerLine/Google + `docs/TIGERLINE.md` (S218); storytelling-with-subtopics framework + Room v17 (S219); subtopic UI on narration banner + auto-gen pipeline 497 POIs (S220). **Operator-stated hard pause from S214 still in effect: Salem Oracle (:8088) + SalemIntelligence (:8089) are undergoing major reformulation to attach full per-claim provenance; without source attribution V1 content cannot ship legally — do not propose work consuming them or backfilling content from them until the operator signals the pause is lifted (memory `project_oracle_intelligence_paused_provenance.md`).** **Cross-repo Salem commit owed (still).** Detail in `docs/session-logs/session-{215..224}-*.md` and `SESSION-LOG.md`.

> Older "Previous" entries (S180 / S183-S189 trimmed S201; S195 trimmed S202; S196-S199 trimmed S204; S213-S216 prose trimmed S224) — moved out to keep STATE.md under the 200-line cap. Detail lives in `SESSION-LOG.md` + `SESSION-LOG-ARCHIVE.md` + per-session live logs at `docs/session-logs/session-NNN-*.md`.
...
### S225+ next-session actions — Find menu refinement (operator direction at S224 close)

While SI provenance reformulation is in progress, the next several sessions are **Find menu refinement**: categories, subcategories, hero pictures, click-through (POI detail) presentation. Specific docket:
...
**Sessions completed:** 224. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
