...
**Last updated:** 2026-05-09 — **Session 238 (Phase 3 wedge fill: perspective softened + cull loosened + diagnostic strip).** Diagnosed S237's "z19 dark wedge" via in-place canvas-clip + MapView geometry instrumentation: geometry was already correct (mv extended 6× container, symmetric placement, full canvas clip) — the operator's complaint was perceptual, not a layout/clip/fetch bug. At the previous `topY = h * 1.0 * t`, the perspective vanishing point sat inside the visible screen and z19 detail crushed to illegible gray in the band between vanishing and trapezoid top. Fix is one constant: `topY = h * 0.6f * t` (new `TOPY_FACTOR` companion shared between `rebuildMatrix` and `depthScale` so both stay in lockstep). At max tilt 48° the trapezoid now covers ~68% of screen, vanishing point is off-screen, and z19 street/building detail stays legible — operator's "fills the screen" requirement met (verified via adb-driven walk-sim teleport + screenshot at NPS Visitor Center, full Salem detail visible across whole screen with only small unfillable corner triangles dark). Marker + sprite culls loosened (`ctnY < 0` was the wedge-floats guard; with wedge filled it's redundant — now allow markers anywhere in MapView's extended drawing rect). S234/S237/S238 diagnostic scaffolding stripped: `BILLBOARD-DIAG`, `WEDGE-DIAG`, `OVERLAY-TYPES`, post-tilt-burst + steady-state frame timing, dispatchTouchEvent timing, `billboardMode` long-press toggle on 3D FAB, `debugSuppressOverlays` + companions, the `framesToLog/steadyFrameCounter/moveEventCounter` field family, `applyMapExtension` verbose log lines. **`TiltContainer.kt` 614 → 463 lines (-25%)**; logcat clean to a single `setTiltDegrees: X° → Y°` line per user input. **Carry to S239:** field-walk validation of S238 perspective fix in motion (current verification is static screenshot only); first-frame layout race in `applyMapExtension` (mv.size still 1340x684 on first dispatchDraw after entering tilt before layout pass — fix with `mv.requestLayout()` after `setLayoutParams`); sprite path field-walk near a haunt POI; S237/S236 carry-forwards (clean-reproduction of +10mi-with-buildings, residual cold-start tile-decode spike, AAB ship story for the larger bundle, splash-tree authoring, field-edit field validation). **Previous (S237/S236) context still applies below.**

— **Session 237 (Billboarded POIs + sprites in matrix-tilt; rotation/scale/cull corrections; tile bake stripped of POI symbol layers; S238 picks up wedge tile fill).** Two-pass tilt rendering shipped: `TiltContainer.dispatchDraw` strips Markers from `mv.overlays` before tilted super-draw, then re-renders them upright in pass-2 with rotation around `mvCenter` (mapOrientation), scale around mvCenter (mapView.scaleX), `+mv.left/+mv.top` offset, `tiltMatrix.mapPoints` final. Cull guard against POIs leaking into the upper "wedge" area. Operator walkthrough surfaced 8 issues triaged into a 4-phase plan (full breakdown in S237 live log). Phase 1 ✅ (rotation sign + cull + FAB scale) and Phase 2 ✅ (POI symbol layers `poi_cemetery_label/poi_monument_label/poi_worship_label` stripped from `tools/tile-bake/style-salem.json`, S234-baseline re-bake of 715,841 tiles → 253.8 MB → merged into 261.6 MB bundle, Lenovo single-asset APK confirmed via unzip-l) shipped. `BILLBOARD_NEAR_SCALE` 2.0→1.4 and `BILLBOARD_FAR_SCALE` 0.6→0.95 final tighten deployed but unverified before session end. Phase 3 carried (now ✅ shipped in S238). Diagnostic infrastructure (`BILLBOARD-DIAG`, `OVERLAY-TYPES`, billboard-mode long-press toggle on 3D FAB) carried (now ✅ stripped in S238).

— **Session 236 (+10mi-with-buildings raster bake executed; Lenovo crash on launch; rolled back to S234 tiles).** S235's vector pipeline (`salem-vector.mbtiles` 75 MB OMT + `buildings.mbtiles` 36.8 MB +10mi buildings) was rasterized through the 8-worker parallel bake in 12 min: 715,841 tiles, fresh `salem-custom.mbtiles` 832.5 MB, merged `dist/salem_tiles.sqlite` 847.9 MB. Built 1.07 GB debug APK (note: accidentally bundled both live + .preS236.bak sqlites because backup was left in `app-salem/src/main/assets/`), installed to Lenovo HNY0CY0W: app launched, fired cinematic zoom, UI thread wedged at fps=0.55, RescueParty killed the process — no FATAL stacktrace. Initial bitmap-pressure diagnosis (4 GB device, 460 MB heap watcher, +10mi+tilt visible-tile count × 262 KB ARGB_8888) is **uncertain** because the .bak duplicate polluted the APK; clean reproduction owed before deciding next step. Operator chose rollback: restored S234's 274 MB tile bundle in app assets, moved .bak out of `assets/` to `tools/tile-bake/dist/`, rebuilt clean (374 MB APK with single sqlite confirmed via `unzip -l`), reinstalled — operator confirmed working. New bake/merge artifacts retained on disk (gitignored: `salem-custom.mbtiles`, `dist/salem_tiles.sqlite`, `*.preS236.bak`, `bake-S*.log`, `merge-S*.log`) for the S237 clean-reproduction test. **Open S237 question:** is the 848 MB tiles-with-buildings asset actually unrunnable on the Lenovo, or was the crash an artifact of the duplicated .bak in the APK? Test = rebuild with only the 848 MB asset in `assets/` (no .bak alongside), reinstall, observe. If still crashes: drop tilt by default OR shrink the bake (q=40, palette quantization, drop building rasterization at z14-z15) OR revisit the matrix-tilt visible-tile-count math. **Previous (S235 / S234) context still applies below.**
...
**Sessions completed:** 238. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
