...
**Last updated:** 2026-04-26 — Session 180 closed. **V1 Play Store gating chores DONE end-to-end**, first signed AAB built (78 MB) + signed APK (87 MB) installed and 60fps-running on Lenovo TB305FU. P1 stripped manifest network surface (INTERNET / ACCESS_NETWORK_STATE / cleartextTraffic gone, WickedMap LAUNCHER intent removed). P2 hard-gated 10 V1-disabled UI bypass sites (Find Reviews/Comments/Website-resolve, Radar NEXRAD, webcam View-Live, PoiDetailSheet) behind `if (!FeatureFlags.V1_OFFLINE_ONLY)`; R8 + const physically strips disabled branches from V1 binary. Find dialog Directions button permanently switched to on-device `walkTo()` (S175 router) instead of Google Maps URL. P3: V1 Visit Website button hands off to external browser via ACTION_VIEW (per operator: "If they have a URL, we can direct users to it"); defensive gates on every social dialog body. P4: `UserDataDatabase` migration lockdown — `fallbackToDestructiveMigration` removed, schema v2 is the v1.0.0 floor; `SalemContentDatabase` keeps fallback (intentional — bundled content). P5: versionCode 1 → 10000, minifyEnabled+shrinkResources, conservative `proguard-rules.pro` (with -dontwarn for sqlite-jdbc/slf4j leaking from `:routing-jvm`), `signingConfigs.release` reading from `~/.gradle/gradle.properties`, `verifyBundledAssets` Gradle task wired to preBuild. Generated upload keystore at `~/keys/wickedsalem-upload.jks` (PKCS12, RSA-2048, 10000-day validity), registered in OMEN credential audit (committed + pushed). Two memories: `feedback_admin_changes_propagate_to_builds.md`, `feedback_v1_feature_scope_explicit.md`. Full detail in `docs/session-logs/session-180-2026-04-26.md`.

**The V1.0.0 first-AAB build path is no longer blocked by technical chores.** It's blocked by commercial chores (Play Developer Account, Privacy Policy public hosting, keystore off-machine backup, Form TX) — see TOP PRIORITY below.

S179 standing (still valid): tour routing unified to live `Router.route()` against bundled walking graph; bake-time edge splitting (47,704 edges / 44,223 walkable nodes; asset 11.2 MB) drove Phillips House snap 95m → 30m, operator-confirmed working. S178 standing: `:routing-jvm` extraction + 60m approach cap + surgical OSM ingest. S177 standing: cache-proxy salem-router + web admin Directions UI. S176: live re-routing + MBTA-origin walk-sim + JVM parity tests. S175: on-device Salem router. S174: web admin Tours tab CRUD. S172: animations live in SalemMainActivity (water visual still "not the best").

---

## TOP PRIORITY — Next Session (S181)

0. **OPERATOR-SIDE — Back up the upload keystore off-machine BEFORE first Play Console upload.** `~/keys/wickedsalem-upload.jks` (PKCS12, mode 0600, 2728 bytes) + the password in `~/.gradle/gradle.properties`. Losing this pair = losing the ability to ship updates without going through Google's "lost upload key" recovery process. Documented in OMEN credential audit. Critical and time-sensitive.

1. **OPERATOR-SIDE — Form TX copyright registration.** $65, hard deadline **2026-05-20** (24 days). Operator-owed; Claude can't file it.

2. **OPERATOR-SIDE — Google Play Developer Account + identity verification.** Multi-week lead time. Hasn't been started. Required before first AAB submission.

3. **Privacy Policy public hosting + OMEN sign-off.** V1-minimal Posture A drafted at `docs/PRIVACY-POLICY-V1.md`. Needs public URL (DestructiveAIGurus.com page, GitHub Pages, or dedicated domain) AND in-app link. Full OMEN-008-compliant draft at `docs/PRIVACY-POLICY.md` still pending OMEN review (32+ sessions).

4. **Interactive eyes-on smoke test of the V1 release APK on Lenovo.** S180 confirmed install + launch + 60fps SalemMainActivity for 12s with no crash. Did NOT interactively test: POI detail Visit Website ACTION_VIEW external browser handoff, Find dialog Reviews/Comments hidden in V1, Find dialog Directions on-device router, walking simulator, tour engine narration, geofence triggers. Worth operator's eyes-on the running app.

5. **Tier 2 — admin → build pipeline auto-bake.** Per the S180-saved `feedback_admin_changes_propagate_to_builds.md` rule. Add a Gradle task that re-runs `salem-content` JVM bake from PG `salem_pois` → `salem_content.db` before `assembleRelease`/`bundleRelease`. Surface a "stale bake" warning when last-bake-mtime < last-admin-edit-mtime in PG.

6. **Tier 3 — Content-data fixes for outlier POIs.** S178 carry-forward, still owed. Two POIs have wrong coords causing >60m snap gaps even with S179's densified graph: `salem_common_2` at (42.5203, -70.8816) is 600m from actual Salem Common (~42.5232, -70.8908); `salem_willows` at (42.535, -70.86945) is mid-parking-lot, not on the Fort Avenue walkway. Both are `UPDATE salem_pois SET lat=…, lng=…` then re-bake + rebuild AAB. Heritage Trail tour also references 6 POIs not in the curated tour-POI set; decide whether to drop the tour for V1 or add the missing POIs.

7. **Carry-forward from S180 — interactive UI verification.** Toolbar buttons hidden? POI detail Visit Website button visible + ACTION_VIEW handoff to Chrome? Find dialog Reviews/Comments hidden? Directions button shows on-device green-polyline route? Webcam dialog hides "View Live"? All of these were const-gated; R8 strip in release should make them invisible. Operator visual confirmation owed.

8. **DEFERRED from S179 (still valid):** Option 2 runtime mid-edge projection in `:routing-jvm` Router; walk-sim + DebugEndpoints `TourRouteLoader` cleanup → full retirement of S178 P6 dead data; water-aware approach segments (post-V1).
...
**Sessions completed:** 180. Salem 400+ target 2026-09-01 now aspirational — operator has committed to quality over schedule. **First signed AAB build path UNBLOCKED on the technical side (S180); blocked only on operator-side commercial chores.**
...
- **PG `salem_pois`:** **2,778 total** (up from 2,303 in STATE post-S163; +475 since). 1,443 visible / 1,335 hidden. 2,354 narrated (is_narrated=true). 2,639 with short_narration > 20 chars; 1,865 with long_narration > 50 chars. 503 with `building_footprint_geojson`. 1,649 with non-blank `website` URL (used by V1 Visit Website ACTION_VIEW external-browser handoff per S180 P3). **0** with `lat_proposed`/`location_status` populated (verifier still blocked on TIGER MA ingest per S162).
