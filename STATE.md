...
**Last updated:** 2026-05-11 — **Session 242 (whole-repo code review + Tier 1 + Tier 2 debt reduction).** Nine parallel Explore agents audited Android / web admin / cache-proxy lib + scripts / JVM modules; one new memory recorded mid-audit (`project_v1plus_features_parked_not_killed.md` — MBTA/aircraft/weather/transit are gated pending lawyer approval, not dead). **Two whole modules deleted as dead code: `:app` (33 files, ~14.8K LOC, zero referrers — pre-pivot generic LocationMapApp) and `:salem-content` (19 files, ~12.8K LOC including `SalemBusinessesExpanded.kt`'s 8991-LOC hardcoded POI snapshot superseded by PG + cache-proxy publish chain).** Plan-mode-approved 8-phase execution, one commit per phase. Phase 1 deleted `:app`. Phase 2 deleted `:salem-content` + rewrote the comment in `routing-jvm/JdbcRoutingBundleLoader.kt:9` + relabeled the 6 lingering `group = 'salem-content'` Gradle task-group display labels in `app-salem/build.gradle` to `'salem publish'`. Phase 3 archived 32 stale scripts to `docs/archive/scripts/{migrations-2026-04,one-shot-authoring,historical-dedup,one-shot-ingest}/` AND retargeted 7 publish-chain scripts to write directly to the bundled asset (the deleted `salem-content/salem_content.db` intermediate had broken `./gradlew :app-salem:assembleDebug`). Phase 4 deleted Room schema JSONs v10–v18 (~540 KB; only v19 referenced at runtime). Phase 5 shipped 6 targeted Android fixes: explicit `gpsObsHeartbeatJob.cancel()` in onDestroy (S110 `while(true)` coroutine now has visible lifecycle ownership), `mv.requestLayout()` after `applyMapExtension` (closes S238 first-frame layout race), `SCREEN_CULL_PRE_PX`/`SCREEN_CULL_POST_PX` constants in TiltContainer companion + mirrored to SpriteOverlay (sprite now has the explicit post-cull it was missing), new `TiltContainer.onMapStateChanged()` single entry point consolidating the S239 MapListener + FAB-animator updateListener invalidations, dropped `READ_EXTERNAL_STORAGE` manifest permission + removed `record_gps`/`email_gpx` stub menu items + their MenuPrefs/BuildDefaults/MenuEventListener/SalemMainActivity wiring (Build Story/Analyze Today/Travel Anomalies stubs kept as parked V1+ features per operator), struck already-deleted `narrationPointsCache` carry from STATE.md. Phase 6 gated `/overpass` route behind `ENABLE_OVERPASS` env var (per memory `feedback_no_osm_use_local_geo.md`, OSM is build-time only; Android has zero runtime callers; same gating pattern as `ENABLE_CHAT`). Phase 7 standing-doc touch-ups (CLAUDE.md monorepo line + arch tree + Salem JSON data bullet; MASTER_SESSION_REFERENCE.md same; ASSETS-MANIFEST.md `historical_facts` row). Phase 8 added `[PAUSED S214]` header comment to 10 Oracle/Intel-dependent scripts. **Aggregate: 223 files / +246 −60,593 lines across 9 commits.** Lenovo HNY0CY0W reinstalled after Phase 5 per S156 discipline. **Important verifications surfaced in plan mode:** `comments.js`/`chat.js` NOT safe to delete (Android `SocialViewModel.kt:89` reads `resp.comments` — pulled to Tier 6 pending paired Android-side retirement); `group = 'salem-content'` in `app-salem/build.gradle` is just `./gradlew tasks` display labels not module deps. **One non-issue surfaced:** `:routing-jvm:test` parity tests fail 4/10 on current bundle — verified pre-existing on HEAD~1 (bundle was re-baked since S178 fixtures); operator decision needed at S243 (re-bake fixtures vs investigate drift). **Memory writes:** new `project_v1plus_features_parked_not_killed.md`; `MEMORY.md` index entry. **Carry to S243** documented at bottom of this file. **Previous (S241/S240/S239/S238/S237/S236) context still applies below.**

**Recent context — for per-session detail see `SESSION-LOG.md`:** S241 shipped Mass-Edit POI workflow (.xlsx/.ods round-trip, per-cell approve/reject, soft-delete via spreadsheet, in-cell pick-lists for 24 enum-bound columns; operator's first field-use dropped POI count 2070 → 2042). S240 decoupled map marker rendering from narration — `default_visible` is now an honest controller (new `renderable` snapshot in `PoiCache` parallel to `narrated`; 51 POIs newly map-renderable including Bit Bar). S239 fixed three tilt-mode rendering bugs (St. Peter's flat duplicate, upright drift, magnify FAB icon scaling) all traced to HW display-list staleness — per-Marker tilt-suppression flag + MapListener + animator updateListener. S238 shipped Phase 3 wedge fill (`topY` 1.0→0.6) + cull loosen + diagnostic strip. S237 shipped two-pass tilt rendering (strip-then-pass-2). S236 attempted +10mi-with-buildings raster bake → Lenovo crash → rolled back to S234 tiles.
...
**Sessions completed:** 242. **Internal ship target: 2026-08-01** (operator-confirmed S201). Salem 400+ peak attendance October 2026 (~1M visitors).
