...
**Last updated:** 2026-05-30 — **Session 309.** Closed the open Tier-2 challenge from the `PerfStabilityReview_20260529.md` workstream — the **deep-tilt-under-walk-sim ANR cluster** (#3 GPS-marker mutate-in-place, #17/#18 debug trigger rings gated to `BuildConfig.DEBUG`, #35 invalidate-collapse, #5 tilt-aware heading hysteresis 5°→15°), shipped coverage-safe (`f27e14e`) and **verified on the min-spec Lenovo at 48° tilt: zero ANR / zero OOM, PSS ~540 MB** → the long-deferred OOM gating test PASSED, **Tier 3 now optional**. Two things tried + reverted: the **#4 viewport-bbox cull** (symmetric bbox too tight vs the forward tilt wedge → dropped horizon markers) and a **POI marker-sizing pass** (canonical-diameter renderer fix + scale→1.0 to equalize ghost badges with normal POIs — real root causes found but still not right on-device; **marker sizing carries to S310** as the active thread; repo reverted to the 0.67 baseline). Detail: `docs/session-logs/session-309-2026-05-30.md`. **Prior — Session 308.** Investigated the BlueStacks 3D-tilt magenta/cyan/green basemap corruption: built a full **remote BlueStacks debug channel** (SSH→`HD-Adb`→instance; memory `reference_bluestacks_remote_debug_channel`) + shipped `markerScale()` DEBUG instrumentation (kept), then **definitively root-caused it as renderer-agnostic texture-memory exhaustion on BlueStacks' virtualized GPU** under the tilt load — OpenGL fails full-screen, DirectX fails partially, a `TiltContainer` software-layer can't escape the HW-accelerated GL composite, and it can't be disabled app-wide without wrecking map perf. **NOT app-fixable** while keeping HW-accel + the no-blank-wedge rule; **real hardware (Pixel/Lenovo) renders tilt correctly.** BlueStacks **spoofs its GPU** (`Adreno (TM) 650` while x86), defeating renderer-string detection (spoof-proof signal = x86 ABI). The S308 adaptive-GPU fix (`0b81e7e`, software-layer-on-WEAK-GPU) was **proven ineffective + REVERTED** (operator: "this is not the solution"); kept the orthogonal markerScale debug log + investigation docs. Two design/review workflows (45 agents) drove it; the adversarial review caught a real false-demote bug before it could ship. Detail: `docs/session-logs/session-308-2026-05-29.md`. **Prior — Session 307.** Marker-scale tune on the `PerfStabilityReview_20260529.md` workstream (closing the S306 overshoot): **(1)** ghost badges equalized to the regular POI size ladder per zoom bucket (`narrationIconForZoom` 40/30/24/18 → 32/20/12/8) so badges == POIs; **(2)** all POI marker graphics rendered 33% smaller via one new knob `MarkerIconHelper.POI_GRAPHIC_RENDER_SCALE=0.67` (ghost + dot + `labeledDot` icon; scoped to POIs; device `markerScale()` still in front). Compile + unit suite green; deployed to Lenovo via bundletool; operator eyeballed Dr. K's tour → "looks good." Detail: `docs/session-logs/session-307-2026-05-29.md`. **Prior — Session 306:** continued the `PerfStabilityReview_20260529.md` workstream from the S305 Tier-0 cold-start ANR fix. A **7-agent coverage-preserving exploration + blank-space skeptic** (driven by the operator's hard no-blank-wedge constraint — see `feedback_tilt_coverage_is_load_bearing`) established the dominant tilt-memory consumer is the grow-only osmdroid tile cache and the regression is the S299/S301 bitmap floor. **Tier 1 (OOM floor) + Tier 2 (jank) shipped + Pixel-8-validated, coverage-safe** (`dfbc99b`/`398102a`): z19 tile-fetch clamp while tilted (coverage-NEUTRAL — no z20 tiles exist in the bundle), ghost(384→192)/hero/dock downsampling via a shared `SampledAssetDecoder`, `onTrimMemory` real eviction, **ambient `WickedAnimationOverlay` suppressed under tilt** (the 30fps jank driver — 0 heartbeats in 14k tilted lines, resumes flat), dispatchDraw alloc hoist, narration-pulse guard, 350ms FAB debounce. **Historical bundle slimmed** — 1692+1911 overlays pulled (kept 1700+1851), 354→299 MB / AAB 237→184 MB, removed maps stored reversibly (`0ec975a`). **Universal device-size-aware marker scaling** (`MarkerIconHelper.markerScale`, Lenovo 1.73× / Pixel 1.0×, `1a59b1c`) — fixes tiny-tablet markers; operator finds it a touch too large → tune next session. Two reported "issues" diagnosed as non-bugs (St. Joseph off-screen/default-hidden landmark; tour "default" = correct start-from-user-location). **Open: deep-tilt-under-walk-sim ANR + the Lenovo 48° OOM gating test (never ran S306) → the Tier 3 decision.** Full S306 detail: `docs/session-logs/session-306-2026-05-29.md`.

**Prior — Sessions 298–301 (condensed 2026-05-29 at S303 close).** Graphics redo SHIPPED end-to-end: S298 locked the art bible (ink-comic woodcut · muted-period + ghost-teal · narration-mined prompts · `fitXY→centerCrop` 20% banner); S299 generated + wired the **124 bespoke heroes (107 HIST_BLDG + 17 WORSHIP)** under the "h3" moonlit mood + **40 woodcut category scenes** replacing photoreal `poi-icons` (subject-aware prompts fix the 32 statues/memorials/wharves rendered as houses; ~5 named monuments txt2img can't reproduce — operator accepted atmospheric); S301 wired the map-marker half (`MarkerIconHelper.ghostBadge()` for 107 HIST_BLDG markers via `ghost_asset_a` + `render_worship_glyph.py` church-glyph circle for 17 WORSHIP, commits `5757954`+`2b56090`). Generated assets gitignored (reproducible from committed generators) + backed up local + `/mnt/sdb-backups`. **Next graphics carries:** field-walk targeted re-rolls (`render_hero_full.py --ids=`); the rest of `poi-circle-icons/` (still pre-redo witch sigils); ~5 hard monuments (ControlNet or accept atmospheric); splash trim 12→4 Katrina; retire dead `hero/` UUID dir + 803 dangling `image_asset` refs + `custom_icon_asset` (art-bible §7). _(S301 also root-caused operator's tour-mode "all POIs" report to S299 prefs-wipe + S206 mirror, not a regression.)_
...
**Cliff 4 — 16 KB native-lib alignment (OPEN, parked S304):** Google Play requires 16 KB-aligned native libs for Android-15+-targeting updates submitted after 2025-11-01 — must clear before the Aug 1 AAB upload. Bundled `libimage_processing_util_jni.so` from **CameraX `camera-core:1.3.4`** (app-salem/build.gradle:258-261) is NOT aligned (debug-only "App Compatibility" warning fires on Pixel 8/Android 16; release users never see the dialog but the AAB requirement still applies). **Fix:** bump CameraX `1.3.4 → 1.4.x`, rebuild, verify (`llvm-readelf -l` LOAD p_align ≥ 0x4000), confirm in-app capture activity still compiles. Parked per operator (drive-test first).
...
### S310 opener (parked at S309 close)

_**ACTIVE WORKSTREAM = `PerfStabilityReview_20260529.md` perf/stability fixes.** Tier 0 (cold-start ANR, S305) + Tier 1 (OOM floor) + Tier 2 (jank) DONE coverage-safe (S306). **S309 closed the open Tier-2 challenge — the deep-tilt-under-walk-sim ANR cluster is FIXED + committed (`f27e14e`) + min-spec-verified:** #3 GPS-marker mutate-in-place, #17/#18 debug trigger rings gated to `BuildConfig.DEBUG` (were ungated in retail), #35 dropped the rings' invalidate, #5 tilt-aware heading hysteresis (5°→15° under tilt). Lenovo 48° walk-sim ~14 min: **zero ANR / zero OOM, PSS ~540 MB → the long-deferred Lenovo OOM gating test PASSED → Tier 3 geometry cut is now OPTIONAL, not required.** **HARD RULE: never shrink the wedge-fill extension (`EXTRA_TOP_FRACTION=5.0`) — no blank wedges (`feedback_tilt_coverage_is_load_bearing`).**_

_**TOP CARRY (S310) — POI MARKER SIZING (active thread; NOT solved).** Operator wants ghost badges == normal POIs in size; they don't read as equal on-device even after a canonical-diameter renderer pass. S309 found the root causes (the −33% `POI_GRAPHIC_RENDER_SCALE` applies ONLY at the narration path, not the layer/find `addPoiMarker` path; `poi-circle-icons/*.webp` fill their box 100%; fallback dots rendered half/tiny; the ghost ink-ring inset the disc) and built a fix (all renderers fill one canonical per-bucket diameter; scale→1.0) — operator still found it wrong, so **ALL sizing changes were REVERTED** (repo at the `POI_GRAPHIC_RENDER_SCALE=0.67` S307 baseline). Next session: rethink the sizing model holistically (renderers + labels + under-tilt depth-scale `BILLBOARD_NEAR/FAR_SCALE` + device `markerScale` + the narration-vs-layer scale split — a global trim MUST be wired at every marker path). **The Lenovo (HNY0CY0W) carries an experimental sizing build that does NOT match the repo — redeploy the committed build when resuming.**_

_**Other perf carries:** (1) **Deep-tilt ~89 ms/frame jank** (CPU display-list record; GPU only 8 ms) — residual after the ANR fix; next lever = a **forward-biased viewport cull** (the #4 symmetric `proj.boundingBox` cull was tried + REVERTED — too tight vs the asymmetric forward tilt wedge, dropped horizon markers). (2) **Tier 3** (angle-proportional `EXTRA_TOP_FRACTION`, runtime-derived; + optional z18 clamp) — now OPTIONAL per the passed OOM test. (3) **BlueStacks/emulator tilt support-posture** (S308) — virtual-GPU tilt corruption is NOT app-fixable (OpenGL+DirectX both fail under load); decision pending (document unsupported-for-tilt / capability-gate / DirectX hint); real hardware unaffected._

_**Carries:** (a) operator's full visual blank-space check at all 4 ladder angles (Lenovo 42–48° + Pixel already showed a filled wedge). (b) **#31 PlaceResolver** off-main fix still needs LOCATION granted to exercise on-device. (c) **TcpLogStreamer** retrying `10.0.0.194:4301` off-WiFi (~6/min noise on the Lenovo) — S303 back-off carry. (d) **Tech-debt plan** `docs/plans/tech-debt-cleanup-2026-05.md` (P0+P1 done; P2/P3/P4/P5/P6 remain) — LOWER priority than the perf tiers; `walkSim` JobCoordinator migration needs a `suspend cancelAndJoin(name)` extension. (e) Cliff 4 CameraX 16KB alignment bump before Play upload. (f) **dist/ `salem_tiles.sqlite` (940M) is DIVERGENT from the shipped pack** (≠ V1-EXACT) — don't sync dist→pack blindly._

_**Full Explore + Beverly demo carry from S302.** `● FULL EXPLORE` teal toolbar pill (R8-stripped in retail; V1 offline posture intact). 760 MB Beverly demo AAB cached at `/tmp/expanded-map-demo.aab` — `bundletool install-apks` to redeploy. **HARD LESSON: V1 `salem_tiles.sqlite` is GITIGNORED — back it up before any asset swap.** Exact V1 backup at `tools/tile-bake/dist/salem_tiles.sqlite.V1-EXACT-20260527`. Deferred: Beverly POIs (Tier 2 MassGIS). **Side carry S303:** `TcpLogStreamer` retried `10.0.0.194:4301` every 10s off-WiFi the whole drive (~60 W-lines/min noise) — back-off-when-cellular candidate, fits tech-debt P3._

_**Graphics redo — heroes + map markers BOTH wired on-device.** Heroes (124 bespoke h3 + 40 category scenes) shipped S299; map markers (107 HIST_BLDG ghost badges + 17 WORSHIP church glyph) shipped S301. Generators in `tools/art-bible-samples/` (`render_hero_full.py`, `render_hero_pilot.py`, `render_hero_haunted.py`, `render_category_scenes.py`, `render_worship_glyph.py`). Generated hero/ghost assets gitignored (reproducible + `/mnt/sdb-backups`); the worship-glyph + ghost-badge code ARE tracked/committed. **NEXT (graphics):** (a) operator field-walk the Pixel 8 → flag wrong heroes/markers → targeted re-roll (`render_hero_full.py --ids=`); (b) the OTHER `poi-circle-icons/` categories still pre-redo witch sigils — redo if wanted (§4 "POI circle marker"); (c) ~5 named monuments txt2img can't draw (Washington Arch, Witch Trials Memorial, WWII Memorial, Samantha) — ControlNet-from-photo or accept atmospheric; (d) splash trims 12→4 woodcut-Katrina (operator: "at the end"); (e) retire dead `hero/` UUID dir + 803 dangling `image_asset` refs + `custom_icon_asset` (art-bible §7). **Note (S206 mirror UX):** a fresh-installed paid user starting a tour sees the full Historical-Buildings layer (mirror defaults tour-mode toggle to the explore ON value); operator said "good for this" — left as-is, but candidate for a retail hard-default-OFF if it bites. OMEN-025 remains the ship-blocking TOP PRIORITY but is gated on operator creds — see below._
...
**Sessions completed:** 309. **Internal ship target: 2026-08-01.**
...
- **Plans:** `docs/plans/` (active) / `docs/archive/` (older + removed STATE content). **Architecture / tech stack:** `CLAUDE.md` + codebase. **Legal:** `GOVERNANCE.md`. **IP:** `IP.md`. **Commercial:** `COMMERCIALIZATION.md`. **UX/arch decisions:** `DESIGN-REVIEW.md`.
