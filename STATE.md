...
**Last updated:** 2026-04-25 — Session 177 closed. **Two S175 carry-forwards SHIPPED**: P4 (web router in cache-proxy) + P5 (web Leaflet Directions UI). The S175 five-task queue (P3b/P3c/P2c/P4/P5) is now fully drained; only the operator-owed visual polish remains. P4: new `cache-proxy/lib/salem-router.js` opens the same `salem-routing-graph.sqlite` the APK ships, ports `RoutingBundle.kt` + `Router.kt` field-for-field (CSR adjacency, expanding-ring grid KNN, binary-heap Dijkstra). `GET /salem/route` + `POST /salem/route-multi` + `GET /salem/route/meta`. `?source=live` falls through to `tiger.route_walking()` over the unix-socket peer-auth pool; MultiLineString result is stitched into travel order by walking endpoints. All four `RouterParityTest.kt` reference fixtures match bit-exact (1240.7187 m / 509.8629 m / 413.7483 m / non-trivial sanity). Live cross-check produces ±0 mm divergence; >5% divergence would log `[Salem Router] DIVERGENCE` (none observed). P5: new optional `onShowDirections` prop on `PoiEditDialog` adds a "Directions" button; AdminLayout owns `directionsTarget`; AdminMap renders a `<DirectionsLayer>` (Polyline #22C55E with #0f5132 outline, origin CircleMarker, fits bounds once per target+source) and a `<DirectionsPanel>` (distance/duration + bundle/live source toggle + close). End-to-end Vite-proxied data path verified clean. Full detail in `docs/session-logs/session-177-2026-04-25.md`.

S176 standing: live re-routing (`DirectionsSession`), MBTA-origin walk-sim, Router JVM parity tests (10/10 green, ±1 mm). S175 standing: on-device Salem walking router with TigerLine-parity bundled SQLite (16,226 edges / 12,742 walkable nodes). S174 standing: web admin Tours tab with full CRUD. S172 standing: animations live in `SalemMainActivity` (water whitecaps + cemetery fireflies + parks rolling-grass). Water visual is "not the best" per operator and is still owed.

---

## TOP PRIORITY — Next Session (S178)

0. **DEBUG / VISUAL POLISH ON DIRECTIONS POLYLINE** — carry-forward from S175 (still owed across S176/S177). Color is `#22C55E` (green) on Lenovo. Operator owes a manual walk-through (tap-POI → Directions, report line width, opacity, labels, start marker, etc.). Routing math is mm-parity with TigerLine on Android AND web (S177 P4 confirmed); any issue is in `drawWalkingRoute` / `showDirectionsInfo` / `showTurnByTurnDialog` (Android) or in `<DirectionsLayer>` / `<DirectionsPanel>` (web). The web side now also offers a fast headless way to eyeball the polyline — open `http://localhost:4302/admin`, click any POI, hit Directions.

1. **P5 visual smoke (operator-owed)** — S177 closed without a browser-side click verification. Confirm that on `http://localhost:4302/admin` clicking a POI then the new "Directions" button: (a) closes the dialog, (b) draws a green polyline + origin dot, (c) shows the top-right panel with distance/duration, (d) bundle/live source toggle re-fetches and re-renders, (e) close (×) clears the overlay. Cache-proxy must be running on 4300; `tiger` DB peer auth must work for live mode.

2. **P6 — Pre-bake tour-leg polylines.** For each `salem_tour_stops` row, pre-compute polyline + distance + turn-by-turn between consecutive stops and store in new `route_geom_to_next` + `turns_to_next` (TEXT JSON) columns. Eliminates router calls during guided tours; admin tour editor can preview polylines immediately when a stop is dragged. P4 makes this easy — same router instance, same parity guarantees.

3. **DirectionsSession JVM tests (deferred).** S176 P2c covered Router only. The on-device `/directions-session-test` endpoint already validates the same state machine + geometry helpers, so this is convenience-not-correctness — pull in if/when DirectionsSession changes.

4. **S177 P4/P5 — done.** Deleted from priority list; full detail in the live log.
...
3. **Field-test S169 TTS-gated dwell.** Carry-forward from S171/S172 (deferred). Real walk to confirm: walker dwells on EVERY narrating POI (not just CPA winners), dwell exits cleanly on idle+empty, manual speaker-tap skip auto-advances, newspapers don't pin walker. Cheap to validate; do whenever a walk happens to coincide.
