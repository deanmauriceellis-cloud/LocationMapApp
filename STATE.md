...
**Last updated:** 2026-05-02 — **Session 218 (per-POI location validate workflow with map-review UX).** Top-of-card **Validate via TigerLine** (fuchsia) and **Validate via Google** (sky blue) buttons + Lint-tab per-row equivalents both run server-side geocoding (PostGIS Tiger or Google Places API New), filter to the Salem area (15 km drops wrong-town matches), and route into a unified proposal-review mode: editor closes, map flies to proposal at z20, fuchsia "?" pin draggable, small floating panel shows current/proposed/source-pill/drift (amber ≥1 km, rose ≥5 km) + Accept/Cancel/Re-center/"Ask Google ↗"-link. Tiger path includes snap-to-edge (50 m) + street-name-near-POI fallback (KNN on `tiger.edges` for cases like Sea Level Oyster Bar where Tiger's house-number range doesn't include #94 in Salem); Google path uses Places API (New) Text Search with `<name> <address>` query for storefront-level accuracy (resolves Salem Witch Village → 11 m vs Tiger's 113 m parking-lot interpolation). Backend in `cache-proxy/lib/admin-pois.js` (three new endpoints: `/validate-location-tiger`, `/geocode-via-google`, `/discard-proposed-location`; existing `/accept-proposed-location` extended to take optional `{lat,lng}` body for committing dragged-pin position) + new `cache-proxy/lib/tiger-geocode.js` (refactored shared helpers with verbose `[validate-tiger <poi_id>]`/`[geocode-google <poi_id>]` step-by-step logging). Frontend: new `web/src/admin/ProposalReviewPanel.tsx` floating panel + `ProposalPreviewLayer` draggable in `AdminMap.tsx` + `proposalReview` state machine in `AdminLayout.tsx` + Validate buttons in `PoiEditDialog.tsx` header and `LintTab.tsx` per-row. Wrote `docs/TIGERLINE.md` (~370 lines, operator reference covering both pipelines, drift bands, log recipes, MassGIS L3 integration plan for future). `GOOGLE_GEOCODE_API_KEY` copied from SalemIntelligence's `GOOGLE_PLACES_API_KEY` to `cache-proxy/.env`. No DB schema changes (S162 columns carried entire workflow); no Android impact. **Session 217 (prior):** Category-aware FAB POI narration line via `BusinessLabel.kt` curated map; Show-All-POIs FAB now overrides every gate (tour, Layers, Audio Control, no-text early-return). **Session 216 (prior, consolidation):** Ten threads — FAB walk onboarding, per-leg active highlight, two-finger rotation, auto-bake gradle preBuild, S189 hard-purge dedup (220→0 dead), live-cluster dedup with BCS-keeper rule (35 clusters → 43 deletes / 93 fields merged), HISTORICAL_LANDMARKS split (484 MHC rows out of HISTORICAL_BUILDINGS), walk-sim onboarding race fix, Czech LLM-disclaimer strip, `historical_note` column dropped Room v15→v16. **Session 215 (prior):** Edge-point Dijkstra — free tour waypoints with `(edge_id, edge_fraction)` thread through the bound edge-foot at routing time. **Session 214 (prior):** Tour waypoints bind to TigerLine edges at placement; TourModePreview panel mirrors S186 narration gate. **Operator-stated hard pause from S214 still in effect: Salem Oracle (:8088) + SalemIntelligence (:8089) are undergoing major reformulation to attach full per-claim provenance; without source attribution V1 content cannot ship legally — do not propose work consuming them or backfilling content from them until the operator signals the pause is lifted (memory `project_oracle_intelligence_paused_provenance.md`).** **Session 213 (prior):** TTS pre-normalization layer in NarrationManager. **Cross-repo Salem commit owed (still).** Detail in `docs/session-logs/session-{208..211,213-218}-*.md`.
