...
**Last updated:** 2026-05-29 — **Session 303.** **Pixel 8 drive forensics → GPS-OBS heartbeat false-stale fix shipped** (`91277a6`, on `origin/master`). Operator's 5/27 drive on Katrina's tour "had issues at start, had to restart" — pulled all 4 logcat buffers + both Room DBs + every shared_pref + routing graph + 367-pt GPS trail + the **27 MB / 207k-line file-sink debug log** (discovered the S110 `DebugLogger.initFileSink` has been live the whole time at `<externalFilesDir>/logs/debug-YYYYMMDD.log`; new memory `reference_debuglogger_file_sink.md` makes it the first-stop for any future field issue). Root cause: raw `onLocationResult` fired the whole drive (~14s cadence) but the Activity-observer-stamped `lastFixAtMs` went stale during a 7s `onPause→onResume` cycle, tripping `W/GPS-OBS: HEARTBEAT STALE → narration reach-out suppressed` for minutes; app looked dead, operator force-closed (clean `onDestroy finishing=true`, NOT a crash). ProcB recovered only because the Pixel's `TYPE_SIGNIFICANT_MOTION` sensor finally fired at 19:19:11 — **same warning template as Lenovo; now confirmed to bite Pixel 8** (memory `reference_lenovo_motion_sensor_broken.md` renamed `motion-sensor-gate-also-bites-pixel-8`). Fix (3 files, +220/-7): `MainViewModel.stampFixSource()` stamps a source-layer freshness clock on every `LocationManager` flow emission (viewModelScope outlives lifecycle hiccups); GPS-OBS heartbeat + narration reach-out gate now consume `maxOf(activity.lastFixAtMs, viewModel.lastFixAtMs)` + a derived-speed alive-signal (>0.3 mps, matching `MotionTracker` S154); new `BuildConfig.DEBUG`-gated `observer-vs-source skew` log surfaces every catch. Verified Pixel 8 (`bundleDebug` 227 MB clean → bundletool over the S302 demo → cold launch → `HEARTBEAT ok age=10s`, no spurious stale). Operator's next drive validates the cure. _(Prior — S302: tech-debt survey + 7-phase plan-of-record `docs/plans/tech-debt-cleanup-2026-05.md` + `NEXTSTEPS_20260527.md` 7 open Qs **still awaiting go/no-go**; Full Explore toolbar pill; 760 MB Beverly demo AAB cached at `/tmp/expanded-map-demo.aab` if wanted back.)_
...
### S304 opener (parked at S303 close)

_**Operator field-walk validates the S303 GPS-OBS fix.** Take the Pixel 8 (now carrying V1 + `91277a6`) on a real drive. After, pull `/sdcard/Android/data/com.destructiveaigurus.katrinasmysticvisitorsguide/files/logs/debug-$(date +%Y%m%d).log` and grep: (a) `D/GPS-OBS: observer-vs-source skew` — every line = a stale-false-alarm the new fix rescued; (b) `W/NARR-STATE: REACH-OUT SUPPRESSED` — should be far fewer; any remaining ones now show `derivedSpeed=X.XXmps` so we can tell whether the pipeline truly stalled vs. an upstream gate. **Cure validated** if narration fires across `onPause→onResume` cycles without the procA-style suppression cascade. **Cure failed** if `HEARTBEAT STALE` still triggers with `derivedSpeed>0.3` and lifecycle is normal — that'd point at a different code path._

_**Tech-debt cleanup plan-of-record — STILL awaiting go/no-go** (S302 carry, untouched this session). Plan at `docs/plans/tech-debt-cleanup-2026-05.md`; 7 open Qs at `NEXTSTEPS_20260527.md`. 7 phases: P0 publish-chain orchestrator (only pre-ship-critical) → P1 JobCoordinator + MarkerController → P2 dead hero/icon retirement → P3 isolate parked network features into `parked/` → P4 migrate active state into existing VMs → P5 `!!`/catch hardening → P6 web (optional). The S303 GPS-OBS fix is **adjacent to but not part of** the plan — done as a targeted defect fix, not under the cleanup tag scheme. **Next: operator answers the 7 NEXTSTEPS questions + picks start point.**_
...
**Sessions completed:** 299. **Internal ship target: 2026-08-01.**
