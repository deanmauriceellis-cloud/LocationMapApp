...
async function runPoiDbImport(manual = false) {
  if (!pgPool) {
...
if (pgPool) {
  ...
  setTimeout(() => {
    ...
    runPoiDbImport().catch(err => console.error('[DB Import] Initial import error:', err.message));
  }, 30 * 1000);

  // Delta imports every 15 minutes
  setInterval(() => {
    runPoiDbImport().catch(err => console.error('[DB Import] Scheduled import error:', err.message));
...
app.post('/db/import', requirePg, async (req, res) => {
  try {
    const result = await runPoiDbImport(true);
    res.json(result);
  } catch (err) {
...
app.get('/cache/stats', (req, res) => {
  ...
  res.json({
    ...
    dbImport: {
      ...
      running: dbImportRunning,
      intervalMinutes: DB_IMPORT_INTERVAL_MS / 60000,
      pendingDelta: [...poiCache.values()].filter(e => e.lastSeen >= lastDbImportTime).length,
      ...dbImportStats,
    },
