 ...
 *
 * Admin Collection endpoints (S268).
 *
 ...
 * Collection — operator-tunable filters that generate per-user lists of POIs
 * the user accumulates "stamps" against by hearing the narration.
 *
 * Routes (all gated by /admin Basic Auth):
 *   GET    /admin/salem/collections               — list all filters
 *   GET    /admin/salem/collections/:id           — get one filter + baked POI list
 *   POST   /admin/salem/collections               — create a filter
 *   PUT    /admin/salem/collections/:id           — update a filter
 *   DELETE /admin/salem/collections/:id           — delete a filter (cascades to pois)
 *   POST   /admin/salem/collections/preview       — run filter SQL without saving, returns count + sample
 *
 ...
 *   - tour_id is metadata-only in V1 (groups collections under a tour name).
 ...
 *
 * The actual bake (writing rows into salem_collection_entries) lives in
 * cache-proxy/scripts/publish-poi-collection.js.
 */
const MODULE_ID = '(C) Destructive AI Gurus, LLC, 2026 - Module admin-collection.js';
...
// Used by both /preview and publish-poi-collection.js (kept in this module so
...
// ════════════════════════════════════════════════════════════════════════════
// S269 — Walk-derived collection candidates.
//
...
// index along the walk. Used by the Tour-editor "Build Collection from Walk"
// dialog.
// ════════════════════════════════════════════════════════════════════════════
...
module.exports = function(app, deps) {
  ...
  (async () => {
    try {
      ...
      console.warn('[AdminCollection] auto_bake migration warning:', err.message);
    }
  })();

  // ─── GET /admin/salem/collections ─────────────────────────────────────────────
  app.get('/admin/salem/collections', requirePg, async (_req, res) => {
    try {
     ...
     LEFT JOIN salem_tours t ON t.id = pf.tour_id
      ...
      `);
      res.json({ count: rows.length,  collections: rows });
    } catch (err) {
      console.error('[AdminCollection] list error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── GET /admin/salem/collections/:id ─────────────────────────────────────────
  app.get('/admin/salem/collections/:id', requirePg, async (req, res) => {
    try {
      ...
      if (!filterQ.rows.length) return res.status(404).json({ error: 'collection not found' });
      ...
      res.json({
        collection: filterQ.rows[0],
        pois: poisQ.rows,
      });
    } catch (err) {
      console.error('[AdminCollection] get error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── POST /admin/salem/collections ────────────────────────────────────────────
  app.post('/admin/salem/collections', requirePg, async (req, res) => {
    try {
      ...
      console.error('[AdminCollection] create error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── PUT /admin/salem/collections/:id ─────────────────────────────────────────
  app.put('/admin/salem/collections/:id', requirePg, async (req, res) => {
    try {
      const id = req.params.id;
      if (!VALID_ID_RE.test(id)) {
        return res.status(400).json({ error: 'invalid collection id format' });
      ...
      const r = await pgPool.query(
        `UPDATE salem_collections SET ${setClauses.join(', ')} WHERE id = $${args.length} RETURNING *`,
        args,
      );
      if (!r.rows.length) return res.status(404).json({ error: 'collection not found' });
      res.json({ collection: r.rows[0] });
    } catch (err) {
      if (err.code === '23503') {
        return res.status(400).json({ error: `tour_id does not reference an existing tour` });
      }
      console.error('[AdminCollection] update error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── DELETE /admin/salem/collections/:id ──────────────────────────────────────
  app.delete('/admin/salem/collections/:id', requirePg, async (req, res) => {
    try {
      const id = req.params.id;
      if (!VALID_ID_RE.test(id)) {
        return res.status(400).json({ error: 'invalid collection id format' });
      ...
      const r = await pgPool.query(
        `DELETE FROM salem_collections WHERE id = $1`,
        [id],
      );
      if (!r.rowCount) return res.status(404).json({ error: 'collection not found' });
      res.json({ ok: true, deleted: id });
    } catch (err) {
      console.error('[AdminCollection] delete error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── POST /admin/salem/collections/preview ────────────────────────────────────
  // Runs the filter SQL without saving. Body carries the unsaved filter
  // parameters. Returns count + sample of matched POIs (cap 25).
  app.post('/admin/salem/collections/preview', requirePg, async (req, res) => {
    try {
      const { params, errors } = normalizeFilterParams(req.body || {});
      if (errors.length) return res.status(400).json({ error: errors.join('; ') });
      ...
      console.error('[AdminCollection] preview error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── GET /admin/salem/tours/:tourId/collection-walk-candidates ────────────────
  //
  ...
  app.get('/admin/salem/tours/:tourId/collection-walk-candidates', requirePg, async (req, res) => {
    try {
      ...
      // S269 — pool ∩ proximity. Tour collection candidates must be in the
      ...
      // non-empty historical_narration" if no global collection is authored,
      ...
      if (pool) {
        ...
        // fallback so dev installs without a default collection still see
      ...
      const savedQ = await pgPool.query(
        `SELECT pf.id AS collection_id, pf.name AS collection_name,
                ARRAY(
                  SELECT pp.poi_id FROM salem_collection_entries pp
                   WHERE pp.collection_id = pf.id
      ...
      res.json({
        ...
        // collection is authored (fallback path used).
        pool: pool
          ? {
              id: pool.id,
              name: pool.name,
      ...
      console.error('[AdminCollection] walk-candidates error:', err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── POST /admin/salem/tours/:tourId/walk-derived-collection ──────────────────
  //
  // S269 — save the operator's edited POI list as a walk-derived collection
  // for this tour. UPSERTs a salem_collections row with auto_bake=false
  // (which tells publish-poi-collection.js to preserve the salem_collection_entries
  // rows on subsequent bakes), then replaces salem_collection_entries with the
  // supplied poi_ids in the supplied order.
  app.post('/admin/salem/tours/:tourId/walk-derived-collection', requirePg, async (req, res) => {
    ...
    try {
      ...
      // Derive deterministic slug for the filter id: <tour_id>_walk_collection.
      // Sanitize to lowercase + underscores in case the tour id carries
      // hyphens or capitals; matches VALID_ID_RE for the public collections
      // API.
      const filterId = `${tourId.toLowerCase().replace(/[^a-z0-9_]/g, '_')}_walk_collection`;
      const filterName = `${tourQ.rows[0].name} — Walk-derived Collection`;
      ...
      // Replace salem_collection_entries rows for this filter.
      await client.query(`DELETE FROM salem_collection_entries WHERE collection_id = $1`, [filterId]);
      ...
      res.json({
        ok: true,
        collection_id: filterId,
        filter_name: filterName,
        poi_count: orderedIds.length,
        note: 'salem_collection_entries replaced. Re-run the publish chain (publish-poi-collection.js + align-asset-schema-to-room.js) to bake into salem_content.db.',
      });
    } catch (err) {
      try { await client.query('ROLLBACK'); } catch (_) {}
      console.error('[AdminCollection] walk-derived save error:', err.message);
      res.status(500).json({ error: err.message });
    } finally {
      client.release();
    }
...
// Export for publish-poi-collection.js to reuse.
