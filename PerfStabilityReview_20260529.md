...
- **Marker scaling (S306 → S307): ✅ TUNED + Lenovo-deployed, operator "looks good".** S306 added universal device-size-aware scale (`1a59b1c`, `MarkerIconHelper.markerScale`, fixes tiny markers on the low-density Lenovo) but overshot. S307 two-piece tune (committed at S307 close): **(1) equalize** — ghost badges now share the regular POI size ladder per zoom bucket (`narrationIconForZoom` 40/30/24/18 → 32/20/12/8 = the `dot`/`labeledDot`-icon ladder) so they render identically; **(2) −33%** — all POI marker graphics rendered 33% smaller via a single knob `MarkerIconHelper.POI_GRAPHIC_RENDER_SCALE = 0.67` (ghost + dot + labeled-icon together; scoped to POIs, NOT route/METAR/transit/GPS/system; `markerScale()` device factor still sits in front; `labeledDot` gained an `iconScale` param). Re-tune = one number.
