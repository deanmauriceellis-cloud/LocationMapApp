...
- **Android publish chain (must run in order before any debug/release build).** **Preferred (S304): `node cache-proxy/scripts/publish-all.js`** — fail-fast orchestrator that runs all 6 steps below in this exact order and prints the `manifestHash` at the end (`--dry-run` runs steps 1–4 count-only with no writes and skips align/sign for a safe PG-connectivity/wiring check; `--from=N` resumes at step N). The manual 6-step sequence remains the canonical reference order and the fallback when a single step needs isolating:
  ...
  5. `node cache-proxy/scripts/align-asset-schema-to-room.js` (auto-discovers latest `app-salem/schemas/<DB>/<v>.json`, stamps Room `identity_hash` + `PRAGMA user_version`, rewrites every Room-managed table via canonical `createSql`. Skipping triggers `fallbackToDestructiveMigration` on first launch and wipes the asset.)
  6. `node cache-proxy/scripts/sign-content-manifest.js` (S293, OMEN-025 Phase 1 Layer 2 — **MUST run last, AFTER align**, because align rewrites `salem_content.db`. SHA-256s the in-scope content assets (`salem_content.db` + `splash_tree_v1.json` + `us_places_v1.sqlite`), records large assets presence-only as `unverifiedLarge`, asserts the manifest's `roomIdentityHash` == `room_master_table` id=42 (catches a skipped align), signs with the offline RSA key, emits `content-manifest.json` + `.sig` into the assets dir, and prints `manifestHash` to paste into the activation Worker's `EXPECTED_MANIFEST_HASHES`. On CI (no private key) the manifest is emitted unsigned with a warning — the signed `.sig` is produced only on the operator's release bake.)
