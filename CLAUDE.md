...
**Version:** v1.5.70 (109 sessions completed as of 2026-04-09; S109 was a side-quest session — operator pivoted from 9P.11 to investigate "the admin tool doesn't have all the POIs" then to investigate "GPS was terrible during my Salem test" — that surfaced four independent fixes plus a new feature: column drift cleanup on `salem_narration_points` (drop dead `pass1_narration`, rename pass2/3 to match bundled side, fix the importer to read `salem_content.db` instead of the unpopulated `.sql`, fix `created_at=0` rows, PG narration_points went from 814 NULL rows to 817 fully populated), proportional GPS dead zone in `SalemMainActivity` (`100m` static → `2× reported accuracy` clamped `[5m, 100m]`, root cause of "10-15 sec GPS updates" while slowly driving), idlePopulate trigger gated to tour mode only (was wasting bandwidth in Explore), and a new "GPS Journey" feature: always-on Room recorder in a new `UserDataDatabase`, 24h rolling retention, bottom-left FAB toggles polyline visibility (recording independent of toggle, manual on/off deferred to utility menu). Lenovo TB305FU GPS hardware audited — chip is fine (lifetime mean TTFF 6.49s, accuracy 3.09m, 4 constellations); the bug was app-side. Also surfaced but NOT fixed: `DWELL_MS` constant in `NarrationGeofenceManager` is dead code (declared, never wired), so single-fix glimpses can fire false-positive narration triggers in dense areas)
