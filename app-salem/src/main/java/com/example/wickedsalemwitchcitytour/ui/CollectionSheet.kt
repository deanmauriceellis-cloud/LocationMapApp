...
/**
 * S268 — Katrina's Collection sheet.
 *
 ...
 * authored collection list (from baked Room `collection_entry`) joined against the
 * user's lifetime visit log (`poi_visit` in `user_data.db`). Each POI
 * row shows a filled/hollow witch-hat marker + heard-count + first-heard date.
 *
 * Persistence:
 *   - The collection list itself is baked into salem_content.db (read-only).
 ...
 */
@AndroidEntryPoint
class CollectionSheet : DialogFragment() {

    @Inject internal lateinit var collectionEntryDao: CollectionEntryDao
    @Inject internal lateinit var poiVisitDao: PoiVisitDao

    /** Active collection id; null means "first available from the bake." */
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.sheet_collection, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        titleView    = view.findViewById(R.id.collectionTitle)
        counterView  = view.findViewById(R.id.collectionCounter)
        ...
        pickerView.setOnClickListener { showCollectionPickerMenu(it) }

        // S269 — caller-supplied target collection (e.g. tour-completion CTA
        // hands us the just-ended tour's collectionId; toolbar tap during an
        // active tour passes the active tour's collectionId). When null we
        // fall back to the first collection from the bake.
        val argPid = arguments?.getString(ARG_COLLECTION_ID)
        if (!argPid.isNullOrBlank()) activeCollectionId = argPid
    ...
    private suspend fun buildRowsForCollection(collectionId: String): RowResult {
        ...
        val visitMap = poiVisitDao.listAll()
        ...
        val orderedCats = grouped.entries.sortedWith(
            compareByDescending<Map.Entry<String, List<CollectionEntry>>> { it.value.size }
                .thenBy { it.key }
        )
    ...
    private fun applyState(newRows: List<RowItem>, heard: Int, total: Int) {
        ...
        val active = allCollections.firstOrNull { it.collection_id == activeCollectionId }
            ?: allCollections.firstOrNull()
        titleView.text = active?.collection_name ?: "Katrina's Collection"
        if (allCollections.isEmpty() || newRows.isEmpty()) {
        ...
        pickerView.visibility = if (allCollections.size > 1) View.VISIBLE else View.GONE
        if (pickerView.visibility == View.VISIBLE) {
            pickerView.text = "Switch collection ▾"
        }
    }
    ...
    private fun showOverflowMenu(anchor: View) {
        ...
        popup.menu.add(0, MENU_RESET, 1, "Reset collection")
    ...
    private fun showCollectionPickerMenu(anchor: View) {
        if (allCollections.size <= 1) return
        val popup = PopupMenu(requireContext(), anchor)
        allCollections.forEachIndexed { idx, p ->
            val tag = if (p.tour_id == null) "(global)" else "(tour)"
            popup.menu.add(0, idx, idx, "${p.collection_name}  $tag")
        }
        popup.setOnMenuItemClickListener { item ->
            val picked = allCollections.getOrNull(item.itemId) ?: return@setOnMenuItemClickListener true
            if (picked.collection_id != activeCollectionId) {
                activeCollectionId = picked.collection_id
                loadAll()
            }
            true
        }
    ...
    private fun confirmReset() {
        AlertDialog.Builder(requireContext())
            .setTitle("Reset collection?")
            .setMessage("This clears every POI you've ever heard from the Collection. This cannot be undone.")
            .setNegativeButton("Cancel", null)
            .setPositiveButton("Reset") { _, _ ->
                lifecycleScope.launch {
                    val n = withContext(Dispatchers.IO) { poiVisitDao.deleteAll() }
                    DebugLogger.i(TAG, "Collection reset: deleted $n visit rows")
                    loadAll()
                }
            }
            .show()
    ...
    private val adapter = object : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
            ...
            return if (viewType == TYPE_HEADER) {
                HeaderVh(li.inflate(R.layout.item_collection_category_header, parent, false))
            } else {
                PoiVh(li.inflate(R.layout.item_collection_row, parent, false))
            }
        }
    ...
    private data class LoadSnapshot(
        val collections: List<CollectionEntryDao.CollectionSummary>,
    ...
    companion object {
        ...
        private const val TAG_SHOW = "collection_sheet"
         ...
         * collection on open (e.g. tour-completion CTA → the just-ended tour's collection;
         * collection; toolbar tap during an active tour → the active tour's collection.
         * collection). Pass null (or omit) for "first collection in the bake".
         */
        fun show(fragmentManager: FragmentManager, collectionId: String? = null) {
            (fragmentManager.findFragmentByTag(TAG_SHOW) as? CollectionSheet)?.dismissAllowingStateLoss()
            val sheet = CollectionSheet()
            if (!collectionId.isNullOrBlank()) {
                sheet.arguments = Bundle().apply { putString(ARG_COLLECTION_ID, collectionId) }
            }
