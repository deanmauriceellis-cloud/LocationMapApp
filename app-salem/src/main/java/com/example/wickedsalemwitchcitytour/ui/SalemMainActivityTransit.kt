...
@Suppress("unused")
private const val MODULE_ID = "(C) Destructive AI Gurus, LLC, 2026 - Module MainActivityTransit.kt"

// =========================================================================
// MBTA TRAINS
// =========================================================================

internal fun SalemMainActivity.addTrainMarker(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
...
internal fun SalemMainActivity.buildTrainSnippet(v: com.example.locationmapapp.data.model.MbtaVehicle): String {
    val statusLine = when {
 ...
 */
internal fun SalemMainActivity.vehicleStalenessTag(isoTimestamp: String): String {
    return try {
...
internal fun SalemMainActivity.clearTrainMarkers() {
 ...
 */
internal fun SalemMainActivity.startTrainRefresh() {
    trainRefreshJob?.cancel()
    DebugLogger.i("SalemMainActivity", "Starting MBTA train refresh every ${trainRefreshIntervalSec}s")
...
internal fun SalemMainActivity.stopTrainRefresh() {
    trainRefreshJob?.cancel()
    trainRefreshJob = null
    DebugLogger.i("SalemMainActivity", "MBTA train refresh stopped")
}
...
internal fun SalemMainActivity.addSubwayMarker(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
    val tint = vehicleRouteColor(vehicle)
    val labeled = binding.mapView.zoomLevelDouble >= 16.0
    val isStale = vehicleStalenessTag(vehicle.updatedAt).isNotEmpty()
    val m = BillboardMarker(binding.mapView).apply {
...
internal fun SalemMainActivity.clearSubwayMarkers() {
    subwayMarkers.forEach { binding.mapView.overlays.remove(it) }
    subwayMarkers.clear()
    binding.mapView.invalidate()
}

internal fun SalemMainActivity.startSubwayRefresh() {
    subwayRefreshJob?.cancel()
    DebugLogger.i("SalemMainActivity", "Starting MBTA subway refresh every ${subwayRefreshIntervalSec}s")
...
internal fun SalemMainActivity.stopSubwayRefresh() {
    subwayRefreshJob?.cancel()
    subwayRefreshJob = null
    DebugLogger.i("SalemMainActivity", "MBTA subway refresh stopped")
}
...
internal fun SalemMainActivity.addBusMarker(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
    val tint = vehicleRouteColor(vehicle)
    val labeled = binding.mapView.zoomLevelDouble >= 16.0
    val isStale = vehicleStalenessTag(vehicle.updatedAt).isNotEmpty()
    val m = BillboardMarker(binding.mapView).apply {
...
internal fun SalemMainActivity.clearBusMarkers() {
    busMarkers.forEach { binding.mapView.overlays.remove(it) }
    busMarkers.clear()
    binding.mapView.invalidate()
}

internal fun SalemMainActivity.startBusRefresh() {
    busRefreshJob?.cancel()
    DebugLogger.i("SalemMainActivity", "Starting MBTA bus refresh every ${busRefreshIntervalSec}s")
...
internal fun SalemMainActivity.stopBusRefresh() {
    busRefreshJob?.cancel()
    busRefreshJob = null
    DebugLogger.i("SalemMainActivity", "MBTA bus refresh stopped")
}
...
internal fun SalemMainActivity.routeColor(routeId: String): Int = when {
...
internal fun SalemMainActivity.routeAbbrev(routeId: String): String = when {
    routeId == "Red"                          -> "RL"
    routeId == "Orange"                       -> "OL"
    routeId == "Blue"                         -> "BL"
    routeId == "Mattapan"                     -> "M"
...
internal fun SalemMainActivity.addStationMarker(stop: com.example.locationmapapp.data.model.MbtaStop) {
...
internal fun SalemMainActivity.addStationMarkersBatched(stations: List<com.example.locationmapapp.data.model.MbtaStop>) {
...
internal fun SalemMainActivity.clearStationMarkers() {
...
internal fun SalemMainActivity.scheduleBusStopReload() {
...
internal fun SalemMainActivity.refreshBusStopMarkersForViewport() {
    clearBusStopMarkers()
    ...
    DebugLogger.d("SalemMainActivity", "Bus stops viewport: ${visible.size} of ${allBusStops.size} visible at zoom ${zoom.toInt()}")
...
internal fun SalemMainActivity.clearBusStopMarkers() {
...
internal fun SalemMainActivity.showArrivalBoardDialog(stop: com.example.locationmapapp.data.model.MbtaStop) {
...
internal fun SalemMainActivity.formatArrivalTime(isoTime: String?): String {
...
@SuppressLint("SetTextI18n")
internal fun SalemMainActivity.showTripScheduleDialog(pred: com.example.locationmapapp.data.model.MbtaPrediction) {
    val density = resources.displayMetrics.density
    val dp = { v: Int -> (v * density).toInt() }
...
internal fun SalemMainActivity.formatScheduleTime(isoTime: String?): String {
    if (isoTime == null) return "—"
    return try {
        val sdf = java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX", java.util.Locale.US)
...
internal fun SalemMainActivity.onVehicleMarkerTapped(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
...
@SuppressLint("SetTextI18n")
internal fun SalemMainActivity.showVehicleDetailDialog(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
    val density = resources.displayMetrics.density
    val dp = { v: Int -> (v * density).toInt() }
