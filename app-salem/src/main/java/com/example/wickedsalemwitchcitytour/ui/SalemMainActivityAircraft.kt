...
@Suppress("unused")
private const val MODULE_ID = "(C) Destructive AI Gurus, LLC, 2026 - Module MainActivityAircraft.kt"
...
internal fun SalemMainActivity.altitudeColor(altitudeMeters: Double?, onGround: Boolean = false): Int {
...
internal fun SalemMainActivity.buildAircraftSnippet(s: com.example.locationmapapp.data.model.AircraftState): String {
...
internal fun SalemMainActivity.aircraftCategoryName(cat: Int): String = when (cat) {
    0  -> ""
    1  -> "No ADS-B category"
    2  -> "Light (<15,500 lbs)"
...
internal fun SalemMainActivity.clearAircraftMarkers() {
...
internal fun SalemMainActivity.redrawFlightTrail() {
...
internal fun SalemMainActivity.appendToFlightTrail(state: com.example.locationmapapp.data.model.AircraftState) {
...
internal fun SalemMainActivity.clearFlightTrail() {
    flightTrailOverlays.forEach { binding.mapView.overlays.remove(it) }
    flightTrailOverlays.clear()
...
internal fun SalemMainActivity.loadFlightTrailHistory(icao24: String, currentState: com.example.locationmapapp.data.model.AircraftState? = null) {
    lifecycleScope.launch {
        ...
        redrawFlightTrail()
        DebugLogger.i("SalemMainActivity", "Flight trail loaded: ${flightTrailPoints.size} points, ${flightTrailOverlays.size} segments")
    }
}

internal fun SalemMainActivity.startAircraftRefresh() {
    aircraftRefreshJob?.cancel()
    DebugLogger.i("SalemMainActivity", "Starting aircraft refresh every ${aircraftRefreshIntervalSec}s")
...
internal fun SalemMainActivity.stopAircraftRefresh() {
    aircraftRefreshJob?.cancel()
    aircraftRefreshJob = null
    DebugLogger.i("SalemMainActivity", "Aircraft refresh stopped")
}

/** Get the route color for a vehicle (handles buses with numeric route IDs). */
internal fun SalemMainActivity.vehicleRouteColor(vehicle: com.example.locationmapapp.data.model.MbtaVehicle): Int {
...
internal fun SalemMainActivity.startFollowing(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
    ...
    DebugLogger.i("SalemMainActivity", "Following vehicle ${vehicle.id} (${vehicle.label} — ${vehicle.routeName})")
...
internal fun SalemMainActivity.stopFollowing() {
    DebugLogger.i("SalemMainActivity", "Stopped following vehicle=$followedVehicleId aircraft=$followedAircraftIcao")
    clearFlightTrail()
...
internal fun SalemMainActivity.updateFollowedVehicle(vehicles: List<com.example.locationmapapp.data.model.MbtaVehicle>) {
    ...
    if (vehicle != null) {
        ...
        if (moved) {
            lastFollowedVehicleLat = vehicle.lat
            lastFollowedVehicleLon = vehicle.lon
            DebugLogger.i("SalemMainActivity", "Follow update: ${vehicle.label} at ${vehicle.lat},${vehicle.lon}")
            binding.mapView.controller.animateTo(vehicle.toGeoPoint())
            // Pre-fill cache: search POIs around the vehicle's current position
            val point = vehicle.toGeoPoint()
            DebugLogger.i("SalemMainActivity", "Follow POI prefetch at ${point.latitude},${point.longitude}")
...
internal fun SalemMainActivity.showFollowBanner(vehicle: com.example.locationmapapp.data.model.MbtaVehicle) {
...
internal fun SalemMainActivity.hideFollowBanner() {
...
internal fun SalemMainActivity.onAircraftMarkerTapped(state: com.example.locationmapapp.data.model.AircraftState) {
...
internal fun SalemMainActivity.startFollowingAircraft(state: com.example.locationmapapp.data.model.AircraftState) {
    // Stop any existing vehicle follow
    followedVehicleId = null
    followedAircraftIcao = state.icao24
    lastFollowedAircraftState = state
    DebugLogger.i("SalemMainActivity", "Following aircraft ${state.icao24} (${state.callsign})")
...
internal fun SalemMainActivity.startFollowedAircraftRefresh(icao24: String) {
    followedAircraftRefreshJob?.cancel()
    DebugLogger.i("SalemMainActivity", "Starting followed aircraft refresh for $icao24 every ${aircraftRefreshIntervalSec}s")
...
internal fun SalemMainActivity.stopFollowedAircraftRefresh() {
    followedAircraftRefreshJob?.cancel()
...
internal fun SalemMainActivity.startAutoFollowAircraft() {
    ...
    if (!prefs.getBoolean(MenuPrefs.PREF_AIRCRAFT_DISPLAY, true)) {
        ...
        DebugLogger.i("SalemMainActivity", "Auto-follow: enabled aircraft layer")
    }
    ...
    autoFollowAircraftJob = lifecycleScope.launch {
        while (true) {
            ...
            DebugLogger.i("SalemMainActivity", "Auto-follow: 20-min rotation — picking new aircraft")
            pickAndFollowRandomAircraft()
        }
    }
...
internal fun SalemMainActivity.stopAutoFollowAircraft() {
    autoFollowAircraftJob?.cancel()
    autoFollowAircraftJob = null
    stopFollowing()
    toast("Auto-follow stopped")
    DebugLogger.i("SalemMainActivity", "Auto-follow stopped")
}
 ...
 */
internal fun SalemMainActivity.pickAndFollowRandomAircraft() {
    ...
    DebugLogger.i("SalemMainActivity", "Auto-follow: wide bbox query ${south},${west},${north},${east} pref=$dirLabel")
    ...
    lifecycleScope.launch {
        ...
        if (freshCandidates.isNotEmpty()) {
            ...
            DebugLogger.i("SalemMainActivity", "Auto-follow: no aircraft ≥ 10,000 ft in wide bbox — will retry in 20 min")
            toast("No aircraft above 10,000 ft — will retry")
        }
    }
}

internal fun SalemMainActivity.filterHighAltitude(
...
internal fun SalemMainActivity.selectAndFollow(candidates: List<com.example.locationmapapp.data.model.AircraftState>) {
    ...
    val pick = if (nsFiltered.isNotEmpty()) {
        DebugLogger.i("SalemMainActivity", "Auto-follow: ${nsFiltered.size} $dirLabel of ${pool.size} candidates")
        nsFiltered.random()
    } else if (ewFiltered.isNotEmpty()) {
        DebugLogger.i("SalemMainActivity", "Auto-follow: ${ewFiltered.size} ${if (autoFollowPreferWest) "W" else "E"}-bound of ${pool.size} (relaxed N/S)")
        ewFiltered.random()
    } else {
        DebugLogger.i("SalemMainActivity", "Auto-follow: no $dirLabel — picking from all ${pool.size}")
        pool.random()
    }
    val hdg = pick.track?.let { "%.0f°".format(it) } ?: "?"
    DebugLogger.i("SalemMainActivity", "Auto-follow: selected ${pick.icao24} (${pick.callsign}) " +
...
internal fun SalemMainActivity.updateFollowedAircraft(aircraft: List<com.example.locationmapapp.data.model.AircraftState>) {
...
internal fun SalemMainActivity.showAircraftFollowBanner(state: com.example.locationmapapp.data.model.AircraftState) {
