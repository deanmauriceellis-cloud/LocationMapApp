...
@Suppress("unused")
private const val MODULE_ID = "(C) Destructive AI Gurus, LLC, 2026 - Module MainActivityPopulate.kt"
...
internal fun SalemMainActivity.startSilentFill(center: org.osmdroid.util.GeoPoint) {
    ...
    silentFillJob = lifecycleScope.launch {
        DebugLogger.i("SalemMainActivity", "Silent fill starting at ${center.latitude},${center.longitude}")
        ...
        try {
            ...
            if (result != null) {
                ...
                DebugLogger.i("SalemMainActivity", "Silent fill complete — ${result.results.size} POIs (${result.poiNew} new) at ${result.radiusM}m coverage=${result.coverageStatus}")
                ...
                DebugLogger.w("SalemMainActivity", "Silent fill returned null (error)")
                if (showDebug) {
            ...
            DebugLogger.i("SalemMainActivity", "Silent fill cancelled")
            if (showDebug) hideSilentFillBanner()
        } catch (e: Exception) {
            DebugLogger.e("SalemMainActivity", "Silent fill error: ${e.message}", e)
            if (showDebug) {
...
internal fun SalemMainActivity.scheduleSilentFill(center: org.osmdroid.util.GeoPoint, delayMs: Long) {
...
internal fun SalemMainActivity.stopSilentFill() {
    silentFillRunnable?.let { binding.mapView.removeCallbacks(it) }
...
internal fun SalemMainActivity.showSilentFillBanner(text: String) {
    runOnUiThread {
        statusLineManager.set(StatusLineManager.Priority.SILENT_FILL, "\uD83D\uDD0D $text") { stopSilentFill() }
    }
}

internal fun SalemMainActivity.hideSilentFillBanner() {
...
internal fun SalemMainActivity.startIdlePopulate(center: GeoPoint) {
    ...
    if (saved != null && saved.calibrated
        ...
        DebugLogger.i("SalemMainActivity", "startIdlePopulate() RESUMING from R${saved.ring} pt${saved.pointIdx} (${saved.stats.cells} cells done)")
        resumeIdlePopulate(saved)
        return
    }
    idlePopulateState = null  // discard stale state

    DebugLogger.i("SalemMainActivity", "startIdlePopulate() FRESH center=$centerLat,$centerLon")
    ...
    idlePopulateJob = lifecycleScope.launch {
        ...
        for (probeAttempt in 1..3) {
            ...
            DebugLogger.w("SalemMainActivity", "Idle probe attempt $probeAttempt failed — retrying in 45s")
        ...
        if (probeResult == null) {
            DebugLogger.w("SalemMainActivity", "Idle probe failed after 3 attempts")
...
internal fun SalemMainActivity.resumeIdlePopulate(state: IdlePopulateState) {
    idlePopulateState = state
...
internal suspend fun SalemMainActivity.runIdlePopulateSpiral(state: IdlePopulateState) {
        ...
        while (pointIdx < points.size) {
            ...
            if (result != null) {
                ...
                if (isFresh) {
                    ...
                    DebugLogger.d("SalemMainActivity", "Idle R$ring pt$pointIdx: FRESH — skipping")
                    showIdlePopulateBanner(ring, state.stats, 0)
                    continue
                }
                ...
                if (state.consecutiveErrors >= 5) {
                    DebugLogger.w("SalemMainActivity", "Idle populate auto-stopped after 5 consecutive errors")
                    break
                }
            }
...
internal fun SalemMainActivity.stopIdlePopulate(clearState: Boolean = false) {
    ...
    DebugLogger.i("SalemMainActivity", "stopIdlePopulate()$resumeInfo")
}

internal fun SalemMainActivity.showIdlePopulateBanner(ring: Int, stats: PopulateStats, countdown: Int) {
...
internal fun SalemMainActivity.estimateFillRadius(poiCount: Int, probeRadiusM: Int): Int {
...
internal fun SalemMainActivity.startProbe10km() {
    ...
    DebugLogger.i("SalemMainActivity", "startProbe10km() center=$centerLat,$centerLon stepLat=${"%.5f".format(stepLat)} stepLon=${"%.5f".format(stepLon)}")
...
internal fun SalemMainActivity.stopProbe10km() {
    probe10kmJob?.cancel()
    probe10kmJob = null
    viewModel.cancelPendingOverpass()
    removeScanningMarker()
    statusLineManager.clear(StatusLineManager.Priority.POPULATE)
    DebugLogger.i("SalemMainActivity", "stopProbe10km()")
}

internal fun SalemMainActivity.showProbe10kmBanner(ring: Int, ptInRing: Int, probes: Int, pois: Int, newPois: Int,
...
internal fun SalemMainActivity.startPopulatePois() {
    ...
    val centerLat = center.latitude
    val centerLon = center.longitude
    DebugLogger.i("SalemMainActivity", "startPopulatePois() center=$centerLat,$centerLon")
    ...
    populateJob = lifecycleScope.launch {
        ...
        for (probeAttempt in 1..3) {
            ...
            if (probeResult != null) break
            DebugLogger.w("SalemMainActivity", "Probe attempt $probeAttempt failed — retrying in 30s")
            ...
            while (pointIdx < points.size) {
                ...
                if (result != null) {
                    ...
                    if (isFresh) {
                        ...
                        DebugLogger.d("SalemMainActivity", "Populate R$ring pt$pointIdx: FRESH \u2014 skipping")
                        showPopulateBanner(ring, stats, 0)
                        continue
                    }
                    ...
                    if (consecutiveErrors >= 5) {
                        ...
                        DebugLogger.w("SalemMainActivity", "Populate auto-stopped after 5 consecutive errors")
                        break
                    }
                }
 ...
 */
internal suspend fun SalemMainActivity.searchCellSubdivisions(
    center: GeoPoint,
...
internal fun SalemMainActivity.stopPopulatePois() {
    populateJob?.cancel()
    populateJob = null
    viewModel.cancelPendingOverpass()
    removeScanningMarker()
    ...
    val prefs = getSharedPreferences("app_bar_menu_prefs", Context.MODE_PRIVATE)
    prefs.edit().putBoolean(MenuPrefs.PREF_POPULATE_POIS, false).apply()
    DebugLogger.i("SalemMainActivity", "stopPopulatePois()")
}
 ...
 */
internal fun SalemMainActivity.generateRingPoints(
...
internal fun SalemMainActivity.removeScanningMarker() {
    scanningMarker?.let { binding.mapView.overlays.remove(it) }
    scanningMarker = null
    binding.mapView.invalidate()
}

internal fun SalemMainActivity.showPopulateBanner(ring: Int, stats: PopulateStats, countdown: Int) {
    val countdownStr = if (countdown > 0) " ${countdown}s" else ""
    val failStr = if (stats.fails > 0) " \u26A0${stats.fails}err" else ""
    val gridStr = if (stats.gridRadius > 0) "${stats.gridRadius}m" else "probe"
