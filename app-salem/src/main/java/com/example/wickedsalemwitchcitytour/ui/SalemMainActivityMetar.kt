...
internal fun SalemMainActivity.addMetarMarker(station: com.example.locationmapapp.data.model.MetarStation) {
...
internal fun SalemMainActivity.buildMetarStationIcon(s: com.example.locationmapapp.data.model.MetarStation, color: Int): android.graphics.drawable.Drawable {
...
internal fun SalemMainActivity.windDirToArrow(deg: Int): String = when ((deg + 22) / 45 % 8) {
    0 -> "↓"; 1 -> "↙"; 2 -> "←"; 3 -> "↖"
    4 -> "↑"; 5 -> "↗"; 6 -> "→"; 7 -> "↘"
    else -> ""
}

internal fun SalemMainActivity.buildPlaceSnippet(p: com.example.locationmapapp.data.model.PlaceResult) =
    listOfNotNull(p.address, p.phone, p.openingHours).joinToString("\n").ifBlank { p.category }

internal fun SalemMainActivity.buildMetarSnippet(s: com.example.locationmapapp.data.model.MetarStation): String {
...
internal fun SalemMainActivity.degreesToCompass(deg: Int): String = when ((deg + 22) / 45 % 8) {
    0 -> "North"; 1 -> "Northeast"; 2 -> "East"; 3 -> "Southeast"
    4 -> "South"; 5 -> "Southwest"; 6 -> "West"; 7 -> "Northwest"
    else -> "${deg}°"
}

internal fun SalemMainActivity.decodeWx(wx: String): String {
    val map = mapOf(
...
fun SalemMainActivity.clearMetarMarkers() {
...
internal fun SalemMainActivity.refreshMetarMarkerIcons() {
    val showLabels = binding.mapView.zoomLevelDouble >= METAR_LABEL_ZOOM
