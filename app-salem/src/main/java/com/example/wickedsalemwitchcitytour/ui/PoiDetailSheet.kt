...
class PoiDetailSheet : DialogFragment() {
    ...
    /** S268 — Katrina's Collection visit log. Recorded when this sheet actually queues
    ...
    // ── Subtopics (S219) ────────────────────────────────────────────────
    //
    // narration_subtopics is a JSONB array of {header, body, source_kind?,
    // source_ref?} — companion content rendered as a horizontal chip strip
    ...
    private fun bindDetourButton(view: View) {
        ...
        val activeTour = (state as? com.example.wickedsalemwitchcitytour.tour.TourState.Active)?.activeTour
            ?: return
        // S269 — skip if this POI is already a  collection member of the
        // active tour. Pre-S269 the guard read `activeTour.currentPoi`
        // (the currently-pointed stop), which is gone with the stops shed;
        // the  collection membership check is the right replacement because
        ...
        if (poi.id in activeTour.collectionPoiIds) return
    ...
    private fun queueSheetReadThrough(
        ...
        //
        // Fallback rule (operator-clarified S193): when the POI is
        // HISTORICAL_BUILDINGS and historical_narration isn't authored yet,
        // fall back to short_narration ONLY — long_narration is rich modern
        // content that would dominate a historical tour with present-day
        // commentary. Non-historic-buildings on a tour stay intro-only so
        // modern blurbs don't sneak in.
        ...
        if (historicalActive) {
            ...
            if (isHistBldg && shortText != null) {
                tourViewModel.speakSheetSection(ttsTag, shortText, name)
                DebugLogger.i(TAG, "tts enqueue HIST_BLDG_SHORT_FALLBACK id=${poi.id} len=${shortText.length}")
                return
            }
            // Non-HIST_BLDG without historical_narration on a tour → intro only.
            // HIST_BLDG with no historical AND no short → also intro only.
            DebugLogger.i(TAG, "tts intro-only id=${poi.id} cat=${poi.category} (no historical${if (isHistBldg) ", no short" else ", not hist-bldg"})")
            return
        }
...
private fun salemPoiToJson(poi: SalemPoi): String = JSONObject().apply {
    ...
    putOpt("narration_subtopics", poi.narrationSubtopics)
...
private fun parseSalemPoi(s: String): SalemPoi? {
    return try {
        ...
        SalemPoi(
            ...
            narrationSubtopics = o.optString("narration_subtopics", "").takeIf { it.isNotEmpty() },
