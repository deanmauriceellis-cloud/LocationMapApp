...
object SalemTours {
    ...
    fun allTours(): List<OutputTour> = listOf(
        heritageTrailTour, essentialsTour, explorerTour, grandTour, witchTrialsTour
    )

    fun allStops(): List<OutputTourStop> =
        heritageTrailStops + essentialsStops + explorerStops + grandStops + witchTrialsStops
    ...
    // ═══════════════════════════════════════════════════════════════════
    // Tour 5 (Phase 9R.0): Salem Heritage Trail — 10 stops, ~78 min, ~5.8 km
    //
    // The official Yellow Line walking loop. Starts and ends at the NPS
    // Regional Visitor Center. Theme = "HERITAGE_TRAIL" is the trigger that
    // auto-engages Historical Mode in TourEngine.startTour() — modern POIs
    // are filtered out and per-POI `historical_note` text (tour-guide voice)
    // takes over from the ambient `short_narration`.
    //
    // The pre-computed OSRM foot-profile polyline lives in
    //   app-salem/src/main/assets/tours/tour_salem_heritage_trail.json
    // and is generated by
    //   cache-proxy/scripts/build-heritage-trail-route.js
    //
    // The 10 anchor POIs' `historical_note` content is synced from
    // SalemIntelligence by
    //   cache-proxy/scripts/sync-historical-notes-from-intel.js
    // ═══════════════════════════════════════════════════════════════════

    private val heritageTrailTour = OutputTour(
        id = "tour_salem_heritage_trail",
        name = "Salem Heritage Trail",
        theme = "HERITAGE_TRAIL",
        description = "The official 2.7-mile Salem Heritage Trail — the Yellow Line painted " +
            "on the sidewalks of downtown Salem. A historical walking loop starting and ending " +
            "at the Regional Visitor Center and passing the Old Jail, Salem Common, Maritime " +
            "National Historic Site, Custom House, Old Town Hall, Witch Trials Memorial, " +
            "Bewitched statue, The Witch House, and the McIntire Historic District. " +
            "Starting this tour puts the app into Historical Mode: modern shops and restaurants " +
            "are hidden from the map, and only historical buildings narrate as you pass them.",
        estimatedMinutes = 78,
        distanceKm = 5.8f,
        stopCount = 10,
        difficulty = "easy",
    ...
    private val heritageTrailStops = listOf(
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "national_park_service_visitor_center", stopOrder = 1,
            transitionNarration = "Welcome to the Salem Heritage Trail. You're standing at the " +
                "National Park Service Regional Visitor Center, our starting point. Grab a map " +
                "inside if you haven't already, then head north on Liberty Street toward Bridge " +
                "Street to begin the loop.",
            walkingMinutesFromPrev = 0, distanceMFromPrev = 0,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "salem_jail_site", stopOrder = 2,
            transitionNarration = "We're now approaching the Old Salem Jail Site near Bridge " +
                "Street and St. Peter Street — the outer edge of the historic downtown, and a " +
                "place where the 1692 trials reached their darkest hour.",
            walkingMinutesFromPrev = 5, distanceMFromPrev = 380,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "salem_common", stopOrder = 3,
            transitionNarration = "Heading south, we're entering Salem Common at Brown Street " +
                "and Washington Square. This open green has been the town's gathering place " +
                "since the 1630s.",
            walkingMinutesFromPrev = 5, distanceMFromPrev = 366,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "salem_maritime_national_historical_park", stopOrder = 4,
            transitionNarration = "South on Hawthorne Boulevard toward Derby Street — we're " +
                "crossing into Salem Maritime National Historic Site, the waterfront that once " +
                "made Salem the wealthiest port in America.",
            walkingMinutesFromPrev = 7, distanceMFromPrev = 540,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "custom_house", stopOrder = 5,
            transitionNarration = "The 1819 Custom House sits ahead at Derby Street and Orange " +
                "Street, where Nathaniel Hawthorne worked as a surveyor and where ship captains " +
                "paid their duties.",
            walkingMinutesFromPrev = 4, distanceMFromPrev = 312,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "old_town_hall", stopOrder = 6,
            transitionNarration = "Turning west on Derby Street, we arrive at Old Town Hall in " +
                "Derby Square — Salem's early-19th-century civic heart and market.",
            walkingMinutesFromPrev = 5, distanceMFromPrev = 329,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "salem_witch_trials_memorial", stopOrder = 7,
            transitionNarration = "Up Liberty Street to Charter Street — the site of the Salem " +
                "Witch Trials Memorial and the adjoining Old Burying Point, Salem's oldest " +
                "cemetery.",
            walkingMinutesFromPrev = 3, distanceMFromPrev = 201,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "bewitched_sculpture_samantha_statue", stopOrder = 8,
            transitionNarration = "West on Essex Street to Washington Street — the Bewitched " +
                "statue marks the heart of Salem's pedestrian mall, where the 1960s TV show " +
                "once filmed on location.",
            walkingMinutesFromPrev = 6, distanceMFromPrev = 411,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "the_witch_house", stopOrder = 9,
            transitionNarration = "Continue west along Essex to North Street — ahead is The " +
                "Witch House, home of Judge Jonathan Corwin and the only structure still " +
                "standing with direct ties to the 1692 trials.",
            walkingMinutesFromPrev = 6, distanceMFromPrev = 451,
            provenance = curated
        ),
        OutputTourStop(
            tourId = "tour_salem_heritage_trail",
            poiId = "chestnut_street", stopOrder = 10,
            transitionNarration = "Heading south on Summer Street to Chestnut Street — we're " +
                "entering the McIntire Historic District, a Federal-style neighborhood of " +
                "mansions built by Salem's merchant princes and designed largely by Samuel " +
                "McIntire. Follow Chestnut, then loop back along Essex to finish at the " +
                "Visitor Center.",
            walkingMinutesFromPrev = 4, distanceMFromPrev = 310,
            provenance = curated
        )
